...
typedef struct { uint32_t h[8]; uint64_t bits; unsigned char b[64]; size_t used; } A209Sha;
static A209Counters g_ctr;
static uint32_t rr(uint32_t x,unsigned n){return(x>>n)|(x<<(32U-n));}
static void sha_block(A209Sha*s,const unsigned char*b){
 ...
 uint32_t w[64];for(int i=0;i<16;i++)w[i]=((uint32_t)b[4*i]<<24)|((uint32_t)b[4*i+1]<<16)|((uint32_t)b[4*i+2]<<8)|b[4*i+3];for(int i=16;i<64;i++){uint32_t x=w[i-15],y=w[i-2];w[i]=w[i-16]+(rr(x,7)^rr(x,18)^(x>>3))+w[i-7]+(rr(y,17)^rr(y,19)^(y>>10));}uint32_t a=s->h[0],c=s->h[2],d=s->h[3],e=s->h[4],f=s->h[5],g=s->h[6],h=s->h[7],bb=s->h[1];for(int i=0;i<64;i++){uint32_t t1=h+(rr(e,6)^rr(e,11)^rr(e,25))+((e&f)^(~e&g))+k[i]+w[i],t2=(rr(a,2)^rr(a,13)^rr(a,22))+((a&bb)^(a&c)^(bb&c));h=g;g=f;f=e;e=d+t1;d=c;c=bb;bb=a;a=t1+t2;}s->h[0]+=a;s->h[1]+=bb;s->h[2]+=c;s->h[3]+=d;s->h[4]+=e;s->h[5]+=f;s->h[6]+=g;s->h[7]+=h;
}
static void sha_init(A209Sha*s){uint32_t h[8]={0x6a09e667U,0xbb67ae85U,0x3c6ef372U,0xa54ff53aU,0x510e527fU,0x9b05688cU,0x1f83d9abU,0x5be0cd19U};memcpy(s->h,h,sizeof(h));s->bits=0;s->used=0;}
static void sha_up(A209Sha*s,const void*v,size_t n){const unsigned char*p=v;s->bits+=(uint64_t)n*8;while(n){size_t z=64-s->used,t=n<z?n:z;memcpy(s->b+s->used,p,t);s->used+=t;p+=t;n-=t;if(s->used==64){sha_block(s,s->b);s->used=0;}}}
static void sha_u64(A209Sha*s,uint64_t x){unsigned char b[8];for(int i=0;i<8;i++)b[7-i]=(unsigned char)(x>>(8*i));sha_up(s,b,8);}
static void sha_f64(A209Sha*s,double x){uint64_t u;memcpy(&u,&x,8);sha_u64(s,u);}
static void sha_done(A209Sha*s,char out[65]){uint64_t bits=s->bits;unsigned char x=0x80,z=0,l[8],d[32];sha_up(s,&x,1);while(s->used!=56)sha_up(s,&z,1);for(int i=0;i<8;i++)l[7-i]=(unsigned char)(bits>>(8*i));sha_up(s,l,8);for(int i=0;i<8;i++){d[4*i]=s->h[i]>>24;d[4*i+1]=s->h[i]>>16;d[4*i+2]=s->h[i]>>8;d[4*i+3]=s->h[i];}static const char h[]="0123456789abcdef";for(int i=0;i<32;i++){out[2*i]=h[d[i]>>4];out[2*i+1]=h[d[i]&15];}out[64]=0;}
