...
# Args: $1=FE_FAC  $2=CO_FAC  $3=NI_UV_FAC  $4=NI2_OPT_FAC
...
# Smoke turnaround: 200K x 6 iter (single seed). Internal score_nw against
...
label="nlteO_recal_fe${fe_tag}_co${co_tag}_niuv${niuv_tag}_n2o${n2o_tag}_${tag}"
...
echo "=== NLTEO-RECAL VI=$VI_KMS L=$L_scale DE=$DE_ARG | FE=$FE_FAC CO=$CO_FAC NI_UV=$NI_UV_FAC NI2_OPT=$NI2_OPT_FAC | (frozen) CA2=$CA2_FAC S9=$S9_FAC NI3=$NI3_FAC SI_OPT=$SI_OPT_FAC [${label}] ==="
...
print(f"  [RECAL fe=$FE_FAC co=$CO_FAC niuv=$NI_UV_FAC n2o=$NI2_OPT_FAC] Snifs [3300,8000]   rms_bn = {r:.4f}   chi_bn = {chi:.3f}")
    ...
    if gH > 0:
        for lo, hi, lbl in [(3000., 8000., "[3000,8000]"), (1700., 3000., "[1700,3000]")]:
            r, chi = rms_bn_nw(mlam, mflu, hl, hf, gH,
                               ...
                               wl_lo=lo, wl_hi=hi)
            print(f"  [RECAL fe=$FE_FAC co=$CO_FAC niuv=$NI_UV_FAC n2o=$NI2_OPT_FAC] HST   {lbl}  rms_bn = {r:.4f}   chi_bn = {chi:.3f}")
PYEOF
