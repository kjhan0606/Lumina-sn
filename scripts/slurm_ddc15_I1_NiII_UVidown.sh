#!/bin/bash
#SBATCH --job-name=ddc15_I1_NiII_UVidown
...
# I1: Ni II-targeted UV internal-down suppression at production fidelity.
# H3 identified Ni II as 68% of UV→red cascade — narrow the P1 knob to Ni II only.
# 800K × 12 iter (skip 200K screening per H1p lesson). C2 base (DDC15 v_in=10400).
#
...
label="ddc15I1_${tag}"
work_root="$ROOT/logs/ddc15I1_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "=== I1 Ni II UV-idown [${label}]  FACTOR=${factor}  Z=28(Ni) ion=1(II)  THRESH=4000Å  N_PKT=${N_PKT}  N_ITER=${N_ITER} ==="
...
LUMINA_MACRO_UV_IDOWN_ZMASK=28 \
