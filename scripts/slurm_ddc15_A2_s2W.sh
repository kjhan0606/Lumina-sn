...
# A2: Revive S II "W" feature 5454/5640 Å via SCALE9 A_ul boost on X1_b3p0.
#
# Diagnostic basis: Production log shows S II NLTE n_total ≈ 1e-28 (solver fails
# silently; same pathology as #223 Si II), and the code falls back to nebular
# n_ion ≈ 2e+08 with Boltzmann@T_rad — effectively LTE, so S II 5454/5640
# line shape is washed out (matches observation in HST comparison plot).
#
# X1_b3p0 base (current RMS champion: RMS_bn=0.171, J=0.237):
#   SCALE1 Ni II UV f=0.3 (Z=28, λ<4000)
#   SCALE2 Si II UV f=0.05 (Z=14, λ<4000)
#   SCALE3 Fe II UV f=0.3  (Z=26, λ<4000)
#   SCALE4 Co II UV f=0.3  (Z=27, λ<4000)
#   SCALE5 iron-peak III red f=0.3 ([4500,7500])
#   SCALE6 Si II opt damp f=0.10 ([4000,7000])
#   SCALE7 Ni II opt damp f=0.05 ([4000,7500])
#   SCALE8 Ca II f=3.0     (Z=20, [3700,9000])
#
# A2 adds:
#   SCALE9 S II f∈{1.0,1.5,2.0,3.0,5.0} (Z=16, ion=1, λ∈[5400,5700])
#
# Cells:
#   0 — ctrl : SCALE9 OFF (= X1_b3p0 reproducibility @ 0.171/0.237)
#   1 — f1p5 : S II f=1.5
#   2 — f2p0 : S II f=2.0
#   3 — f3p0 : S II f=3.0
#   4 — f5p0 : S II f=5.0
...
LABELS=(ctrl  f1p5 f2p0 f3p0 f5p0)
S2_BOOSTS=(1.0 1.5  2.0  3.0  5.0)
tag=${LABELS[$SLURM_ARRAY_TASK_ID]}
s2W=${S2_BOOSTS[$SLURM_ARRAY_TASK_ID]}
...
label="ddc15A2_${tag}"
work_root="$ROOT/logs/ddc15A2_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "=== A2 [${label}]  X1_b3p0 base + SCALE9 S II [5400,5700] f=${s2W}  N_PKT=${N_PKT}  N_ITER=${N_ITER} ==="
...
LUMINA_AUL_SCALE9_FACTOR="$s2W" \
LUMINA_AUL_SCALE9_LAMBDA_MIN=5400 \
LUMINA_AUL_SCALE9_LAMBDA_MAX=5700 \
LUMINA_AUL_SCALE9_ZMASK=16 \
...
grep -E '\[AUL_SCALE' stdout.log | head -12
