...
# V4 smoke: W5 niF005 frozen stack on CLEAN CMFGEN atomic data
# (data/tardis_reference_cmfgen with 17,743 lev / 321,887 lines / 965,661 trans).
#
# 2 cells = p+3.7d × {Pereira-L=1.00, L×1.45} (V3 champion epoch)
# Direct comparison to V3 156953 idx 2 (Snifs 0.143 / HST opt 0.170 / UV 0.375)
# and idx 6 (Snifs 0.143 / HST opt 0.170 / UV ratio 1.016).
...
TAGS=(p+3.7d p+3.7d)
T_EXP_S=(1788480 1788480)
L_BASE=(1.1e+43 1.1e+43)
L_SCALE=(1.00 1.45)
SNIFS=(sn2011fe_p4d2d.csv sn2011fe_p4d2d.csv)
HST_TAG=(phase+03.7 phase+03.7)
SUFFIX=(pereira hot1p45)
...
# W5 niF005 frozen stack (same as V3)
FE_FAC=0.3
CO_FAC=0.3
SI_OPT_FAC=0.10
NI2_OPT_FAC=0.05
...
label="v4_smoke_${tag}_${suffix}"
work_root="$ROOT/logs/v4_smoke_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "=== V4 SMOKE [${label}] L_base=${L_base} L_scale=${L_scale} L=${L_inner} N_PKT=${N_PKT} ==="
...
print(f"  V4_smoke ${tag} ${suffix}  Snifs RMS_bn [3300,8000]  = {r_snifs:.4f}  (V3 baseline 0.143)")
...
if parts:
    ...
    if gH > 0:
        ...
        print(f"  V4_smoke ${tag} ${suffix}  HST RMS_bn [3000,8000]   = {r_hst_can:.4f}  (V3 0.170)")
        print(f"  V4_smoke ${tag} ${suffix}  HST RMS_bn [1700,3000]   = {r_hst_uv:.4f}  (V3 0.375)")
        def br(lo, hi):
        ...
        ref_m, ref_h = bm["Ref"]
        print(f"  V4_smoke ${tag} ${suffix}  --- SED band ratios (LUMINA/HST, normed to Ref) ---")
        for (n,lo,hi) in bands:
            mm, hh = bm[n]
            if ref_m>0 and ref_h>0 and hh>0:
                r = (mm/ref_m)/(hh/ref_h)
                print(f"  V4_smoke ${tag} ${suffix}    band {n:<6} [{lo:5d},{hi:5d}]  L/HST = {r:.3f}")
PYEOF
