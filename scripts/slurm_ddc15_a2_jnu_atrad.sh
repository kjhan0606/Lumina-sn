#!/bin/bash
#SBATCH --job-name=ddc15_a2jnatrad
#SBATCH --partition=h200
#SBATCH --nodelist=syn104
#SBATCH --gres=gpu:1
...
# A2 fix (B): BOTH the NLTE rate-matrix assembly AND the coupled-Newton shell
# loop (was the 2nd serial bottleneck, ~30 min/iter once diffusion-BC engages)
# are now OpenMP-parallel over the 49 shells. cpus-per-task=8 -> up to 8x.
...
env LUMINA_BF_OPACITY=1 \
    ...
    LUMINA_COUPLED_JNU_PHOTOION=${LUMINA_COUPLED_JNU_PHOTOION:-1} \
    LUMINA_COUPLED_JNU_ALPHA_TRAD=${LUMINA_COUPLED_JNU_ALPHA_TRAD:-1} \
