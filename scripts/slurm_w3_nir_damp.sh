...
# W3: NIR iron-peak A_ul damp axis on L=1.30 nored anchor.
# W2 winner: L130_nored HST[3000,8000]=0.1745, but Red/HST=2.07.
# Hypothesis: SCALE9 NIR damp [7500,10000] iron-peak attacks Red excess
# without touching UV (which structure has handled).
#   0 — repro  L=1.30 nored bare (no SCALE9)
#   1 — n9F05ion1 NIR damp Z=26,27 ion=1 f=0.5
#   2 — n9F03ion1 NIR damp Z=26,27 ion=1 f=0.3
#   3 — n9F05ion2 NIR damp Z=26,27 ion=2 f=0.5
#   4 — n9F03ion2 NIR damp Z=26,27 ion=2 f=0.3
#   5 — n9F03Ni1  NIR damp Z=28    ion=1 f=0.3 (Ni-only)
#   6 — L145bare L=1.45 no SCALE9 (push hotter)
#   7 — L145n9F05 L=1.45 + SCALE9 ion=1 f=0.5 (combined hot + NIR damp)
...
# 0:repro  1:F05ion1  2:F03ion1  3:F05ion2  4:F03ion2  5:F03Ni1  6:L145bare  7:L145n9F05
L_SCALE_ARR=(1.30 1.30 1.30 1.30 1.30 1.30 1.45 1.45)
S9_FAC_ARR=(1.00 0.50 0.30 0.50 0.30 0.30 1.00 0.50)
S9_Z_ARR=("26,27" "26,27" "26,27" "26,27" "26,27" "28" "26,27" "26,27")
S9_ION_ARR=(1 1 1 2 2 1 1 1)
TAG_ARR=(repro n9F05ion1 n9F03ion1 n9F05ion2 n9F03ion2 n9F03Ni1 L145bare L145n9F05)

L_SCALE=${L_SCALE_ARR[$SLURM_ARRAY_TASK_ID]}
S9_FAC=${S9_FAC_ARR[$SLURM_ARRAY_TASK_ID]}
S9_Z=${S9_Z_ARR[$SLURM_ARRAY_TASK_ID]}
S9_ION=${S9_ION_ARR[$SLURM_ARRAY_TASK_ID]}
tag=${TAG_ARR[$SLURM_ARRAY_TASK_ID]}

L_inner=$(python3 -c "print(${L_BASE} * ${L_SCALE})")

label="w3_${tag}"
work_root="$ROOT/logs/w3_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "=== W3 [${label}] L=${L_SCALE} L_inner=${L_inner} SCALE9: Z=${S9_Z} ion=${S9_ION} f=${S9_FAC} band=[7500,10000] ==="
echo "Host: $(hostname)  GPU: $(nvidia-smi --query-gpu=name --format=csv,noheader|head -1)"
echo "Binary: $BIN"
echo "Time: $(date)"

# X1_b3p0 stack WITHOUT SCALE5 (RED damp dropped per W2 finding).
# SCALE9 = NIR iron-peak damp [7500,10000] (the new lever).
LUMINA_BF_OPACITY=1 \
...
LUMINA_AUL_SCALE9_IONMASK="$S9_ION" \
...
echo ""
echo "=== score [${label}] L=${L_SCALE} SCALE9 Z=${S9_Z} ion=${S9_ION} f=${S9_FAC} ==="
python3 <<PYEOF
...
print(f"  W3 ${tag}  Snifs RMS_bn [3300,8000]  = {r_snifs:.4f}")
...
if parts:
    ...
    if gH > 0:
        ...
        print(f"  W3 ${tag}  HST RMS_bn [3000,8000]   = {r_hst_can:.4f}")
        print(f"  W3 ${tag}  HST RMS_bn [1700,3000] = {r_hst_uv:.4f}")
