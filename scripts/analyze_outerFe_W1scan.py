#!/usr/bin/env python3
"""outerFe + W1 scan vs champion + HST: 6-band RMS + Fe II UV gap check."""
...
RUNS = {
    "champion 152761 (W1=1.7, no oFe)": CHAMP,
    "outerFe W1=1.0 (154062)": ROOT / "logs/outerFe_W11p0_154062/lumina_spectrum_formal.csv",
    "outerFe W1=1.3 (154063)": ROOT / "logs/outerFe_W11p3_154063/lumina_spectrum_formal.csv",
    "outerFe W1=1.7 (154061)": ROOT / "logs/outerFe_W11p7_154061/lumina_spectrum_formal.csv",
}

BANDS = [("UVbl",1700,2900),("CaK",3700,3950),("UVtg",2900,3700),
         ("fluo",3950,4500),("grn",4500,5800),("red",5800,7000)]

# Fe II UV diagnostics: (label, λ_rest, expected v window km/s)
...
for name, p in RUNS.items():
    ...
    # Fe II UV diagnostic depths in NORMALIZED (model/model_cont) for fair compare
...
# HST diagnostic depths (ground truth)
...
for name, (_, _, fe_dep) in results.items():
    ...
    print(f"{name:<36} | " + " | ".join(f"{c:<14}" for c in cells))
