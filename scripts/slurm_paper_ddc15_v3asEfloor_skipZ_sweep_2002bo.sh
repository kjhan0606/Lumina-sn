#!/bin/bash
#SBATCH --job-name=paper_ddc15_v3asEfloor_skipZ_2002bo
...
# #292 Per-Z SKIP-NLTE 3-cell ablation on 2002bo paper-DDC15 v3asEfloor base.
# Yardstick = observation-fit (NOT T_inner-vs-TARDIS).
# Baseline 157921 (SKIP_Z=14 only) per-line residual budget:
#   Mg II 9218 2.00× / [Ni II] 7378 1.85× / [Fe II] 7155 1.59× /
#   O I 7774 1.57× / NIR continuum +30-50%.
# Probe: turn OFF NLTE for one iron-peak species per cell to localize carrier.
#   cell 0: SKIP_Z=14,26  (Fe NLTE off)  -> targets [Fe II] 7155
#   cell 1: SKIP_Z=14,27  (Co NLTE off)
#   cell 2: SKIP_Z=14,28  (Ni NLTE off)  -> targets [Ni II] 7378
# Each cell residual map vs 157921 -> isolate which Z drives which residual.

SKIP_Z_LIST=("14,26" "14,27" "14,28")
SKIP_Z="${SKIP_Z_LIST[$SLURM_ARRAY_TASK_ID]}"
SKIP_Z_TAG="$(echo "$SKIP_Z" | tr ',' 'p')"
...
label="paperDDC15v3asEfloorSKIPZ${SKIP_Z_TAG}_2002bo_vi${vi_tag}_L${l_tag}_${RUN_TAG}"
...
echo "=== PAPER-DDC15-v3 SKIP_Z=$SKIP_Z vs SN 2002bo  VI=$VI_KMS L=$L_scale EPS_IR=$EPS_IR  RUN=$RUN_TAG [${label}] ==="
