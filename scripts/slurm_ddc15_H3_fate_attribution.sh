#!/bin/bash
#SBATCH --job-name=ddc15_H3_fate_attribution
...
# H3: per-(Z, ion, entry_band, exit_band) macro-atom fate attribution.
# Runs C2 base + LUMINA_MA_FATE_ZIHIST=1 to dump ma_fate_zihist.csv at
# the end of the final iteration.
#
#   array 0 — EPS_UV=0.0 (no bypass) — pure cascade attribution
#   array 1 — EPS_UV=0.9 (full H1 bypass) — what survives the gate
...
label=$(printf "ddc15H3_epsUV%s_fate" "$eps_uv")
work_root="$ROOT/logs/ddc15H3_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "=== H3 fate attribution [${label}]  EPS_UV=${eps_uv}  ZIHIST=1 ==="
...
grep -E 'Mean \|W error|Mean \|T_rad error|T_inner final|L_emitted|EPS-UV|MA-FATE-ZI' stdout.log | tail -10
ls -la ma_fate_zihist.csv 2>/dev/null || echo "  WARN: no zihist CSV"
echo "Done: $(date)"
