#!/bin/bash
#SBATCH --job-name=ddc15_J1_SiII_UVidown
...
# J1: Si II-targeted UV internal-down suppression at production fidelity.
# H3 attribution: Si II = 21% of UV→red cascade + 83% of optical→red self-coupling.
# After I1 (Ni II 68%) NULL, test if Si II branch responds differently.
# 800K × 12 iter, C2 base (DDC15 v_in=10400, X_Fe_outer=0.05).
# NOTE: LUMINA_NLTE_SKIP_Z=14 still set (Si non-NLTE); UV-IDOWN acts on macro-atom branching regardless.
#
...
LABELS=(SiII_f1p0_ctrl SiII_f0p7 SiII_f0p5 SiII_f0p3)
...
label="ddc15J1_${tag}"
work_root="$ROOT/logs/ddc15J1_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "=== J1 Si II UV-idown [${label}]  FACTOR=${factor}  Z=14(Si) ion=1(II)  THRESH=4000Å  N_PKT=${N_PKT}  N_ITER=${N_ITER} ==="
...
LUMINA_MACRO_UV_IDOWN_THRESH=4000 \
LUMINA_MACRO_UV_IDOWN_ZMASK=14 \
LUMINA_MACRO_UV_IDOWN_IONMASK=1 \
...
grep -E 'Mean \|W error|Mean \|T_rad error|T_inner final|L_emitted|MACRO-UV-IDOWN' stdout.log | tail -12
echo "Done: $(date)"
