#!/bin/bash
#SBATCH --job-name=ddc15_K1_NiII_Aul
...
# K1: Ni II A_ul direct atomic-rate scaling at production fidelity.
# After I1 (Ni II UV-IDOWN macro-branching) NULL and J1 (Si II UV-IDOWN) marginal,
# test cutting Ni II UV-line strength at the source (A_ul + f_lu + B_lu + B_ul scaled).
# Unlike UV-IDOWN this also reduces UV line opacity — fewer macro-atom UV events
# initiated, not just rerouted afterward.
# 800K × 12 iter, C2 base (DDC15 v_in=10400, X_Fe_outer=0.05). Binary: lumina_cuda_h100_aulscale.
#
#   array 0 — factor=1.0  control (no suppress)
#   array 1 — factor=0.7  mild
#   array 2 — factor=0.5  moderate
#   array 3 — factor=0.3  aggressive
...
LABELS=(NiII_f1p0_ctrl NiII_f0p7 NiII_f0p5 NiII_f0p3)
...
label="ddc15K1_${tag}"
work_root="$ROOT/logs/ddc15K1_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "=== K1 Ni II A_ul scale [${label}]  FACTOR=${factor}  Z=28(Ni) ion=1(II)  λ_max=4000Å  N_PKT=${N_PKT}  N_ITER=${N_ITER} ==="
