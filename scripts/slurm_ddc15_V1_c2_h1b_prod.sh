#!/bin/bash
#SBATCH --job-name=ddc15_V1_c2_h1b
...
# V1 (#230): C2_xFeO0.15 + H1b_eps0p10 production validation on aulscale8 binary.
#
# Both were 200K×8 cefix-binary screening champions (MC ~0.188).
# Re-run on aulscale8 (h3-era atomic data, same as S1/T1) at 800K×12 for
# apples-to-apples vs T1_eps0p7 (current production champion at MC=0.196).
#
#   array 0 — C2_xFeO0p15 : X_Fe_outer=0.15, no EPS_UV, no SCALE knobs
#   array 1 — H1b_eps0p10 : X_Fe_outer=0.05, EPS_UV=0.10 (all-λ), no SCALE knobs
...
LABELS=(C2_xFeO0p15 H1b_eps0p10)
XFE_OUTERS=(0.15 0.05)
EPS_UVS=(0.0 0.10)
EPS_RED_ONLYS=(0 0)
tag=${LABELS[$SLURM_ARRAY_TASK_ID]}
xfeo=${XFE_OUTERS[$SLURM_ARRAY_TASK_ID]}
eps_uv=${EPS_UVS[$SLURM_ARRAY_TASK_ID]}
eps_red=${EPS_RED_ONLYS[$SLURM_ARRAY_TASK_ID]}
...
label="ddc15V1_${tag}"
work_root="$ROOT/logs/ddc15V1_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "=== V1 [${label}]  X_Fe_outer=${xfeo} EPS_UV=${eps_uv} RED_ONLY=${eps_red}  N_PKT=${N_PKT}  N_ITER=${N_ITER} ==="
...
LUMINA_EPS_UV_RED_ONLY="$eps_red" \
