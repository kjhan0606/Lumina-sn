...
Searches a 5D physical parameter space (log_L, v_inner, log_rho_0, X_Si, X_Fe)
...
Uses coarse-to-fine strategy:
  Phase 1: 100 Latin Hypercube samples, 20K packets x 5 iters  (~25 min)
...
def latin_hypercube(n_samples, param_ranges, rng=None):
    """Generate Latin Hypercube samples in 5D parameter space.
    ...
    def _write_density_csv(self, dirpath, params):
        ...
        with open(dirpath / "density.csv", 'w') as f:
            ...
            for i in range(N_SHELLS):
                ...
                vo = v_min + (i + 1) * dv
                v_mid = (vi + vo) / 2.0
                rho = rho_0 * (v_mid / v_min) ** DENSITY_EXPONENT
                f.write(f"{i},{rho}\n")
    ...
    def _write_electron_densities_csv(self, dirpath, params):
        ...
        with open(dirpath / "electron_densities.csv", 'w') as f:
            ...
            for i in range(N_SHELLS):
                ...
                v_mid = (vi + vo) / 2.0
                rho_new = rho_0_new * (v_mid / v_min_new) ** DENSITY_EXPONENT
...
PARAM_NAMES = ['log_L', 'v_inner', 'log_rho_0', 'X_Si', 'X_Fe']
...
def plot_sensitivity(results, output="fit_sensitivity.png"):
    """5-panel scatter: RMS vs each parameter."""
...
def main():
    ...
    parser.add_argument('--n-samples', type=int, default=100,
                        help='Number of LHS samples for Phase 1 (default: 100)')
