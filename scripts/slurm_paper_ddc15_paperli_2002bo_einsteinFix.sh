#!/bin/bash
#SBATCH --job-name=paperli_2002bo_einsteinFix
...
# Paper comparison run on einsteinFix reference (2026-05-29).
#
# einsteinFix = v3_femerge_capraise_bbfix_metafix_asE base PLUS:
#   B1 : line_list globally sorted descending in nu (1024 ascending pairs -> 0)
#   B2 : B_ul/B_lu/f_lu re-derived from A_ul (Einstein detailed-balance
#         residual 3.79e-16; Fe II 89.6% pre-fix violation -> 0%; iron-peak
#         III B_ul drift up to 11.57x corrected)
#   #305-3 : line2macro_level_upper rebuilt with level-index (was transition-
#         index; 0.54% -> 100% mapped). Macro-atom now active across all lines.
#
# Binaries: lumina_cuda_paperli_{a100,h100}_einsteinFix — built 2026-05-29
# with Cat A source patches (locale-C, malloc tardis_W, fread checks,
# CUDA singular-info, RNG splitmix, NOT-DESCENDING FATAL, atomic mass
# table for v_th, NLTE O II slot-29 preservation).
#
# Method preserved from metafix baseline: LINE_SCATTER (paper-comp default),
# LUMINA_NLTE_SKIP_Z=14, no LUMINA_SELF_CONSISTENT_TE.
...
N_PKT=${N_PKT:-200000}
N_ITER=${N_ITER:-6}
# CMF formal solver (paper-method line transfer): set TRANSPORT=cmf to also emit
# lumina_spectrum_cmf.csv. Default sobolev → unchanged behavior.
TRANSPORT=${TRANSPORT:-sobolev}   # sobolev | cmf
CMF_NZ=${CMF_NZ:-2000}
CMF_NIMPACT=${CMF_NIMPACT:-50}
CMF_VTURB_KMS=${CMF_VTURB_KMS:-50}   # CMFGEN-convention default; v-gradient dominates
RUN_TAG=paperli_einsteinFix_${LINE_INT}
...
if echo "$GPU_NAME" | grep -qi H100; then
    BIN="$ROOT/lumina_cuda_paperli_h100_einsteinFix"
elif echo "$GPU_NAME" | grep -qi H200; then
    BIN="$ROOT/lumina_cuda_paperli_h100_einsteinFix"
else
    BIN="$ROOT/lumina_cuda_paperli_a100_einsteinFix"
fi

SRC_REF="$ROOT/data/tardis_reference_v3_femerge_capraise_bbfix_metafix_asE_einsteinFix"
L_scale=$L_SCALE_ARG
...
echo "=== Paper comparison (einsteinFix ref)  LINE_INTERACTION=$LINE_INT  VI=$VI_KMS L=$L_scale  N_PKT=$N_PKT N_ITER=$N_ITER  TRANSPORT=$TRANSPORT ==="
...
env LUMINA_BF_OPACITY=1 \
    ...
    LUMINA_TRANSPORT=$TRANSPORT \
    LUMINA_CMF_NZ=$CMF_NZ \
    LUMINA_CMF_NIMPACT=$CMF_NIMPACT \
    LUMINA_CMF_VTURB_KMS=$CMF_VTURB_KMS \
