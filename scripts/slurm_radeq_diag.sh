...
# Converged O-NLTE-fix production run for paper comparison.
# Same einsteinFix ref + gate (INV_CEIL) + scatter config as gate-on baseline
# 160465, but with the oxygen overlap-pair fix (pair_shares_slot) compiled into
# the paperli_einsteinFix binaries. Verified on smoke 160756 (O I/II/III=1.0).
# Converged (N_ITER=15) so the O fix's spectrum impact is measured at
# paper quality, directly A/B-able against 160465.

VI_KMS=9019
L_SCALE_ARG=1.0
N_PKT=${N_PKT:-200000}
N_ITER=${N_ITER:-4}
RUN_TAG=${RUN_TAG:-radeq_ab}
LINE_INT=${LINE_INT:-scatter}
LEVEL_DUMP=${LEVEL_DUMP:-0}
...
if echo "$GPU_NAME" | grep -qi H100; then
    BIN="$ROOT/lumina_cuda_radeq_h100"
elif echo "$GPU_NAME" | grep -qi H200; then
    BIN="$ROOT/lumina_cuda_radeq_h100"
elif echo "$GPU_NAME" | grep -qi A40; then
    BIN="$ROOT/lumina_cuda_radeq_a40"
else
    BIN="$ROOT/lumina_cuda_radeq_a100"
fi
...
X_SC_FLOOR = float(os.environ.get("X_SC_FLOOR", "1e-5"))
...
def ddc15_profile_v3(v):
    ...
    if v < V_CORE_TOP:
        ...
        X_O  = 1e-6; X_C  = 1e-6
        X_Ti = 1e-4; X_V = 5e-5; X_Sc = X_SC_FLOOR
    elif v < V_NI_TOP:
        X_init_Ni  = 0.625
        X_stableFe = 0.014
        ...
        X_Ti = 1e-4 * (1 - t); X_V = 5e-5 * (1 - t); X_Sc = X_SC_FLOOR
    else:
        X_init_Ni  = 0.0
        X_stableFe = 1e-3
...
# Optional: seed a converged plasma state (W,T_rad) + T_inner for frozen-plasma tests.
if [ -n "$SEED_PLASMA" ] && [ -f "$SEED_PLASMA" ]; then
    cp "$SEED_PLASMA" "$REF_DIR/plasma_state.csv"
    echo "  [SEED] plasma_state <- $SEED_PLASMA"
fi
if [ -n "$SEED_TINNER" ]; then
    python3 -c "import json; c=json.load(open('$REF_DIR/config.json')); c['T_inner_K']=float($SEED_TINNER); json.dump(c,open('$REF_DIR/config.json','w'),indent=2)"
    echo "  [SEED] T_inner_K <- $SEED_TINNER"
fi

echo "=== O-NLTE-fix converged prod  VI=$VI_KMS L=$L_scale  N_PKT=$N_PKT N_ITER=$N_ITER ==="
...
env LUMINA_BF_OPACITY=1 \
    ...
    LUMINA_NLTE_START_ITER=${START_ITER:-2} \
    LUMINA_NLTE_FLOOR_REG=1 \
    LUMINA_NLTE_INV_CEIL=${INV_CEIL:-1e4} \
    LUMINA_LINE_INTERACTION=$LINE_INT \
    LUMINA_NLTE_LEVEL_DUMP=$LEVEL_DUMP \
    LUMINA_SUPER_LEVELS=${SUPER_LEVELS:-0} \
    LUMINA_W_CAP=${W_CAP:-0} \
    LUMINA_J_NU_UV_CAP=${UV_CAP:-0} \
    LUMINA_J_NU_UV_CAP_LAMBDA_MIN=${UV_CAP_LMIN:-1200} \
    LUMINA_J_NU_UV_CAP_LAMBDA_MAX=${UV_CAP_LMAX:-3500} \
    LUMINA_J_NU_UV_W_CAP=${UV_CAP_W:-1.0} \
    LUMINA_TAU_BY_ION=1 \
    LUMINA_TAU_BY_ION_SHELL=${TAU_SHELL:-0} \
    LUMINA_TAU_BY_ION_LAM_LO=${TAU_LAM_LO:-1700} \
    LUMINA_TAU_BY_ION_LAM_HI=${TAU_LAM_HI:-10000} \
    LUMINA_RADEQ_TE=${RADEQ_TE:-1} LUMINA_RADEQ_DIAG=1 \
    LUMINA_RADEQ_DAMP=${RADEQ_DAMP:-0.5} \
