...
# #219e2 OptionC: structural bug-fix via Mihalas-Lucy two-row conservation.
#
# OptA/OptA' (157278/157279) confirmed SKIP_Z bypass breaks plasma convergence:
# the 9 V3.1 SCALE knobs were tuned to compensate for the iron-peak III NLTE bug;
# removing it via SKIP_Z → uncompensated bb opacity → T_inner +9..+12% runaway,
# W err 947%/1450%, Snifs rms_bn 0.135 → 0.373.
#
# OptionC: keep the NLTE solver active but enable LUMINA_NLTE_ION_LOCK=1, which
# places TWO conservation rows (per-ion totals) in the rate matrix instead of one
# combined all-1's row at N-1. This structurally constrains the upper-ion excited
# block → bug fixed at the source, without bypassing tau.
#
# Trade-off: ION_LOCK also freezes plasma transport-only (cuda.cu:2594-2604) at
# iter >= LOCK_START_ITER. To preserve V3.1 plasma convergence as the baseline,
# we set LOCK_START_ITER=11 — iters 2..10 run normal V3.1 NLTE (with the bug),
# iter 11 NLTE solve uses bug-fix rate-matrix structure, iter 12 transport
# packets through the bug-fixed opacity. Spectrum at iter 12 reflects the delta.
#
# Comparison: 157277 baseline (V3.1, SKIP_Z=14 only).
...
label="nlte3fix_optC_${tag}"
work_root="$ROOT/logs/nlte3fix_optC_${SLURM_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "Time: $(date)"
echo "** ION_LOCK=1 + LOCK_START_ITER=11 — V3.1 plasma converges iters 2..10, bug-fix two-row conservation at iter 11, transport iter 12 **"
...
LUMINA_NLTE_LOCK_START_ITER=11 \
...
print(f"  [OptC] Snifs [3300,8000]   rms_bn = {r:.4f}   chi_bn = {chi:.3f}")
    ...
    if gH > 0:
        for lo, hi, lbl in [(3000., 8000., "[3000,8000]"), (1700., 3000., "[1700,3000]")]:
            r, chi = rms_bn_nw(mlam, mflu, hl, hf, gH,
                               ...
                               wl_lo=lo, wl_hi=hi)
            print(f"  [OptC] HST   {lbl}  rms_bn = {r:.4f}   chi_bn = {chi:.3f}")
PYEOF
