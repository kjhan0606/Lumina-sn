#!/bin/bash
#SBATCH --job-name=superlev_identity_smoke
...
# self-pin T_inner, N_ITER=15) EXCEPT LUMINA_LINE_INTERACTION=scatter instead
...
N_PKT=100000
N_ITER=4
RUN_TAG=superlev_identity_smoke
...
if echo "$GPU_NAME" | grep -qi H100; then
    BIN="$ROOT/lumina_cuda_h100"
elif echo "$GPU_NAME" | grep -qi H200; then
    BIN="$ROOT/lumina_cuda_h100"
elif echo "$GPU_NAME" | grep -qi A40; then
    BIN="$ROOT/lumina_cuda_a40"
else
    BIN="$ROOT/lumina_cuda_a100"
fi
