#!/bin/bash
#SBATCH --job-name=nlte3fix_femerge
...
# #219f surgical FEMERGE: V3 base + CMFGEN iron-peak III bb atomic data.
#
# #268 closed: SKIP_Z (OptA/OptA') and ION_LOCK (OptC/OptD) probes confirmed
# V3.1 SCALE stack absorbs the iron-peak III NLTE rate-matrix bug downstream;
# no single env knob can lift the spectrum on the V3.1 atomic data.
#
# This probe: replace V3 iron-peak III (Ti/Cr/Mn/Fe/Co/Ni III) bb data with
# CMFGEN line lists + truncated level tables. CMFGEN's lower level caps
# (600-1500) avoid the bb-isolated top-level singularity (#219e). Rest of
# the periodic table keeps V3 data so V3.1 SCALE stack tuning still applies.
#
# Ref dir: data/tardis_reference_v3_femerge (built by scripts/merge_iron_peak_III_bb.py
# + scripts/finalize_cmfgen_ref_npy.py).
#
# Comparison: 157277 baseline (V3.1, SKIP_Z=14 only).
# Decision tree:
#   ΔSnifs < -0.010 → real signal, push 4-epoch validation
#   |ΔSnifs| ≈ 0   → bug-fix exists but spectrum-equivalent; iron-peak III SCALE re-tune next
#   HST UV +0.05+  → partial V4 over-blanketing; revert to fewer ions (just bug carriers)
...
label="nlte3fix_femerge_${tag}"
work_root="$ROOT/logs/nlte3fix_femerge_${SLURM_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "Time: $(date)"
echo "** FEMERGE — V3 base with CMFGEN iron-peak III bb (Ti/Cr/Mn/Fe/Co/Ni III). Sc III, V III stay V3. No ION_LOCK, no SKIP_Z (except 14 for Si II). V3.1 SCALE stack unchanged. **"
...
print(f"  [FEMERGE] Snifs [3300,8000]   rms_bn = {r:.4f}   chi_bn = {chi:.3f}")
    ...
    if gH > 0:
        for lo, hi, lbl in [(3000., 8000., "[3000,8000]"), (1700., 3000., "[1700,3000]")]:
            r, chi = rms_bn_nw(mlam, mflu, hl, hf, gH,
                               ...
                               wl_lo=lo, wl_hi=hi)
            print(f"  [FEMERGE] HST   {lbl}  rms_bn = {r:.4f}   chi_bn = {chi:.3f}")
PYEOF
