#!/bin/bash
#SBATCH --job-name=ddc15_G1_xFeInner
...
# G1: Inner X_Fe sweep on C2 champion (v_in=10400, X_Fe_outer=0.05).
# Tests whether inner Fe (v<11000, fixed at 0.15 in DDC15-C2 profile) is the
# UV-trap → red-cascade source.  F1 closed "NIR opacity deficit", D1 closed
# "thermal continuum", C2 closed "outer Fe". This closes/opens the inner-Fe
# axis with the SAME diagnostic pattern (4-cell sweep, fixed-outer reference).
#   array 0 — X_Fe_inner=0.15 (C2 baseline reproducibility)
#   array 1 — X_Fe_inner=0.05 (1/3)
#   array 2 — X_Fe_inner=0.02 (1/7.5)
#   array 3 — X_Fe_inner=0.00 (full zero-out diagnostic)
# X_Cr=0.05·X_Fe, X_Mn=0.03·X_Fe follow inner Fe automatically.
...
X_FE_INNERS=(0.15 0.05 0.02 0.00)
xfei=${X_FE_INNERS[$SLURM_ARRAY_TASK_ID]}
xfeo=0.05    # C2 champion outer cap
vi=10400
ro=0.00
de=9.0
label=$(printf "ddc15G1_xFeI%s" "$xfei")

work_root="$ROOT/logs/ddc15G1_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
X_FE_INNER, X_FE_OUTER = $xfei, $xfeo
...
def ddc15_profile(v, x_fe_inner, x_fe_outer):
    if   v < 9000:           X_Si = 0.05
    ...
    # Inner Fe plateau (v<11000): now SWEPT (was fixed=0.15 in C2)
    # Outer shoulder (v>=11000): exp decay from C2 champion x_fe_outer=0.05
    if   v < 11000:          X_Fe = x_fe_inner
...
for s in range(n_shells):
    prof = ddc15_profile(v_mid_new[s], X_FE_INNER, X_FE_OUTER)
...
print(f"  built: v_in={vi_kms} X_Fe_inner={X_FE_INNER} X_Fe_outer={X_FE_OUTER}")
print(f"         rho0={rho0:.3e}  T_in_init={T_inner:.0f}")
print(f"  sh   v_mid    X_Fe    X_Cr    X_Mn    X_O")
for s in [0, 2, 4, 6, 10, 15, 20, 25, 29]:
    print(f"  {s:>2}  {v_mid_new[s]:>6.0f}  {X[Z_LIST.index(26),s]:.2e}  "
          f"{X[Z_LIST.index(24),s]:.2e}  {X[Z_LIST.index(25),s]:.2e}  "
          f"{X[Z_LIST.index(8),s]:.2e}")
PYEOF

echo "=== G1 inner X_Fe sweep [${label}]  inner=${xfei}  outer=${xfeo}  v_in=${vi}  N_PKT=${N_PKT} ==="
