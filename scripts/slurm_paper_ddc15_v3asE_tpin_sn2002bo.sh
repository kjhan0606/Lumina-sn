#!/bin/bash
#SBATCH --job-name=paper_ddc15_v3asEtpin_2002bo
...
if echo "$GPU_NAME" | grep -qi H100; then
    BIN="$ROOT/lumina_cuda_oTri_h100_tpin"
elif echo "$GPU_NAME" | grep -qi H200; then
    BIN="$ROOT/lumina_cuda_oTri_h100_tpin"
else
    BIN="$ROOT/lumina_cuda_oTri_a100_tpin"
fi
...
label="paperDDC15v3asE_2002bo_vi${vi_tag}_L${l_tag}_${RUN_TAG}"
