#!/bin/bash
#SBATCH --job-name=nlte_rate_dump
...
# Investigation (A): dump R_bf, R_rec, n_star_ratio per (Z, ion_pair, shell)
# to identify why NLTE over-ionizes Si II by 10⁹-10¹³×.
...
cell_dir="$ROOT/logs/nlte_rate_dump_${SLURM_JOB_ID}"
mkdir -p "$cell_dir"
cd "$cell_dir"

echo "=== NLTE rate-balance instrumentation run ==="
...
rc=$?
echo "  exit=$rc"
ls -la nlte_rate_balance.csv lumina_spectrum_formal.csv nlte_levels_iter*.csv 2>&1 | tail -10
echo "Done:   $(date)"
