#!/bin/bash
#SBATCH --job-name=ddc15_P1_FeCo_push
...
# P1: Push Fe II and Co II A_ul factors around current champion
# (N1 + Co II f=0.5 = K+L+M(0.5)+Co(0.5), drop Cr II).
# O1 finding: Co II f=0.5 = champion 0.2310 RMS_bn@F20k vs HST.
# Drop Cr II (closed, −0.006 noise) and Co+Cr antagonized.
# Question: does Fe II push (0.5→0.3→0.2) help further, or saturate
# (M1 sweep alone showed f=0.5 optimum, but Co II baked may shift).
# Co II push (0.5→0.3) tests Co f-sensitivity.
# Binary: lumina_cuda_h100_aulscale5 (4 of 5 suffixes used).
#
#   array 0 — N1+Co champion replicate (Fe 0.5, Co 0.5)
#   array 1 — Fe push: Fe 0.3, Co 0.5
#   array 2 — Fe push: Fe 0.2, Co 0.5
#   array 3 — Co push: Fe 0.5, Co 0.3
...
LABELS=(champ_repl Fe0p3 Fe0p2 Co0p3)
...
label="ddc15P1_${tag}"
work_root="$ROOT/logs/ddc15P1_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "=== P1 [${label}]  Ni(0.3) + Si(0.05) + Fe(${FE_FAC}) + Co(${CO_FAC})  λ<4000Å  N_PKT=${N_PKT}  N_ITER=${N_ITER} ==="
