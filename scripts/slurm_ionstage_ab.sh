...
# Task #29 (Probe-B fix) A/B: feed NLTE-solved ion stage back into bulk opacity.
# Identical config to job 161922 (DDC15-stratified abundances, radeq-OFF, scatter,
# superlev_ionfix_ddc15strat ref). The ONLY difference between arms is
# LUMINA_NLTE_OPACITY_IONSTAGE: ARM=A (off, reproduces 161922 = keeper baseline)
# vs ARM=B (on, NLTE II/III split drives non-tracked iron-peak line opacity).
# Submit BOTH arms with the SAME fresh binary so the comparison is clean.
#   sbatch --export=ALL,ARM=A scripts/slurm_ionstage_ab.sh
#   sbatch --export=ALL,ARM=B scripts/slurm_ionstage_ab.sh

ARM=${ARM:-B}                 # A=baseline(off)  B=treatment(on)
VI_KMS=9019
L_SCALE_ARG=1.0
N_PKT=200000
N_ITER=10
RUN_TAG="ionstage_${ARM}"
...
# Fresh binaries built from the task-#29 source (write-back helper compiled in).
if echo "$GPU_NAME" | grep -qiE "H100|H200"; then
    BIN="$ROOT/lumina_cuda_ionstage_h100"
else
    BIN="$ROOT/lumina_cuda_ionstage_a40"   # sm_86: a40 / a100(sm80 ok via PTX) / a10
fi
...
echo "=== Probe-B ion-stage A/B  ARM=$ARM  VI=$VI_KMS L=$L_scale  N_PKT=$N_PKT N_ITER=$N_ITER ==="
echo "Host: $(hostname)  GPU: $GPU_NAME"
echo "Binary: $BIN  Time: $(date)"
echo "LUMINA_NLTE_OPACITY_IONSTAGE = $([ "$ARM" = B ] && echo 1 || echo '(off)')"
...
env LUMINA_BF_OPACITY=1 \
    ...
    $([ "$ARM" = B ] && echo "LUMINA_NLTE_OPACITY_IONSTAGE=1") \
...
echo "  exit=$rc"
echo ""
echo "--- ion-stage write-back trace ---"
grep -E "OPACITY_IONSTAGE" stdout.log | head
echo ""
