#!/bin/bash
#SBATCH --job-name=ddc15_U1_feII_opt
...
# U1 (Task #224 Tier-2 push): Fe II optical-band A_ul damp stacked on S1_f01.
#
# S1_f01 (job 156411 SCALE6 Si II opt f=0.1) landed RMS_bn=0.2077 → +0.008 to T1 target.
# #220 zoom-in shows next-largest residuals (after Si II) on Fe II m42/5018:
#   Fe II m42 (4924): LUMINA depth 0.65 vs HST 0.39 (Δd=+0.26)
#   Fe II 5018:       LUMINA depth 0.47 vs HST 0.20 (Δd=+0.27)
# Q1b r03 already damps Fe II at λ<4000Å (SCALE3 f=0.3); extending to optical
# [4000,5500]Å is orthogonal. Stack on S1_f01 to push toward Tier 2 ≤0.18.
#
# Binary: lumina_cuda_a100_aulscale8 (sm_80 build, A100).
# Submitted to a100/a100_pcie/a40 partitions (h100/h200 busy with S2 156421).
#
#   array 0 — U1_ref:   SCALE7 f=1.0 → replicate S1_f01 0.2077
#   array 1 — U1_f05:   SCALE7 f=0.5
#   array 2 — U1_f03:   SCALE7 f=0.3
#   array 3 — U1_f01:   SCALE7 f=0.1
#   array 4 — U1_f005:  SCALE7 f=0.05
...
LABELS=(U1_ref U1_f05 U1_f03 U1_f01 U1_f005)
tag=${LABELS[$SLURM_ARRAY_TASK_ID]}

FE_FAC=0.3
CO_FAC=0.3
RED_FAC=0.3
SI_OPT_FAC=0.10        # S1_f01 winner
FE_OPT_FAC=1.0
...
label="ddc15U1_${tag}"
work_root="$ROOT/logs/ddc15U1_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "=== U1 [${label}]  S1_f01 base + SCALE7 Fe II opt λ∈[4000,5500] f=${FE_OPT_FAC}  N_PKT=${N_PKT}  N_ITER=${N_ITER} ==="
...
LUMINA_AUL_SCALE7_FACTOR="$FE_OPT_FAC" \
LUMINA_AUL_SCALE7_LAMBDA_MIN=4000 \
LUMINA_AUL_SCALE7_LAMBDA_MAX=5500 \
LUMINA_AUL_SCALE7_ZMASK=26 \
...
grep -E 'Mean \|W error|Mean \|T_rad error|T_inner final|L_emitted|AUL_SCALE' stdout.log | tail -16
echo "Done: $(date)"
