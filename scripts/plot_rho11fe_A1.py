#!/usr/bin/env python3
"""A1: ρ-11fe + v_in sweep {9500,9800,10100,10400} — Si II 6355 trough scan vs HST."""
...
JOB  = sys.argv[1] if len(sys.argv) > 1 else "155704"

VINS = [9500, 9800, 10100, 10400]
CELLS = [(vi, ROOT/f"logs/rho11feA1_{JOB}_rho11fe_v{vi:05d}/lumina_spectrum_formal.csv")
         for vi in VINS]
PATH_A_REF = ROOT/"logs/rho11feA_155702_rho11fe_skipSi/lumina_spectrum_formal.csv"  # baseline
OUT = ROOT/f"figures/rho11fe_A1_{JOB}_vs_hst.png"
C_KMS = 299792.458
...
for (vi, (lam, flu)), c in zip(data.items(), cmap):
    ax.plot(lam, flu*1e14, color=c, lw=0.9, alpha=0.85, label=f'ρ-11fe +SkipSi v_in={vi}')
ax.set_xlim(1700, 9000)
ax.set_ylim(0, max(hflu[(hlam>2900)&(hlam<9000)])*1e14*1.15)
ax.set_xlabel('λ (Å)'); ax.set_ylabel('F_λ × 10¹⁴')
ax.set_title(f'A1: ρ-11fe + v_in sweep (job {JOB}) — full spectrum')
...
for (vi, (lam, flu)), c in zip(data.items(), cmap):
    ...
    if tm:
        ...
        ax.text(tm, ymax*0.45 + (vi-9500)/3000*ymax*0.1, f'v_in={vi}\nλ={tm:.0f}\nv={v_obs:.0f}',
...
if tm:
    ...
    ax.text(tm, ymax*0.3, f'HST {tm:.0f}\nv={v_obs:.0f}', color='k', fontsize=9,
            ha='center', va='bottom', fontweight='bold')
ax.set_xlabel('λ (Å)'); ax.set_ylabel('F_λ × 10¹⁴')
ax.set_title('Si II 6355 zoom — A1 v_in sweep vs HST 9934 km/s')
ax.legend(loc='upper right', fontsize=9); ax.grid(alpha=0.2)
...
ax.set_xlabel('λ (Å)'); ax.set_ylabel('F_λ × 10¹⁴')
ax.set_title('UV blanketing 2300-4000Å (composition+structure sensitive)')
ax.legend(loc='upper right', fontsize=9); ax.grid(alpha=0.2)
...
# Trough table
print(f"\n=== Si II 6355 trough position (A1 ρ-11fe + v_in sweep) ===")
tm = trough_min(hlam, hflu)
print(f"{'HST 2011fe B-max':35s}  λ={tm:.1f}  v={(1-tm/6355)*C_KMS:.0f} km/s")
for vi, (lam, flu) in data.items():
    tm = trough_min(lam, flu)
    if tm:
        v = (1-tm/6355)*C_KMS
        print(f"{'ρ-11fe v_in='+str(vi):35s}  λ={tm:.1f}  v={v:.0f} km/s  (HST diff {v-9934:+.0f})")
...
for vi, (lam, flu) in data.items():
    ...
    print(f"ρ-11fe +SkipSi v_in={vi}                  RMS_bn = {rms:.4f}")
