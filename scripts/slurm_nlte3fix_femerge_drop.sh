#!/bin/bash
#SBATCH --job-name=nlte3fix_fdrop
...
# #219f Phase 3: light SCALE re-search on FEMERGE base.
#
# Drop-group ablation. $1 = "eps" | "uv" | "uve" | "none"
#   eps  → drop EPS_UV (keep all 8 SCALEs)
#   uv   → drop SCALE1+SCALE3+SCALE4 (UV-band damps on Ni/Fe/Co II), keep EPS_UV
#   uve  → drop both (UV-band SCALEs AND EPS_UV)
#   none → V3.1-style stack (replica of 157291 sanity)
#
# Anchors:
#   157285 V3.1+V3:        Snifs 0.138 / HST opt 0.168 / HST UV 0.365 (bug compensated)
#   157291 V3.1+FEMERGE:   Snifs 0.271 / HST opt 0.298 / HST UV 0.297 (bug fixed, over-corrected)
#   157294 bare+FEMERGE:   Snifs 0.291 / HST opt 0.321 / HST UV 0.297 (no SCALE, no EPS)
#
# Hypothesis: UV-band damps + EPS_UV were over-correcting because V3.1 bug
# created excess UV blanketing. On FEMERGE, UV is already matched (0.297) without
# them. Dropping UV-knobs should improve Snifs without hurting UV.
...
SRC_REF="$ROOT/data/tardis_reference_v3_femerge"
L_scale=1.55
suffix="nlte3fix_femerge_drop${DROP_SET}"
...
label="nlte3fix_femerge_drop${DROP_SET}_${tag}"
...
echo "=== NLTE3FIX-DROP[$DROP_SET] [${label}] L=${L_inner} N_PKT=${N_PKT} ==="
echo "Host: $(hostname)  GPU: $(nvidia-smi --query-gpu=name --format=csv,noheader|head -1)"
echo "Binary: $BIN  Snifs: $snifs  HST tag: $hst_tag"
echo "Time: $(date)"
case "$DROP_SET" in
    eps)  echo "** DROP[eps] — V3.1 SCALE stack MINUS EPS_UV (red-only cascade bypass). **" ;;
    uv)   echo "** DROP[uv] — V3.1 stack MINUS SCALE1/3/4 (UV-band damps on Ni/Fe/Co II λ<4000). **" ;;
    uve)  echo "** DROP[uve] — V3.1 stack MINUS EPS_UV AND SCALE1/3/4. **" ;;
    none) echo "** DROP[none] — V3.1 SCALE stack replica (157291 sanity). **" ;;
    *)    echo "** UNKNOWN DROP_SET=$DROP_SET — falling back to none **"; DROP_SET=none ;;
esac

# Build env-var prefix list dynamically
env_args=()
env_args+=("LUMINA_BF_OPACITY=1")
env_args+=("LUMINA_CMFGEN_SIGMA_BF=$ROOT/data/atomic/cmfgen_sigma_bf.bin")
env_args+=("LUMINA_DYNAMIC_TRANSPROB=1")
env_args+=("LUMINA_NLTE_SKIP_Z=14")
env_args+=("LUMINA_NLTE_START_ITER=2")

# EPS_UV block (drop on eps/uve)
if [[ "$DROP_SET" != "eps" && "$DROP_SET" != "uve" ]]; then
    env_args+=("LUMINA_EPS_UV=$EPS_UV")
    env_args+=("LUMINA_EPS_UV_RED_ONLY=1")
fi

# UV-band SCALE1/3/4 block (drop on uv/uve)
if [[ "$DROP_SET" != "uv" && "$DROP_SET" != "uve" ]]; then
    env_args+=("LUMINA_AUL_SCALE_FACTOR=0.3" "LUMINA_AUL_SCALE_LAMBDA_MAX=4000")
    env_args+=("LUMINA_AUL_SCALE_ZMASK=28"  "LUMINA_AUL_SCALE_IONMASK=1")
    env_args+=("LUMINA_AUL_SCALE3_FACTOR=$FE_FAC" "LUMINA_AUL_SCALE3_LAMBDA_MAX=4000")
    env_args+=("LUMINA_AUL_SCALE3_ZMASK=26"      "LUMINA_AUL_SCALE3_IONMASK=1")
    env_args+=("LUMINA_AUL_SCALE4_FACTOR=$CO_FAC" "LUMINA_AUL_SCALE4_LAMBDA_MAX=4000")
    env_args+=("LUMINA_AUL_SCALE4_ZMASK=27"      "LUMINA_AUL_SCALE4_IONMASK=1")
fi

# Always-on: SCALE2 (Si II UV — irrelevant since Si is SKIP_Z'd, kept for consistency)
env_args+=("LUMINA_AUL_SCALE2_FACTOR=0.05" "LUMINA_AUL_SCALE2_LAMBDA_MAX=4000")
env_args+=("LUMINA_AUL_SCALE2_ZMASK=14"   "LUMINA_AUL_SCALE2_IONMASK=1")
# Always-on: optical/NIR SCALE6/7/8/9
env_args+=("LUMINA_AUL_SCALE6_FACTOR=$SI_OPT_FAC" "LUMINA_AUL_SCALE6_LAMBDA_MIN=4000")
env_args+=("LUMINA_AUL_SCALE6_LAMBDA_MAX=7000"    "LUMINA_AUL_SCALE6_ZMASK=14"  "LUMINA_AUL_SCALE6_IONMASK=1")
env_args+=("LUMINA_AUL_SCALE7_FACTOR=$NI2_OPT_FAC" "LUMINA_AUL_SCALE7_LAMBDA_MIN=4000")
env_args+=("LUMINA_AUL_SCALE7_LAMBDA_MAX=7500"     "LUMINA_AUL_SCALE7_ZMASK=28"  "LUMINA_AUL_SCALE7_IONMASK=1")
env_args+=("LUMINA_AUL_SCALE8_FACTOR=$CA2_FAC" "LUMINA_AUL_SCALE8_LAMBDA_MIN=3700")
env_args+=("LUMINA_AUL_SCALE8_LAMBDA_MAX=9000" "LUMINA_AUL_SCALE8_ZMASK=20"  "LUMINA_AUL_SCALE8_IONMASK=1")
env_args+=("LUMINA_AUL_SCALE9_FACTOR=$S9_FAC" "LUMINA_AUL_SCALE9_LAMBDA_MIN=7500")
env_args+=("LUMINA_AUL_SCALE9_LAMBDA_MAX=10000" "LUMINA_AUL_SCALE9_ZMASK=28"  "LUMINA_AUL_SCALE9_IONMASK=1")

echo "  active env count: ${#env_args[@]}"
env "${env_args[@]}" "$BIN" "$REF_DIR" "$N_PKT" "$N_ITER" spectrum nlte \
...
echo ""
echo "--- SKIP_Z confirmation ---"
grep -E "LUMINA_NLTE_SKIP_Z|NLTE.*active|ion-lock|plasma frozen" stdout.log | head -10
...
print(f"  [DROP-$DROP_SET] Snifs [3300,8000]   rms_bn = {r:.4f}   chi_bn = {chi:.3f}")
    ...
    if gH > 0:
        for lo, hi, lbl in [(3000., 8000., "[3000,8000]"), (1700., 3000., "[1700,3000]")]:
            r, chi = rms_bn_nw(mlam, mflu, hl, hf, gH,
                               ...
                               wl_lo=lo, wl_hi=hi)
            print(f"  [DROP-$DROP_SET] HST   {lbl}  rms_bn = {r:.4f}   chi_bn = {chi:.3f}")
PYEOF
