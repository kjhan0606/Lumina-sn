...
# OPTION 2+3 VALIDATION — static A_ul-weighted branching table + env-bug fix.
# Fixes both compounding defects from the 6-agent verdict
# (project_macroatom_transprob_J_pump_root.md):
#   (3) cuda.cu:2513 now parses the VALUE, so DYNAMIC_TRANSPROB=0 TRULY disables
#       the uncapped B_lu*J up-pump (the trpbfix binary).
#   (2) the static fallback transition_probabilities.npy is now A_ul-weighted
#       radiative rates (emit/idn ∝ A_ul, iup ∝ B_lu*W*B_nu(T_rad_ref)) instead
#       of the flat 1/N placeholder (ref ..._aulbranch).
#
# THIS RUN: macroatom + frozen 162212 thermal structure, DYNAMIC_TRANSPROB=0
# (now genuinely OFF) so the spectrum is produced PURELY by the new static
# A_ul-weighted branching. Tests whether a physical static table alone restores
# blue / bounds NIR (vs 162323's blue 0.062 / NIR 2.34 from the 1/N + J-pump).
#
#   sbatch scripts/slurm_aulbranch_validate.sh
...
if echo "$GPU_NAME" | grep -qiE "H100|H200"; then
    BIN="$ROOT/lumina_cuda_binnedj_h100_trpbfix"
else
    BIN="$ROOT/lumina_cuda_binnedj_a40_trpbfix"
fi

SRC_REF="$ROOT/data/${REF_NAME:-tardis_reference_cmfgen_superlev_ionfix_ddc15strat_aulbranch}"
L_scale=$L_SCALE_ARG
...
echo "=== AUL-branch validation  REF=$SRC_REF  LINE_INT=$LINE_INT  FIXED_PROFILE=$FIXED_PROFILE  DIFFUSE_BC=$DIFFUSE_BC  VI=$VI_KMS L=$L_SCALE_ARG  N_PKT=$N_PKT N_ITER=$N_ITER ==="
echo "Host: $(hostname)  GPU: $GPU_NAME"
echo "Binary: $BIN  Time: $(date)"
echo "Option 2+3: A_ul-weighted static branching (ref _aulbranch) + env-bug fix (trpbfix binary). DYNAMIC_TRANSPROB=0 now TRULY disables the B_lu*J up-pump; spectrum produced purely by the new static A_ul table. Tests vs 162323 (1/N + J-pump: blue 0.062/NIR 2.34)."
...
echo "  exit=$rc"
echo ""
echo "--- transprob mode (MUST say FROZEN — static A_ul table, no dynamic recompute) ---"
grep -E "Transition probs:|\[TransProb\]" stdout.log | head -20
echo ""
