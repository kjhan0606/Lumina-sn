#!/usr/bin/env python3
"""macroatom (paper-method) A/B vs scatter baseline, overlaid on SN 2002bo B-max.
160767 = scatter self-pin baseline; 160823 = LUMINA_LINE_INTERACTION=macroatom (paper fluorescence).
Both Sobolev formal spectra, anchor-normalized on [4000,6000]A vs dereddened obs.
Outputs full overlay + 4-panel zoom matching the user's P2/P3/P4 band comments,
and prints per-band model/obs ratios so the prediction can be scored.
"""
...
RUNS = [("ofix_160767",      "scatter (baseline)",   "darkorange"),
        ("macroatom_160823", "macroatom (paper)",    "navy")]
...
print(f"{'band':24s}" + "".join(f"{l:>22s}" for _,l,_ in RUNS))
...
ax.set_title("macroatom (paper-method) vs scatter baseline — SN 2002bo B-max [4000,6000]A anchor")
...
plt.tight_layout()
out1=f"{ROOT}/figures/2026-05-31_macroatom_ab_sn2002bo_bmax.png"
plt.savefig(out1,dpi=130); print(f"saved: {out1}")

# ---- 4-panel zoom matching user's panels ----
...
fig.suptitle("macroatom vs scatter — user panels (P2 UV/blue, P3 Si6355, P4 CaNIR)",fontsize=13)
plt.tight_layout()
out2=f"{ROOT}/figures/2026-05-31_macroatom_ab_zoom_sn2002bo_bmax.png"
