#!/bin/bash
#SBATCH --job-name=plain_ddc15_2002bo
...
# #283 Plain DDC15 vs SN 2002bo at B-max (-0.2d from UVOIR max per Blondin+2013 Table 3).
# Target = reproduce Blondin Fig 6: F_scl ≈ 1.05, Q_uvoir ≈ 1.06.
# ALL 9 LUMINA_AUL_SCALE* knobs STRIPPED. NLTE-O (#281) topology retained as physics floor.
# Smoke: N_PKT=200000 N_ITER=3 (≈5 min). Production: 800000 / 12.
#
# Paper params (Blondin+2013 Table 2 at bmax):
#   t_rise = 17.76 d  →  t_exp = 1534464 s
#   L_bol  = 1.22e43 erg/s   (set L_inner=L_bol; emergent should match if model ok)
#   v_τ    = 9019 km/s       (photospheric v at τ_es=2/3)
#   T_τ    = 12630 K
#   X_Fe_τ = 0.0927, X_Si_τ = 0.214, X_Ca_τ = 0.0560 at photosphere
...
# Smoke vs production toggle: pass "smoke" as $1 for 200K x 3, else 800K x 12.
...
ro=0.00
de=$DE_ARG
xfeo=0.05                    # outer Fe scale (synthetic ddc15_profile knee)

vi_tag=$(echo "$vi" | tr '.' 'p')
l_tag=$(echo "$L_scale" | tr '.' 'p')
de_tag=$(echo "$de" | tr '.' 'p')
label="plainDDC15_2002bo_vi${vi_tag}_L${l_tag}_de${de_tag}_${RUN_TAG}"
...
# Same synthetic DDC15 profile as oTri_prod.
# TODO #283 follow-up: tune knee points to match Blondin Table 2 photospheric X at v_τ=9019:
#   X(Fe)=0.0927, X(Si)=0.214, X(Ca)=0.0560.
# Current ddc15_profile @ v=9019: X_Fe=0.15 (60% over paper), X_Si=0.40 (87% over paper).
...
echo "=== PLAIN-DDC15 vs SN 2002bo  VI=$VI_KMS L=$L_scale DE=$DE_ARG  RUN=$RUN_TAG [${label}] ==="
echo "Host: $(hostname)  GPU: $GPU_NAME"
echo "Binary: $BIN  Time: $(date)"
echo "NO LUMINA_AUL_SCALE* knobs (plain physics)."
