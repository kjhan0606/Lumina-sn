#!/usr/bin/env python3
"""Q1b: red-window iron-peak III A_ul damp via LAMBDA_MIN env knob."""
...
CELLS = [
    ("Q1b_ref",  f"logs/ddc15Q1b_{JOB}_ddc15Q1b_Q1b_ref/lumina_spectrum_formal.csv"),
    ("Q1b_r05",  f"logs/ddc15Q1b_{JOB}_ddc15Q1b_Q1b_r05/lumina_spectrum_formal.csv"),
    ("Q1b_r03",  f"logs/ddc15Q1b_{JOB}_ddc15Q1b_Q1b_r03/lumina_spectrum_formal.csv"),
    ("Q1b_r01",  f"logs/ddc15Q1b_{JOB}_ddc15Q1b_Q1b_r01/lumina_spectrum_formal.csv"),
]
...
print("="*88)
print(f"=== Q1b LAMBDA_MIN: iron-peak III A_ul damp in [4500,7500]Å, job {JOB} ===")
print("    Stack baked: Ni 0.3 + Si 0.05 + Fe 0.3 + Co 0.3 (λ<4000Å, ion=1)")
print("    SCALE5 added: Cr/Fe/Co III in [4500,7500]Å")
...
print()
print("="*88)
print("=== Δ RMS_bn vs Q1b_ref (intra-job anchor) @ FWHM=20k & 40k ===")
print(f"{'comparison':<28s}  {'Δ HST_20k':>11s} {'Δ HST_40k':>11s} {'Δ TAR_20k':>11s} {'Δ TAR_40k':>11s}")
print("-"*88)
ch = rms["Q1b_ref"]
for name in ["Q1b_r05", "Q1b_r03", "Q1b_r01"]:
    d = tuple(rms[name][i] - ch[i] for i in range(4))
    print(f"{name+' − Q1b_ref':<28s}  {d[0]:>+11.4f} {d[1]:>+11.4f} {d[2]:>+11.4f} {d[3]:>+11.4f}")

# Cross-job
print()
print("="*88)
print("=== Cross-job reproducibility (Q1b_ref vs prior stack baselines) ===")
print(f"    job 156183 P2 Fe0.3+Co0.3 (stack):   HST_20k=0.2384")
print(f"    job 156190 Q1_ref (stack):           HST_20k=0.2379")
print(f"    job 156281 Q1b_ref (new binary):     HST_20k={ch[0]:.4f}")
print(f"    σ ~ 0.02 envelope")
...
print()
deltas = {name: rms[name][0] - ch[0] for name in ["Q1b_r05", "Q1b_r03", "Q1b_r01"]}
best_q = min(deltas.items(), key=lambda x: x[1])
print("  Q1b LAMBDA_MIN verdict:")
if best_q[1] <= -0.005:
