#!/bin/bash
#SBATCH --job-name=ddc15_S1_siII_opt
...
# S1 (Task #221): Si II optical-band A_ul damp on Q1b r03 stack.
#
# #220 trough zoom-in identified Si II 3-line uniform over-absorption:
#   Si II 4130: LUMINA depth 0.64 vs HST 0.18 (Δd=+0.46)
#   Si II 5972: LUMINA depth 0.59 vs HST 0.10 (Δd=+0.50)
#   Si II 6355: LUMINA depth 0.76 vs HST 0.62 (Δd=+0.14)  ← largest in-window residual
# All ~+1500-1900 km/s bluer trough velocity. Q1b r03 only damps Si II at
# λ<4000Å (SCALE2 f=0.05). Si II OPTICAL (4000-7000Å) is untouched.
#
# New SCALE6 rule scales Si II A_ul / B_lu / B_ul in [4000,7000]Å.
# Base: Q1b r03 production stack (RMS_bn=0.2375, job 156281).
# Binary: lumina_cuda_h100_aulscale8 (supports SCALE1..8).
#
#   array 0 — S1_ref:  SCALE6 f=1.0 (effectively off) → reproduce Q1b r03 0.2375
#   array 1 — S1_f05:  SCALE6 f=0.5
#   array 2 — S1_f03:  SCALE6 f=0.3
#   array 3 — S1_f01:  SCALE6 f=0.1
...
LABELS=(S1_ref S1_f05 S1_f03 S1_f01)
tag=${LABELS[$SLURM_ARRAY_TASK_ID]}

FE_FAC=0.3
CO_FAC=0.3
RED_FAC=0.3       # Q1b r03 setting
SI_OPT_FAC=1.0
...
label="ddc15S1_${tag}"
work_root="$ROOT/logs/ddc15S1_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "=== S1 [${label}]  Q1b r03 base + SCALE6 Si II opt λ∈[4000,7000] f=${SI_OPT_FAC}  N_PKT=${N_PKT}  N_ITER=${N_ITER} ==="
echo "Host: $(hostname)  GPU: $(nvidia-smi --query-gpu=name --format=csv,noheader|head -1)"
echo "Time: $(date)"

# Suffix 1-5 = Q1b r03 stack (unchanged).
# Suffix 6 = NEW Si II optical-band damp.
LUMINA_BF_OPACITY=1 \
