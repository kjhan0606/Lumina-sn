#!/bin/bash
#SBATCH --job-name=ddc15_A1_eps_fine
...
# A1 (Probe A — fine-ε sweep): EPS_UV around 0.7 on X1_b3p0 base.
#
# X1_b3p0 (job 156525, Tier-3 champion): SCALE7 Ni II opt f=0.05 + SCALE8 Ca II f=3.0,
# EPS_UV=0.7 RED_ONLY → RMS_bn=0.1707, J=0.237.
# Probe A asks: does the EPS_UV knee shift now that line-shape and Ca H&K are dialed?
#
# Array cells (5):
#   0 — e0p55  : EPS_UV=0.55
#   1 — e0p65  : 0.65
#   2 — e0p70  : 0.70  (X1_b3p0 reproducer; MC-noise calibration)
#   3 — e0p80  : 0.80
#   4 — e0p90  : 0.90
...
LABELS=(e0p55 e0p65 e0p70 e0p80 e0p90)
EPS_GRID=(0.55 0.65 0.70 0.80 0.90)
...
label="ddc15A1_${tag}"
work_root="$ROOT/logs/ddc15A1_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "=== A1 [${label}]  X1_b3p0 base + EPS_UV=${EPS_UV}  N_PKT=${N_PKT}  N_ITER=${N_ITER} ==="
