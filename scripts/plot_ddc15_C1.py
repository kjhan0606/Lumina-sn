#!/usr/bin/env python3
"""C1: DDC15 v10400 X_Ti sweep — Ti II curtain [3200,3500]Å + red [5500,9500]Å 동시 측정."""
...
JOB  = sys.argv[1] if len(sys.argv) > 1 else "155742"

X_TIS = ["3e-6", "1e-5", "3e-5", "1e-4"]
CELLS = [(xti, ROOT/f"logs/ddc15C1_{JOB}_ddc15C1_xTi{xti}/lumina_spectrum_formal.csv")
         for xti in X_TIS]
OUT = ROOT/f"figures/ddc15_C1_{JOB}_xTi_sweep.png"
C_KMS = 299792.458

A4_BASELINE = ROOT/"logs/ddc15A4_155722_ddc15_v10400/lumina_spectrum_formal.csv"
...
a4 = scale_to_hst(A4_BASELINE)

fig, axes = plt.subplots(3, 1, figsize=(15, 13))
cmap = plt.cm.viridis(np.linspace(0.15, 0.85, len(data)))

# Panel 1: full overlay
ax = axes[0]
ax.plot(hlam, hflu*1e14, 'k-', lw=1.2, label='HST 2011fe B-max', alpha=0.95)
for (xti, (lam, flu)), c in zip(data.items(), cmap):
    ax.plot(lam, flu*1e14, color=c, lw=0.9, alpha=0.85, label=f'C1 X_Ti={xti}')
if a4 is not None:
    ax.plot(a4[0], a4[1]*1e14, color='tab:red', lw=0.7, alpha=0.55, ls='--',
            label='A4 baseline (X_Ti=1e-4)')
ax.set_xlim(1700, 9500)
ax.set_ylim(0, max(hflu[(hlam>2900)&(hlam<9000)])*1e14*1.15)
ax.set_xlabel('λ (Å)'); ax.set_ylabel('F_λ × 10¹⁴')
ax.set_title(f'C1 X_Ti sweep on DDC15 v_in=10400 (job {JOB})  — Ti II curtain test')
ax.legend(loc='upper right', fontsize=8); ax.grid(alpha=0.2)

# Panel 2: [3000,4500] Ti II curtain zoom
ax = axes[1]
m = (hlam>=3000)&(hlam<=4500)
ymax = hflu[m].max()*1e14*1.20
ax.plot(hlam, hflu*1e14, 'k-', lw=2.0, label='HST', alpha=0.95)
for (xti, (lam, flu)), c in zip(data.items(), cmap):
    ax.plot(lam, flu*1e14, color=c, lw=1.3, alpha=0.9, label=f'X_Ti={xti}')
ax.set_xlim(3000, 4500); ax.set_ylim(0, ymax)
# Mark Ti II / Fe II / Ca II lines
markers = [(3242,'Ti II'), (3349,'Ti II'), (3372,'Ti II'),
           (3613,'Fe II'), (3727,'Fe II'), (3938,'Ca II K'), (3968,'Ca II H'),
           (4233,'Fe II')]
for lam_m, lab in markers:
...
ax.set_xlabel('λ (Å)'); ax.set_ylabel('F_λ × 10¹⁴')
ax.set_title('Ti II curtain [3000-4500]Å — A4 baseline 0.37 at λ=3350')
ax.legend(loc='upper right', fontsize=8); ax.grid(alpha=0.2)

# Panel 3: red [5500,9500]
ax = axes[2]
...
for (xti, (lam, flu)), c in zip(data.items(), cmap):
    ax.plot(lam, flu*1e14, color=c, lw=1.3, alpha=0.9, label=f'X_Ti={xti}')
if a4 is not None:
    ax.plot(a4[0], a4[1]*1e14, color='tab:red', lw=1.0, alpha=0.55, ls='--', label='A4 X_Ti=1e-4')
...
for lam_m, lab in [(5972,'Si II'), (6355,'Si II'), (7773,'O I'),
    ...
    ax.text(lam_m, ymax*0.92, lab, rotation=90, fontsize=7, va='top', ha='right', alpha=0.5)
ax.set_xlabel('λ (Å)'); ax.set_ylabel('F_λ × 10¹⁴')
ax.set_title('Red continuum [5500-9500]Å — Ti↓ → red↓ if redistribution hypothesis is right')
ax.legend(loc='upper right', fontsize=8); ax.grid(alpha=0.2)
...
# Energy budget + band ratios
print(f"\n=== Band ratios (model/HST, green-normalized; <1 dark, >1 bright) ===")
...
print(hdr)
for xti, (lam, flu) in data.items():
    row = f"{xti:<10s}"
    for lo,hi,_ in BANDS:
        rH = band_int(hlam, hflu, lo, hi)
        rM = band_int(lam, flu, lo, hi)
        row += f"{rM/rH:>10.3f}"
    print(row)
if a4 is not None:
    row = f"{'A4 base':<10s}"
    for lo,hi,_ in BANDS:
        rH = band_int(hlam, hflu, lo, hi)
        rM = band_int(a4[0], a4[1], lo, hi)
        row += f"{rM/rH:>10.3f}"
    print(row)

print(f"\n=== Energy redistribution: ΔF_UV [3000,4500] vs ΔF_red [5500,9500] ===")
I_HST_uv  = band_int(hlam, hflu, 3000, 4500)
I_HST_red = band_int(hlam, hflu, 5500, 9500)
print(f"  HST  UV={I_HST_uv:.3e}  red={I_HST_red:.3e}")
print(f"  {'X_Ti':<10s} {'ΔF_UV':>11s} {'ΔF_red':>11s} {'red/uv':>8s}")
for xti, (lam, flu) in data.items():
    ...
    print(f"  {xti:<10s} {dF_uv:>11.3e} {dF_red:>11.3e} {r:>8.2f}")
...
for xti, (lam, flu) in data.items():
    print(f"  C1 X_Ti={xti:8s}  band-log10-RMS = {band_log_rms(lam,flu):.4f}")
if a4 is not None:
    print(f"  A4 baseline (X_Ti=1e-4)   band-log10-RMS = {band_log_rms(a4[0],a4[1]):.4f}")

print(f"\n=== Baseline-norm RMS [3000,8000]Å (line-shape only; secondary) ===")
...
for xti, (lam, flu) in data.items():
    ...
    diff = fm(hlam_n) - hflu_n
    rms = np.sqrt(np.nanmean(diff**2))
    print(f"  C1 X_Ti={xti:8s}  RMS_bn = {rms:.4f}")
if a4 is not None:
    mlam, mflu = gauss_baseline(a4[0], a4[1])
    if mlam is not None:
        fm = interp1d(mlam, mflu, kind='linear', bounds_error=False, fill_value=np.nan)
        rms = np.sqrt(np.nanmean((fm(hlam_n) - hflu_n)**2))
        print(f"  A4 baseline (X_Ti=1e-4)   RMS_bn = {rms:.4f}")
