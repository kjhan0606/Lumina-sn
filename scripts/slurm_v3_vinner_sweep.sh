...
# V3 V_INNER fine sweep around p+3.7d × L=1.45 × W5 champion (Snifs 0.143).
# Pure structural search: vary v_inner in 200 km/s steps.
# Goal: break the 0.143 line-shape floor toward Snifs RMS_bn <= 0.10.
...
VI_LIST=(9800 10000 10200 10300 10400 10500 10600 10800)
...
label="v3_vin${vi}"
work_root="$ROOT/logs/v3vin_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
print(f"  t_exp={t_exp:.0f}s  L={L_inner:.3e}  T_inner={T_inner:.1f}K  v_inner={vi_kms} km/s")
PYEOF

echo "=== V3 VIN SWEEP [${label}] vi=${vi} km/s ==="
...
print(f"  V3 VIN ${vi}km/s  Snifs RMS_bn [3300,8000]  = {r_snifs:.4f}  ({'<=0.10 TARGET MET' if r_snifs<=0.10 else ('beats V3 0.143' if r_snifs<0.143 else 'no improvement')})")
...
if parts:
    ...
    if gH > 0:
        ...
        print(f"  V3 VIN ${vi}km/s  HST RMS_bn [3000,8000]  = {r_hst_can:.4f}")
        print(f"  V3 VIN ${vi}km/s  HST RMS_bn [1700,3000]= {r_hst_uv:.4f}  (UV blanketing)")
PYEOF
