...
# vi × L × ζ 3-knob corner grid — basin search beyond single-knob NULLs.
# 12 cells × 800K × 12 iter at p+3.7d, single seed (23111963), de=9.5, W5 niF005.
#
# Layout (idx, vi, L, ζ):
#   0: 10400  1.45  ref     6: 10400  1.55  ref
#   1: 10400  1.45  clean   7: 10400  1.55  clean
#   2: 10600  1.45  ref     8: 10600  1.55  ref
#   3: 10600  1.45  clean   9: 10600  1.55  clean
#   4: 10800  1.45  ref    10: 10800  1.55  ref
#   5: 10800  1.45  clean  11: 10800  1.55  clean
#
# Champion criterion: any cell Snifs < 0.130 (3σ_MC below V3.x floor 0.139).
...
VI_LIST=(10400 10400 10600 10600 10800 10800 10400 10400 10600 10600 10800 10800)
L_LIST=(1.45 1.45 1.45 1.45 1.45 1.45 1.55 1.55 1.55 1.55 1.55 1.55)
ZETA_LIST=(refz cleanz refz cleanz refz cleanz refz cleanz refz cleanz refz cleanz)

vi=${VI_LIST[$SLURM_ARRAY_TASK_ID]}
L_scale=${L_LIST[$SLURM_ARRAY_TASK_ID]}
zeta_tag=${ZETA_LIST[$SLURM_ARRAY_TASK_ID]}

if [ "$zeta_tag" = "cleanz" ]; then
...
label="vlz_${tag}_vi${vi}_L${L_scale//./p}_${zeta_tag}"
work_root="$ROOT/logs/vlz_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
t_exp = $t_exp_s; L_inner = $L_inner; SEED = $SEED
...
print(f"  t_exp={t_exp:.0f}s  L={L_inner:.3e}  T_inner={T_inner:.1f}K  rho0_v11k={rho0:.3e}  seed={SEED}")
print(f"  vi={vi_kms} km/s  L_scale=$L_scale  zeta={'$zeta_tag'}")
print(f"  SRC_REF = {src}")
PYEOF

echo "=== VLZ [${label}] L=${L_inner} N_PKT=${N_PKT} seed=${SEED} ==="
...
echo ""
echo "=== score [${label}] vs Snifs $snifs + HST $hst_tag (noise-weighted) ==="
python3 - <<PYEOF
...
print(f"  [vi=$vi L=$L_scale $zeta_tag] Snifs [3300,8000]   rms_bn = {r:.4f}   chi_bn = {chi:.3f}")
    ...
    if gH > 0:
        for lo, hi, lbl in [(3000., 8000., "[3000,8000]"), (1700., 3000., "[1700,3000]")]:
            r, chi = rms_bn_nw(mlam, mflu, hl, hf, gH,
                               ...
                               wl_lo=lo, wl_hi=hi)
            print(f"  [vi=$vi L=$L_scale $zeta_tag] HST   {lbl}  rms_bn = {r:.4f}   chi_bn = {chi:.3f}")
PYEOF
