...
# #273 NLTE O II/III 3-seed MC bracket on FEMERGE Phase 9b best cell.
# Same physics as nlteO_prod; SEED env var (default 23111964) is injected into
# config.json so each invocation uses a distinct MC realization. Submit with
# SEED=23111964 (s2) and SEED=23111965 (s3); job 157524 (default 23111963) = s1.
# 800K x 12 iter per seed. σ_MC≈0.005 expected, champion threshold ΔSnifs<-0.006.
...
# Use the freshly built NLTE_MAX_IONS=30 binary (A100 sm_80).
# H100/H200 falls back to same binary — sm_80 PTX is forward-compatible.
...
label="nlteO_seed${seed_tag}_vi${vi_tag}_L${l_tag}_de${de_tag}_${tag}"
...
echo "=== NLTEO-SEED${seed_tag} VI=$VI_KMS L=$L_scale DE=$DE_ARG SEED=$SEED | NI=$NI_UV_FAC FE=$FE_FAC CO=$CO_FAC NI2_OPT=$NI2_OPT_FAC CA2=$CA2_FAC S9=$S9_FAC [${label}] ==="
...
echo ""
echo "--- NLTE init lines (confirm Z=8 ion=1/2 appear, total ion count = 30) ---"
grep -E "Total NLTE levels|Z=8 ion=|Z=" stdout.log | head -40

echo ""
echo "--- convergence / final lines ---"
grep -E 'Mean \|W error|Mean \|T_rad error|T_inner final|L_emitted|exit' stdout.log | tail -15
