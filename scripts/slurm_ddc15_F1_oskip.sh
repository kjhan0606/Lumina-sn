#!/bin/bash
#SBATCH --job-name=ddc15_F1_oskip
...
# F1: Diagnostic toggle — zero-out light-element line opacity on C2 champion
# (DDC15 v_in=10400, X_Fe_outer=0.05). Tests NIR opacity audit hypothesis
# (project_nir_opacity_audit_oi_dominance): O I dominates [5500,9500]Å.
# Predicted: variant 1 (Z=8) should drop red-tot meaningfully if O I lever real.
#   array 0 — Z=     (baseline reproducibility, no mask)
#   array 1 — Z=8    (O zero-out)
#   array 2 — Z=6,8  (C+O zero-out, full light-element check)
...
BIN="$ROOT/lumina_cuda_h100_oskip"
...
SKIPS=("" "8" "6,8")
LABELS=("base" "Z8" "Z6_8")
skip="${SKIPS[$SLURM_ARRAY_TASK_ID]}"
label="${LABELS[$SLURM_ARRAY_TASK_ID]}"

# Fixed C2 champion params
vi=10400
ro=0.00
de=9.0
xfe=0.05

work_root="$ROOT/logs/ddc15F1_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "=== F1 oskip [${label}]  SKIP_Z='${skip}'  X_Fe_outer=${xfe}  v_in=${vi}  N_PKT=${N_PKT} ==="
...
grep -E 'Mean \|W error|Mean \|T_rad error|T_inner final|L_emitted|OPACITY_SKIP' stdout.log | tail -10
echo "Done: $(date)"
