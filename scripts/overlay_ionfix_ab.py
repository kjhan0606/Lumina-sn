#!/usr/bin/env python3
"""Overlay: SN2002bo (dereddened) vs old superlev (161630) vs ionfix (161737).
...
runs = [load_pinned("161630","old superlev (161630)"),
        load_pinned("161737","ionfix, radeq OFF (161737)"),
        load_pinned("161773","ionfix + radeq ON (161773)")]
...
for lo,hi,name in [(1500,3356,"unobserved FUV"),(4600,4800,"4700 trap"),(9000,10200,"NIR II")]:
    ax.axvspan(lo,hi,color="gold",alpha=0.12)
    ax.text((lo+hi)/2, 0, name, ha="center", va="bottom", fontsize=8, color="darkgoldenrod")
ax.axvline(3356, color="gray", ls=":", lw=0.8)
ax.set_xlim(1800,10200); ax.set_ylim(0, None)
ax.set_xlabel("wavelength (Å)"); ax.set_ylabel("F_λ (erg/s/cm²/Å, optical-anchored)")
ax.set_title("ionfix + radeq: optical/NIR shape recovered, but FUV excess grows (unobserved in 2002bo)")
ax.legend(fontsize=9, loc="upper right")
out = ROOT/"figures/2026-06-01_ionfix_radeq_3way_sn2002bo.png"
