#!/bin/bash
#SBATCH --job-name=ddc15_223_ionlock
...
# #223 smoke: does LUMINA_NLTE_ION_LOCK=1 populate S II level pops?
#
# Hypothesis: at SN T_e ≈ 8000 K with Saha S II→III balance, the gauss-solved
# b[i] vector concentrates mass in S III levels. The default combined-Σ rescale
# (lumina_plasma.c:2977 / lumina_cuda.cu:558) then preserves that ratio, so
# S II levels end up at the 1e-30 clamp × tiny weight ≈ 1e-28. Ion-lock mode
# instead applies per-ion (scale_lo, scale_hi) rescale, which should pin
# Σ S II levels to the nebular n_ion ≈ 2e+8.
#
# X1_b3p0 base (RMS champion stack), 200K × 6 iter for fast feedback.
# Cells:
#   0 — ctrl     : ion-lock OFF (= current production behavior)
#   1 — ionlock  : LUMINA_NLTE_ION_LOCK=1
#
# Diagnostic: grep stdout for "S II shell 0: NLTE n_total" and "Si II shell 0".
# Pass = n_total ≈ 2e+8 (nebular), Fail = n_total ≈ 1e-28 (clamped).
...
ilock=${ION_LOCK_VALS[$SLURM_ARRAY_TASK_ID]}
...
work_root="$ROOT/logs/ddc15_223_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "=== #223 [${label}]  ION_LOCK=${ilock}  N_PKT=${N_PKT}  N_ITER=${N_ITER} ==="
echo "Host: $(hostname)  GPU: $(nvidia-smi --query-gpu=name --format=csv,noheader|head -1)"
echo "Binary: $BIN"
echo "Time: $(date)"

# Note: DROPPING LUMINA_NLTE_SKIP_Z=14 to expose Si II to NLTE for diagnostic.
LUMINA_BF_OPACITY=1 \
LUMINA_CMFGEN_SIGMA_BF="$ROOT/data/atomic/cmfgen_sigma_bf.bin" \
LUMINA_DYNAMIC_TRANSPROB=1 \
LUMINA_NLTE_START_ITER=2 \
LUMINA_NLTE_ION_LOCK="$ilock" \
...
echo ""
echo "=================================================="
echo "  S II / Si II / Ca II shell-0 n_total trajectory  "
echo "=================================================="
grep -E "S II shell 0|Si II shell 0|Ca II shell 0|S III shell 0|Si III shell 0" stdout.log | head -40
echo ""
echo "--- NLTE fallback warnings (any Z=14/16/20 in stderr?) ---"
grep -E "NLTE-FALLBACK.*Z=(14|16|20)" stderr.log | head -10
echo ""
echo "--- convergence ---"
