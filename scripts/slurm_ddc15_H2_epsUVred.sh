#!/bin/bash
#SBATCH --job-name=ddc15_H2_epsUV_redonly
...
# H2: wavelength-conditional EPS_UV. LUMINA_EPS_UV_RED_ONLY=1 moves the
# UV-entry gate to fire POST-cascade and only when the cascade exit lands
# in [5500,10000)A. Hypothesis: this kills UV->red conversion (the source
# of red excess) while preserving the useful UV->UV/blue downbranching
# (which H1b ε>0.4 showed was working).
#
#   array 0 — EPS_UV=0.0 (control)
#   array 1 — EPS_UV=0.4  (matched H1b joint optimum)
#   array 2 — EPS_UV=0.7
#   array 3 — EPS_UV=0.9  (matched H1 ε=0.9 — direct H2/H1 comparison)
...
BIN="$ROOT/lumina_cuda_h100_h2"

N_PKT=200000
N_ITER=8

EPS_UVS=(0.0 0.4 0.7 0.9)
...
label=$(printf "ddc15H2_epsUV%s_redonly" "$eps_uv")
work_root="$ROOT/logs/ddc15H2_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "=== H2 epsUV red-only [${label}]  EPS_UV=${eps_uv}  RED_ONLY=1  N_PKT=${N_PKT} ==="
