#!/bin/bash
#SBATCH --job-name=nlte3fix_optscan
...
# #219f Phase 8: optical-band SCALE7/8/9 re-tune on FEMERGE best base.
# UV (SCALE1/3/4) fixed at best (NI=0.3, FE=0.02, CO=0.3); SCALE2 disabled.
#
# Args: $1 = NI2_OPT_FAC  (SCALE7 Ni II opt [4000,7500], default 0.05)
#       $2 = CA2_FAC      (SCALE8 Ca II    [3700,9000], default 3.0)
#       $3 = S9_FAC       (SCALE9 Ni II NIR [7500,10000], default 0.05)
...
label="nlte3fix_optscan_ni2o${ni2opt_tag}_ca${ca2_tag}_s9${s9_tag}_${tag}"
...
echo "=== NLTE3FIX-OPTSCAN NI2_OPT=$NI2_OPT_FAC CA2=$CA2_FAC S9=$S9_FAC | NI=$NI_UV_FAC FE=$FE_FAC CO=$CO_FAC [${label}] ==="
...
print(f"  [OPTSCAN ni2o=$NI2_OPT_FAC ca=$CA2_FAC s9=$S9_FAC] Snifs [3300,8000]   rms_bn = {r:.4f}   chi_bn = {chi:.3f}")
    ...
    if gH > 0:
        for lo, hi, lbl in [(3000., 8000., "[3000,8000]"), (1700., 3000., "[1700,3000]")]:
            r, chi = rms_bn_nw(mlam, mflu, hl, hf, gH,
                               ...
                               wl_lo=lo, wl_hi=hi)
            print(f"  [OPTSCAN ni2o=$NI2_OPT_FAC ca=$CA2_FAC s9=$S9_FAC] HST   {lbl}  rms_bn = {r:.4f}   chi_bn = {chi:.3f}")
PYEOF
