#!/bin/bash
#SBATCH --job-name=ddc15_R1_eps2step
...
# Replaces H1's full Planck(T_rad) thermalization (which destroyed P-Cygni shape
# at 800K → RMS_bn 0.288) with band-constrained re-emit into [3500,5500]Å. The
# packet then resumes normal line transport and produces red flux via optical
# Si II/Fe II/Ca II line cascades, preserving line shapes.
#
# Base: P2 stack (Ni 0.3 + Si 0.05 + Fe 0.3 + Co 0.3 λ<4000Å, ion=1) + Q1b r03
#       (Cr/Fe/Co III f=0.3 λ∈[4500,7500] ion=2). Current production floor 0.2375.
# Binary: lumina_cuda_h100_eps2step (sm_90, LUMINA_EPS_UV_2STEP + 2STEP_BAND_*).
#
#   array 0 — R1_ref:  ε_UV=0 (recovers Q1b r03 baseline, intra-job anchor)
#   array 1 — R1_e02:  ε_UV=0.2 + 2STEP [3500,5500]
#   array 2 — R1_e04:  ε_UV=0.4 + 2STEP [3500,5500]
#   array 3 — R1_e06:  ε_UV=0.6 + 2STEP [3500,5500]
#   array 4 — R1_e09:  ε_UV=0.9 + 2STEP [3500,5500]  (matches H1 ε=0.9 for direct compare)
...
BIN="$ROOT/lumina_cuda_h100_eps2step"

N_PKT=800000
N_ITER=12

LABELS=(R1_ref R1_e02 R1_e04 R1_e06 R1_e09)
...
label="ddc15R1_${tag}"
work_root="$ROOT/logs/ddc15R1_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "=== R1 [${label}]  Q1b-r03 base + EPS_UV=${EPS_UV} 2STEP[3500,5500]Å  N_PKT=${N_PKT}  N_ITER=${N_ITER} ==="
...
LUMINA_EPS_UV_2STEP=1 \
LUMINA_EPS_UV_2STEP_BAND_LO=3500 \
LUMINA_EPS_UV_2STEP_BAND_HI=5500 \
