#!/bin/bash
#SBATCH --job-name=nlte3fix_fsw34
...
# #219f Phase 4: SCALE3 (Fe II UV) × SCALE4 (Co II UV) narrow re-search on FEMERGE.
#
# Args: $1 = FE_FAC (SCALE3, Z=26 ion=1 λ<4000)
#       $2 = CO_FAC (SCALE4, Z=27 ion=1 λ<4000)
# Baseline: V3.1 stack on FEMERGE MINUS EPS_UV (proven null in drop[eps] 157412).
#   SCALE1 Ni II UV f=0.3, SCALE2 Si II UV f=0.05 (consistency, Si SKIP'd),
#   SCALE6 Si II opt f=0.10, SCALE7 Ni II opt f=0.05,
#   SCALE8 Ca II f=3.0, SCALE9 Ni II NIR f=0.05.
#
# Anchors (V3.1 default FE=0.3 CO=0.3):
#   157412 DROP[eps] = same baseline as this sweep: Snifs 0.274 / HST UV 0.297
...
# fixed knobs (all f values shipped in 9-knob stack)
SI_OPT_FAC=0.10
NI2_OPT_FAC=0.05
CA2_FAC=3.0
S9_FAC=0.05
...
label="nlte3fix_fsw34_fe${fe_tag}_co${co_tag}_${tag}"
...
echo "=== NLTE3FIX-SW34 FE=$FE_FAC CO=$CO_FAC [${label}] L=${L_inner} ==="
echo "Host: $(hostname)  GPU: $(nvidia-smi --query-gpu=name --format=csv,noheader|head -1)"
echo "Binary: $BIN  Time: $(date)"
echo "** Baseline: V3.1 stack MINUS EPS_UV (proven null). Sweep SCALE3 (Fe II UV) × SCALE4 (Co II UV). **"
...
print(f"  [SW34 FE=$FE_FAC CO=$CO_FAC] Snifs [3300,8000]   rms_bn = {r:.4f}   chi_bn = {chi:.3f}")
    ...
    if gH > 0:
        for lo, hi, lbl in [(3000., 8000., "[3000,8000]"), (1700., 3000., "[1700,3000]")]:
            r, chi = rms_bn_nw(mlam, mflu, hl, hf, gH,
                               ...
                               wl_lo=lo, wl_hi=hi)
            print(f"  [SW34 FE=$FE_FAC CO=$CO_FAC] HST   {lbl}  rms_bn = {r:.4f}   chi_bn = {chi:.3f}")
PYEOF
