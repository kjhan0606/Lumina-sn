#!/bin/bash
#SBATCH --job-name=w2_logL_red5_diag
...
# W2: p+3.7d structural × iron-peak III lever sweep with SED band-ratio diagnostic.
# Anchor: W1 r070 (X1_b3p0 + RED5=0.70) — best HST canonical cell.
# Tests the "reciprocal SED" hypothesis:
#   UV [1700,2500] LUMINA/HST = 0.3-0.5  (under-emit)
#   NIR [6500,10000] LUMINA/HST = 1.7-2.4  (over-emit)
#   → energy is being shifted UV→NIR by cascade
#   Real lever should move BOTH together (UV up + NIR down).
#
#   0 — ctrl   (L=1.00, RED5=0.70) = W1 r070 reproducer
#   1 — L085   (L=0.85, RED5=0.70) cooler structure
#   2 — L115   (L=1.15, RED5=0.70) hotter structure
#   3 — L130   (L=1.30, RED5=0.70) very hot
#   4 — nored  (L=1.00, RED5=1.00) no iron-peak III damp
#   5 — L115_nored (L=1.15, RED5=1.00) hot + full iron-peak III
#   6 — L085_nored (L=0.85, RED5=1.00) cool + full iron-peak III
#   7 — L130_nored (L=1.30, RED5=1.00) very hot + full iron-peak III
...
# X1_b3p0 fixed (W1 r070 inherits these unchanged)
FE_FAC=0.3
CO_FAC=0.3
SI_OPT_FAC=0.10
NI2_OPT_FAC=0.05
...
L_SCALE_ARR=(1.00 0.85 1.15 1.30 1.00 1.15 0.85 1.30)
RED_FAC_ARR=(0.70 0.70 0.70 0.70 1.00 1.00 1.00 1.00)
TAG_ARR=(ctrl L085 L115 L130 nored L115_nored L085_nored L130_nored)
...
label="w2_${tag}"
work_root="$ROOT/logs/w2_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "=== W2 [${label}] L_scale=${L_SCALE} L_inner=${L_inner} RED5=${RED_FAC} ==="
...
echo ""
echo "=== score [${label}] L_scale=${L_SCALE} RED5=${RED_FAC} ==="
python3 <<PYEOF
...
print(f"  W2 ${tag}  Snifs RMS_bn [3300,8000]  = {r_snifs:.4f}")
...
if parts:
    ...
    if gH > 0:
        ...
        print(f"  W2 ${tag}  HST RMS_bn [3000,8000]   = {r_hst_can:.4f}")
        print(f"  W2 ${tag}  HST RMS_bn [1700,3000] = {r_hst_uv:.4f}")
        # === SED band-ratio diagnostic (THIS IS NEW) ===
        # normalize LUMINA to HST [4500,5800]
