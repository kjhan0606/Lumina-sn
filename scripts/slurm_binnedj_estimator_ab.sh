...
# Task #27 binned-J estimator A/B. The whole over-absorption investigation
# converged on the Lucy (W,T_rad) estimator: the nu_bar/j first-moment T_rad
# collapses under any redshifted field (Phase-2c downbranch: T_rad 5786->2297K,
# W 0.48->23, NIR x3), because the first moment is dragged to the red tail.
# This replaces the moment with a dilute-Planck SHAPE FIT to the frequency-
# resolved J_nu histogram already accumulated for NLTE rates (W absorbs
# amplitude, T set by the SED slope -> robust to redistribution).
#   A (ARM=A): moment estimator   LUMINA_BINNED_J_ESTIMATOR=0  (Phase-2c A repro)
#   B (ARM=B): binned-J fit       LUMINA_BINNED_J_ESTIMATOR=1
# BOTH: scatter, diffusive BC ON, self-pin ON, DDC15-strat, 200k, N_ITER=10,
# DYNAMIC_TRANSPROB=0. Single variable = the radiation-field estimator.
# Pre-registered checks: (1) B is STABLE (no NaN, converges); (2) B's W stays
# physical (W<~1, not the moment's inflated W>1 inner shells); (3) B's T_rad
# does NOT collapse; (4) spectrum effect on blue[3500-4500] / NIR bands. If B
# is sane, the NEXT step is to re-run the downbranch fluorescence A/B WITH
# binned-J on (the estimator collapse was the blocker that made fluorescence
# unusable).
#   sbatch --export=ALL,ARM=A scripts/slurm_binnedj_estimator_ab.sh
#   sbatch --export=ALL,ARM=B scripts/slurm_binnedj_estimator_ab.sh

ARM=${ARM:-A}
DIFFUSE_BC=1                      # both arms build on the Phase-2a fix
LINE_INT=scatter                 # both arms scatter; isolate the estimator
if [ "$ARM" = "B" ]; then
    BINNEDJ=1
else
    BINNEDJ=0
...
echo "=== Task #27 binned-J estimator A/B  ARM=$ARM  BINNED_J=$BINNEDJ  LINE_INT=$LINE_INT  DIFFUSE_BC=$DIFFUSE_BC  VI=$VI_KMS L=$L_SCALE_ARG  N_PKT=$N_PKT N_ITER=$N_ITER ==="
echo "Host: $(hostname)  GPU: $GPU_NAME"
echo "Binary: $BIN  Time: $(date)"
echo "Both arms: scatter, diffusive BC ON, self-pin ON. A=moment estimator (Phase-2c A repro), B=binned-J dilute-Planck fit. Static clean transprobs (DYNAMIC_TRANSPROB=0)."
