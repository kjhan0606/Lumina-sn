#!/bin/bash
#SBATCH --job-name=ddc15_O1_CoCr_stack
...
# O1: Add Co II (suffix=4) and Cr II (suffix=5) UV A_ul cuts on top of N1
# (K+L+M = Ni II f=0.3 + Si II f=0.05 + Fe II f=0.5, λ<4000Å).
# Carsus audit: Co II ratios 0.55-0.83 vs NIST, Cr II 0.70-1.47. Bulk
# unchecked lines (132k Co II + 31k Cr II) may carry hidden over-strength
# similar to Fe II 2260 (46× over NIST).
# Binary: lumina_cuda_h100_aulscale5 (5-suffix AUL_SCALE loop).
#
#   array 0 — N1 triple (K+L+M only) — local fresh-seed reference
#   array 1 — N1 + Co II f=0.5 (suffix=4)
#   array 2 — N1 + Cr II f=0.5 (suffix=5)
#   array 3 — full 5-ion stack: N1 + Co + Cr
...
LABELS=(N1_repl O1_Co O1_Cr O1_CoCr)
...
label="ddc15O1_${tag}"
work_root="$ROOT/logs/ddc15O1_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "=== O1 5-ion stack [${label}]  N1=baked(Ni 0.3, Si 0.05, Fe 0.5)  CO_FAC=${CO_FAC}  CR_FAC=${CR_FAC}  λ<4000Å  N_PKT=${N_PKT}  N_ITER=${N_ITER} ==="
echo "Host: $(hostname)  GPU: $(nvidia-smi --query-gpu=name --format=csv,noheader|head -1)"
echo "Time: $(date)"

# N1 baked rules (always on):
#   suffix=""  Ni II f=0.3
#   suffix=2   Si II f=0.05
#   suffix=3   Fe II f=0.5
# O1 extras:
#   suffix=4   Co II f=$CO_FAC
#   suffix=5   Cr II f=$CR_FAC
LUMINA_BF_OPACITY=1 \
...
LUMINA_AUL_SCALE5_FACTOR="$CR_FAC" \
LUMINA_AUL_SCALE5_LAMBDA_MAX=4000 \
LUMINA_AUL_SCALE5_ZMASK=24 \
LUMINA_AUL_SCALE5_IONMASK=1 \
...
grep -E 'Mean \|W error|Mean \|T_rad error|T_inner final|L_emitted|AUL_SCALE' stdout.log | tail -20
echo "Done: $(date)"
