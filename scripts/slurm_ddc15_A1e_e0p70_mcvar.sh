#!/bin/bash
#SBATCH --job-name=ddc15_A1e_e0p70_mcvar
...
# A1e (MC variance at ε=0.7): paired ensemble vs A1d at ε=1.0.
#
# A1d (job 156569) 5-seed at ε=1.0 → σ_typical=0.003, ~17% outliers at +6σ
# (s5=0.1998 vs typical mean 0.1771). X1_b3p0 (job 156525, ε=0.7, base seed)
# = 0.171 was a single realization — could be lucky. A1e gives 5 more seeds
# at ε=0.7 to characterize σ_MC and compare ensemble vs A1d's ε=1.0 mean
# 0.177±0.003.
#
# Array cells (5):
#   0-4 — e0p70_s{1..5}: EPS_UV=0.70, same 5 seeds as A1d for paired diff
...
LABELS=(e0p70_s1 e0p70_s2 e0p70_s3 e0p70_s4 e0p70_s5)
EPS_GRID=(0.70 0.70 0.70 0.70 0.70)
...
label="ddc15A1e_${tag}"
work_root="$ROOT/logs/ddc15A1e_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "=== A1e [${label}]  X1_b3p0 base + EPS_UV=${EPS_UV}  SEED=${SEED}  N_PKT=${N_PKT}  N_ITER=${N_ITER} ==="
