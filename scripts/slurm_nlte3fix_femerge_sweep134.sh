#!/bin/bash
#SBATCH --job-name=nlte3fix_fsw134
...
# #219f Phase 5: SCALE1 (Ni II UV) × SCALE3 (Fe II UV) × SCALE4 (Co II UV).
#
# Args: $1 = NI_UV_FAC (SCALE1, Z=28 ion=1 λ<4000)
#       $2 = FE_FAC    (SCALE3, Z=26 ion=1 λ<4000)
#       $3 = CO_FAC    (SCALE4, Z=27 ion=1 λ<4000)
#
# Phase4 found SCALE4 NULL axis. SCALE3 monotone improving as FE→0 (best 0.1 = 0.263 Snifs).
# Extend SCALE3 below 0.1, then sweep SCALE1 at new optimum.
...
label="nlte3fix_fsw134_ni${ni_tag}_fe${fe_tag}_co${co_tag}_${tag}"
...
echo "=== NLTE3FIX-SW134 NI=$NI_UV_FAC FE=$FE_FAC CO=$CO_FAC [${label}] ==="
...
print(f"  [SW134 NI=$NI_UV_FAC FE=$FE_FAC CO=$CO_FAC] Snifs [3300,8000]   rms_bn = {r:.4f}   chi_bn = {chi:.3f}")
    ...
    if gH > 0:
        for lo, hi, lbl in [(3000., 8000., "[3000,8000]"), (1700., 3000., "[1700,3000]")]:
            r, chi = rms_bn_nw(mlam, mflu, hl, hf, gH,
                               ...
                               wl_lo=lo, wl_hi=hi)
            print(f"  [SW134 NI=$NI_UV_FAC FE=$FE_FAC CO=$CO_FAC] HST   {lbl}  rms_bn = {r:.4f}   chi_bn = {chi:.3f}")
PYEOF
