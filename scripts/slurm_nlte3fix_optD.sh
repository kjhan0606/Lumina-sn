...
# #219e2 OptionD: ION_LOCK earlier-start to give the bug-fix more iters.
#
# OptC (157285, LOCK_START_ITER=11) was marginal: ΔSnifs +0.003 (NULL within σ_MC),
# ΔHST_UV +0.032 (real degradation), T_inner err -1.26% → -0.91% (small improvement).
# Interpretation: only 1 fixed-NLTE iter + 1 transport iter is too brief for the
# corrected opacity to redistribute through the V3.1 SCALE stack — radiation field
# never re-equilibrates.
#
# OptD: LOCK_START_ITER=8. Iters 2..7 run normal V3.1 plasma + NLTE (with bug);
# iters 8..11 run frozen plasma + bug-fix two-row NLTE conservation (4 iters for
# the corrected level populations to propagate through opacity → tau → spectrum);
# iter 12 transport-only. Trade-off: lose 4 iters of plasma-W/T_rad evolution
# (frozen at iter-7 state), but gain 4 iters of bug-fixed radiation field.
#
# Comparison: 157277 baseline (V3.1, SKIP_Z=14 only), 157285 OptC (LOCK_START=11).
# Decision tree:
#   ΔSnifs < -0.006 → real signal, push to LOCK_START_ITER=5 or full freeze
#   ΔSnifs ≈ 0     → ION_LOCK structurally invisible on V3.1, defer to #219f CMFGEN
#   ΔSnifs > +0.010 → freeze trade-off too costly, abandon ION_LOCK direction
...
label="nlte3fix_optD_${tag}"
work_root="$ROOT/logs/nlte3fix_optD_${SLURM_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "Time: $(date)"
echo "** ION_LOCK=1 + LOCK_START_ITER=8 — V3.1 plasma converges iters 2..7, bug-fix two-row conservation iters 8..11, transport iter 12 (4 frozen iters for fix to propagate) **"
...
print(f"  [OptD] Snifs [3300,8000]   rms_bn = {r:.4f}   chi_bn = {chi:.3f}")
    ...
    if gH > 0:
        for lo, hi, lbl in [(3000., 8000., "[3000,8000]"), (1700., 3000., "[1700,3000]")]:
            r, chi = rms_bn_nw(mlam, mflu, hl, hf, gH,
                               ...
                               wl_lo=lo, wl_hi=hi)
            print(f"  [OptD] HST   {lbl}  rms_bn = {r:.4f}   chi_bn = {chi:.3f}")
PYEOF
