#!/bin/bash
#SBATCH --job-name=ddc15_K1b_NiII_Aul_strong
...
# K1b: Push Ni II A_ul scaling further after K1 f0p3 hit (ΔRMS_bn -0.039,
# red 1.50→1.43, depth 0.166→0.201). K1 was non-monotone: 0.7/0.5 worse,
# 0.3 better. Test extreme to detect monotone trend / saturation knee.
# f0p3 reused as replication/control; new cells push to {0.2, 0.1, 0.05}.
# 800K × 12 iter, C2 base (DDC15 v_in=10400, X_Fe_outer=0.05). Binary: lumina_cuda_h100_aulscale.
#
#   array 0 — factor=0.3   K1 winner (replication)
#   array 1 — factor=0.2   strong
#   array 2 — factor=0.1   very strong
#   array 3 — factor=0.05  extreme
...
FACTORS=(0.3  0.2  0.1  0.05)
LABELS=(NiII_f0p3_rep NiII_f0p2 NiII_f0p1 NiII_f0p05)
...
label="ddc15K1b_${tag}"
work_root="$ROOT/logs/ddc15K1b_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "=== K1b Ni II A_ul scale strong [${label}]  FACTOR=${factor}  Z=28(Ni) ion=1(II)  λ_max=4000Å  N_PKT=${N_PKT}  N_ITER=${N_ITER} ==="
