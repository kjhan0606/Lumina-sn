#!/bin/bash
#SBATCH --job-name=ddc15_223_perionresc
...
# #223 smoke v2: LUMINA_NLTE_PER_ION_RESCALE=1 (new env decoupled from freeze).
#
# Previous job 156849 (ION_LOCK=1) was contaminated by the freeze-plasma-transport-only
# path at cuda.cu:2602 + main.c:439 — at iter≥1 the freeze fires first and the NLTE
# solver never runs, so the per-ion rescale code is dead.
#
# New env LUMINA_NLTE_PER_ION_RESCALE=1 gates only the per-pair rescale path
# (plasma.c:2941+2989 and cuda.cu:458+520). No freeze, no matrix-row replacement.
#
# Cells:
#   0 — ctrl       : both knobs OFF (= current production behavior, control)
#   1 — perionresc : LUMINA_NLTE_PER_ION_RESCALE=1, ION_LOCK=0
#
# Pass: S II shell 0 n_total ≈ 2e+8 (nebular), Si II ≈ 7e+8, Ca II ≈ 9.6e+3.
# Fail: still ≈ 1e-28 / 1e+6 collapse — fix doesn't address the bug.
...
BIN="$ROOT/lumina_cuda_h100_perionresc"

N_PKT=200000
N_ITER=6

LABELS=(ctrl perionresc)
RESCALE_VALS=(0 1)
tag=${LABELS[$SLURM_ARRAY_TASK_ID]}
prescale=${RESCALE_VALS[$SLURM_ARRAY_TASK_ID]}
...
label="ddc15_223_${tag}"
work_root="$ROOT/logs/ddc15_223pr_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "=== #223pr [${label}]  PER_ION_RESCALE=${prescale}  N_PKT=${N_PKT}  N_ITER=${N_ITER} ==="
echo "Host: $(hostname)  GPU: $(nvidia-smi --query-gpu=name --format=csv,noheader|head -1)"
echo "Binary: $BIN"
echo "Time: $(date)"

# Note: SKIP_Z=14 dropped, ION_LOCK=0 forced; only per-ion rescale knob varies.
LUMINA_BF_OPACITY=1 \
LUMINA_CMFGEN_SIGMA_BF="$ROOT/data/atomic/cmfgen_sigma_bf.bin" \
LUMINA_DYNAMIC_TRANSPROB=1 \
LUMINA_NLTE_START_ITER=2 \
LUMINA_NLTE_ION_LOCK=0 \
LUMINA_NLTE_PER_ION_RESCALE="$prescale" \
...
echo ""
echo "=================================================="
echo "  S II / Si II / Ca II / Fe II shell-0 n_total     "
echo "=================================================="
grep -E "S II shell 0|Si II shell 0|Ca II shell 0|Fe II shell 0|Mg II shell 0|Al II shell 0|S III shell 0|Si III shell 0" stdout.log | head -40
echo ""
echo "--- NLTE fallback warnings ---"
grep -E "NLTE-FALLBACK" stderr.log | head -10
echo ""
echo "--- convergence ---"
