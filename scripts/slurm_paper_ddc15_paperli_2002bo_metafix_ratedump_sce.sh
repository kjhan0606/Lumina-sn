#!/bin/bash
#SBATCH --job-name=paperli_2002bo_ratedump_sce
...
# SCE VARIANT (2026-05-29): LUMINA_SELF_CONSISTENT_TE=1 to test #305-1 lever.
# Probes whether per-shell energy-balance T_e closes the per-band UV/NIR gap
# vs the production ratio-mode T_e = 0.9*T_rad uniform.
# All other paper-comp env identical to the production script.
#
...
RUN_TAG=paperli_metafix_ratedump_sce_${LINE_INT}
...
env LUMINA_BF_OPACITY=1 \
    ...
    LUMINA_F_COLL_BOOST=${LUMINA_F_COLL_BOOST:-12} \
