#!/bin/bash
#SBATCH --job-name=nlte3fix_femerge_bare
...
# #219f Phase 2: bare-FEMERGE floor probe.
#
# Strip ALL V3.1 SCALE knobs + EPS_UV from the FEMERGE smoke (157291).
# Keeps only SKIP_Z=14 (Si II solver bug, unrelated to iron-peak III) and
# the CMFGEN bb merge atomic data. This measures the unscaled line-shape
# floor of the corrected iron-peak III bb network.
#
# Comparison points:
#   V3.1 stack (157285 OptC):     Snifs 0.138 / HST opt 0.168 / HST UV 0.365
#   FEMERGE V3.1 stack (157291):  Snifs 0.271 / HST opt 0.298 / HST UV 0.297
#   FEMERGE bare (this job):      ?         / ?            / ?
#
# Decision tree:
#   bare Snifs ≈ 0.20-0.25 → SCALE knobs were over-correcting on the buggy state;
#                            incremental SCALE rebuild from bare baseline (one
#                            knob at a time, keep only improvers)
#   bare Snifs ≈ 0.4+      → corrected bb still needs structural knobs but
#                            different ones; consider widened bands or different
#                            (Z, ion) targets
#   bare HST UV ≤ 0.25     → headline target hit (V3.1 floor 0.34); push 4-epoch
...
label="nlte3fix_femerge_bare_${tag}"
work_root="$ROOT/logs/nlte3fix_femerge_bare_${SLURM_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "=== NLTE3FIX-BARE [${label}] L=${L_inner} N_PKT=${N_PKT} ==="
echo "Host: $(hostname)  GPU: $(nvidia-smi --query-gpu=name --format=csv,noheader|head -1)"
echo "Binary: $BIN  Snifs: $snifs  HST tag: $hst_tag"
echo "Time: $(date)"
echo "** FEMERGE BARE — V3+CMFGEN iron-peak III bb. NO SCALE knobs, NO EPS_UV. Only SKIP_Z=14 (Si II solver). Pure unscaled line-shape floor on corrected atomic data. **"
...
print(f"  [BARE] Snifs [3300,8000]   rms_bn = {r:.4f}   chi_bn = {chi:.3f}")
    ...
    if gH > 0:
        for lo, hi, lbl in [(3000., 8000., "[3000,8000]"), (1700., 3000., "[1700,3000]")]:
            r, chi = rms_bn_nw(mlam, mflu, hl, hf, gH,
                               ...
                               wl_lo=lo, wl_hi=hi)
            print(f"  [BARE] HST   {lbl}  rms_bn = {r:.4f}   chi_bn = {chi:.3f}")
PYEOF
