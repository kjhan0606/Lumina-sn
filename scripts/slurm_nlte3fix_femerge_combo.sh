#!/bin/bash
#SBATCH --job-name=nlte3fix_combo
...
# #219f Phase 9: stack-compound test. SCALE2 Ni III + SCALE7/8/9 all parametric.
#
# Args: $1 = NI2_OPT_FAC  (SCALE7 Ni II opt, default 0.05)
#       $2 = CA2_FAC      (SCALE8 Ca II,     default 3.0)
#       $3 = S9_FAC       (SCALE9 Ni II NIR, default 0.05)
#       $4 = NI3_FAC      (SCALE2 Ni III UV [<4000], default 0.0=off)
...
ni2opt_tag=$(echo "$NI2_OPT_FAC" | tr '.' 'p')
ca2_tag=$(echo "$CA2_FAC" | tr '.' 'p')
s9_tag=$(echo "$S9_FAC" | tr '.' 'p')
ni3_tag=$(echo "$NI3_FAC" | tr '.' 'p')
label="nlte3fix_combo_n2o${ni2opt_tag}_ca${ca2_tag}_s9${s9_tag}_n3${ni3_tag}_${tag}"
...
echo "=== NLTE3FIX-COMBO NI2_OPT=$NI2_OPT_FAC CA2=$CA2_FAC S9=$S9_FAC NI3=$NI3_FAC | NI=$NI_UV_FAC FE=$FE_FAC CO=$CO_FAC [${label}] ==="
...
print(f"  [COMBO n2o=$NI2_OPT_FAC ca=$CA2_FAC s9=$S9_FAC n3=$NI3_FAC] Snifs [3300,8000]   rms_bn = {r:.4f}   chi_bn = {chi:.3f}")
    ...
    if gH > 0:
        for lo, hi, lbl in [(3000., 8000., "[3000,8000]"), (1700., 3000., "[1700,3000]")]:
            r, chi = rms_bn_nw(mlam, mflu, hl, hf, gH,
                               ...
                               wl_lo=lo, wl_hi=hi)
            print(f"  [COMBO n2o=$NI2_OPT_FAC ca=$CA2_FAC s9=$S9_FAC n3=$NI3_FAC] HST   {lbl}  rms_bn = {r:.4f}   chi_bn = {chi:.3f}")
PYEOF
