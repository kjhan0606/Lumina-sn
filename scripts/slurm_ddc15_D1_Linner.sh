#!/bin/bash
#SBATCH --job-name=ddc15_D1_Linner
...
# D1: L_inner sweep on C2-base (DDC15 v_in=10400, X_Fe_outer=0.05).
# Tests cascade-reddening-independent hypothesis: red continuum is set by
# inner photosphere thermal emission, not UV→opt cascade.
# Expectation: red-tot ratio (currently ~1.39) drops monotonically with L_inner.
# Risk: low L_inner → green band drops too, structure mismatch.
...
L=${L_VALS[$SLURM_ARRAY_TASK_ID]}
vi=10400
ro=0.00
de=9.0
xfe_outer=0.05
label=$(printf "ddc15D1_L%s" "$L")

work_root="$ROOT/logs/ddc15D1_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
src = "$SRC_REF"
dst = "$REF_DIR"
ro, de, vi_kms, X_FE_OUTER, L_OVERRIDE = $ro, $de, $vi, $xfe_outer, $L

with open(f"{src}/config.json") as f: cfg = json.load(f)
t_exp = cfg["time_explosion_s"]
n_shells = cfg["n_shells"]
# Override L_inner (D1 lever)
L_inner = float(L_OVERRIDE)
cfg["luminosity_inner_erg_s"] = L_inner
cfg["luminosity_requested_erg_s"] = L_inner
...
# C2-base profile: DDC15 + outer X_Fe cap at 0.05 (champion from C2)
...
print(f"  D1 built: L_inner={L_inner:.2e}  T_inner={T_inner:.0f}K  v_in={vi_kms}  X_Fe_out={X_FE_OUTER}")
PYEOF

echo "=== DDC15 D1 [${label}]  L=${L} erg/s  v_in=${vi}  X_Fe_out=${xfe_outer}  N_PKT=${N_PKT} ==="
