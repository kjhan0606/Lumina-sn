#!/bin/bash
#SBATCH --job-name=w5_stack_closer
...
# W5: Push from W4 champ L145niF010 (HST 0.1717) toward Tier-4 (0.164).
#   0 — repro            L=1.45 Ni II f=0.10 [7500,10000]   (W4 champ MC bound)
#   1 — niCoF010         L=1.45 Ni+Co II f=0.10 [7500,10000] (add Co stack)
#   2 — niCoF005         L=1.45 Ni+Co II f=0.05 [7500,10000] (push f)
#   3 — niCoF010b6500    L=1.45 Ni+Co II f=0.10 [6500,10000] (wider band)
#   4 — niF005           L=1.45 Ni II f=0.05 [7500,10000]   (push f Ni only)
#   5 — niF010eps05      L=1.45 Ni II f=0.10 [7500,10000] EPS_UV=0.5
#   6 — L150niF010       L=1.50 Ni II f=0.10 [7500,10000]   (push L)
#   7 — niCoFeF010       L=1.45 Ni+Co+Fe II f=0.10 [7500,10000] (Fe II in NIR mask)
...
# 0:repro 1:niCoF010 2:niCoF005 3:niCoF010b6500 4:niF005 5:niF010eps05 6:L150niF010 7:niCoFeF010
L_SCALE_ARR=(1.45 1.45 1.45 1.45 1.45 1.45 1.50 1.45)
S9_FAC_ARR=(0.10 0.10 0.05 0.10 0.05 0.10 0.10 0.10)
S9_LMIN_ARR=(7500 7500 7500 6500 7500 7500 7500 7500)
S9_Z_ARR=("28" "27,28" "27,28" "27,28" "28" "28" "28" "26,27,28")
EPS_ARR=(0.70 0.70 0.70 0.70 0.70 0.50 0.70 0.70)
TAG_ARR=(repro niCoF010 niCoF005 niCoF010b6500 niF005 niF010eps05 L150niF010 niCoFeF010)
...
EPS_UV=${EPS_ARR[$SLURM_ARRAY_TASK_ID]}
tag=${TAG_ARR[$SLURM_ARRAY_TASK_ID]}

L_inner=$(python3 -c "print(${L_BASE} * ${L_SCALE})")

label="w5_${tag}"
work_root="$ROOT/logs/w5_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "=== W5 [${label}] L=${L_SCALE} EPS=${EPS_UV} SCALE9: Z=${S9_Z} ion=1 f=${S9_FAC} band=[${S9_LMIN},10000] ==="
...
echo ""
echo "=== score [${label}] L=${L_SCALE} Z=${S9_Z} f=${S9_FAC} band=[${S9_LMIN},10000] EPS=${EPS_UV} ==="
python3 <<PYEOF
...
print(f"  W5 ${tag}  Snifs RMS_bn [3300,8000]  = {r_snifs:.4f}")
...
if parts:
    ...
    if gH > 0:
        ...
        print(f"  W5 ${tag}  HST RMS_bn [3000,8000]   = {r_hst_can:.4f}")
        print(f"  W5 ${tag}  HST RMS_bn [1700,3000] = {r_hst_uv:.4f}")
