#!/bin/bash
#SBATCH --job-name=a10_kx_milnec2ne64
...
#
# Binary: lumina_cuda.withMilneC2 (= kpr10b source + the TE_POP gate).
...
# --- [PARALLEL] own-binary via LUMINA_BIN (no ./lumina_cuda cp -> no Text-file-busy contention) ---
export LUMINA_BIN=lumina_cuda.withMilneC2_ne64
[ -x "$LUMINA_BIN" ] || { echo "FATAL: $LUMINA_BIN missing/not built"; exit 2; }
echo "[parallel] using $LUMINA_BIN directly (run_coevolve honors LUMINA_BIN)"
