...
#
# Binary: lumina_cuda.withTepop (= kpr10b source + the TE_POP gate).
...
# --- install the withTepop binary as ./lumina_cuda for run_coevolve_s01.sh ------
...
TEPOP_BIN=lumina_cuda.withTepop
...
# --- FBCKT SINGLE-VARIABLE DELTA vs fbcb (secondary amplitude lever; step 3 of 3->1) ---
#     C_fb weight (h*nu0 + kTe) -> kTe only (photoelectron KE, ARTIS/CMFGEN cooling ledger).
#     Cuts the inflated p_fb~0.99 AND re-weights the fb edge CDF flatter (less blue-biased).
#     plasma.c:2388-2389 g_fb_cool_kt; feeds both C_fb_real (p_fb) and e_w (edge CDF).
