#!/bin/bash
#SBATCH --job-name=ddc15_P2_FeCo_stack
...
# P2: Stacked Fe+Co A_ul push around P1 winner (Fe0.5/Co0.3).
# P1 finding (job 156163, intra-job vs champ_repl Fe0.5/Co0.5):
#   Fe0.3/Co0.5  Δ −0.0099
#   Fe0.2/Co0.5  Δ −0.0048 (saturated)
#   Fe0.5/Co0.3  Δ −0.0164 (best)
# P2 stacks both axes; pushes Co axis further to f=0.2.
# Binary: lumina_cuda_h100_aulscale5 (Ni+Si+Fe+Co suffixes).
#
#   array 0 — Co0.3 baseline (Fe 0.5, Co 0.3)        ← intra-job anchor
#   array 1 — stack: Fe 0.3, Co 0.3
#   array 2 — stack: Fe 0.2, Co 0.3
#   array 3 — Co axis: Fe 0.5, Co 0.2
...
LABELS=(Co0p3_repl Fe0p3Co0p3 Fe0p2Co0p3 Co0p2)
tag=${LABELS[$SLURM_ARRAY_TASK_ID]}

FE_FAC=0.5
CO_FAC=0.3
case $SLURM_ARRAY_TASK_ID in
    1) FE_FAC=0.3; CO_FAC=0.3 ;;
    2) FE_FAC=0.2; CO_FAC=0.3 ;;
    3) FE_FAC=0.5; CO_FAC=0.2 ;;
esac
...
label="ddc15P2_${tag}"
work_root="$ROOT/logs/ddc15P2_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "=== P2 [${label}]  Ni(0.3) + Si(0.05) + Fe(${FE_FAC}) + Co(${CO_FAC})  λ<4000Å  N_PKT=${N_PKT}  N_ITER=${N_ITER} ==="
