#!/bin/bash
#SBATCH --job-name=paper_ddc15_v3asEfloor_aul_2002bo
...
# #295 (β) Atomic data A_ul probe — per-line A_ul scale on paper-DDC15 v3asEfloor.
# After #292/#293 (NLTE NULL) and #294 (abundance NULL), test the remaining structural lever:
# per-line A_ul magnitude. Each cell targets one NIR residual carrier via LUMINA_AUL_SCALE.
#   cell 0: Mg II 9218 doublet A_ul × 0.3   (damp 2.00× over-emission carrier)
#   cell 1: [Ni II] 7378 A_ul × 0.3         (damp 1.85× forbidden over-emission)
#   cell 2: O I 7774 A_ul × 3.0             (boost 1.57× absorption-deficit carrier)
# Knob masks line A_ul/B_lu/B_ul for (Z, ion, λ-band) selection.

AUL_Z_LIST=(    "12" "28" "8")
AUL_ION_LIST=(  "1"  "1"  "0")
AUL_FAC_LIST=(  "0.3" "0.3" "3.0")
AUL_LMIN_LIST=( "9100" "7300" "7700")
AUL_LMAX_LIST=( "9300" "7450" "7850")
AUL_NAME_LIST=( "MgII9218" "NiII7378" "OI7774")

export AUL_Z="${AUL_Z_LIST[$SLURM_ARRAY_TASK_ID]}"
export AUL_ION="${AUL_ION_LIST[$SLURM_ARRAY_TASK_ID]}"
export AUL_FAC="${AUL_FAC_LIST[$SLURM_ARRAY_TASK_ID]}"
export AUL_LMIN="${AUL_LMIN_LIST[$SLURM_ARRAY_TASK_ID]}"
export AUL_LMAX="${AUL_LMAX_LIST[$SLURM_ARRAY_TASK_ID]}"
AUL_NAME="${AUL_NAME_LIST[$SLURM_ARRAY_TASK_ID]}"
AUL_TAG="${AUL_NAME}_f${AUL_FAC//./p}"
...
label="paperDDC15v3asEfloorAUL_${AUL_TAG}_2002bo_vi${vi_tag}_L${l_tag}_${RUN_TAG}"
...
echo "=== PAPER-DDC15-v3 AUL $AUL_NAME (Z=$AUL_Z ion=$AUL_ION λ∈[$AUL_LMIN,$AUL_LMAX]) ×$AUL_FAC  vs SN 2002bo  VI=$VI_KMS L=$L_scale  RUN=$RUN_TAG [${label}] ==="
...
env LUMINA_BF_OPACITY=1 \
    ...
    LUMINA_AUL_SCALE_FACTOR="$AUL_FAC" \
    LUMINA_AUL_SCALE_ZMASK="$AUL_Z" \
    LUMINA_AUL_SCALE_IONMASK="$AUL_ION" \
    LUMINA_AUL_SCALE_LAMBDA_MIN="$AUL_LMIN" \
    LUMINA_AUL_SCALE_LAMBDA_MAX="$AUL_LMAX" \
