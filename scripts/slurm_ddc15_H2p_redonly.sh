#!/bin/bash
#SBATCH --job-name=ddc15_H2p_redonly
...
# H2p (Task #228 follow-up): H2 EPS_UV redonly production sweep.
#
# MC-truth re-ranking found H2 ε=0.7_redonly @ 200K = MC_RMS_bn 0.182 (best ever),
# but H1p (job 156038) at 800K×12 used h3 binary and tested ε={0.4, 0.9} but NOT
# ε=0.7. Need production validation of the actual best 200K cell.
#
# Same h3 binary as H1p for consistency. Also re-runs ε=0.0 control on h3.
#
# Array cells:
#   0 — ε=0.3 redonly
#   1 — ε=0.5 redonly
#   2 — ε=0.7 redonly  ← 200K MC champion (need 800K confirmation)
#   3 — ε=0.9 redonly  (re-run of H1p h2_eps0p9_redonly)
#   4 — ε=0.0          (control)
...
# Pick binary by partition (h3 was used by H1p production)
...
EPS_UVS=(0.3 0.5 0.7 0.9 0.0)
RED_ONLYS=(1 1 1 1 0)
LABELS=(eps0p3 eps0p5 eps0p7 eps0p9 ctrl0p0)
...
label="ddc15H2p_${tag}"
work_root="$ROOT/logs/ddc15H2p_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "=== H2p [${label}]  EPS_UV=${eps_uv} RED_ONLY=${red_only}  N_PKT=${N_PKT} N_ITER=${N_ITER} ==="
...
echo "  exit=$rc"
echo "--- convergence ---"
grep -E 'Mean \|W error|Mean \|T_rad error|T_inner final|L_emitted' stdout.log | tail -16
echo "Done: $(date)"
