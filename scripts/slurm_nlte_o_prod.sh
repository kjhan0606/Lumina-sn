...
# #273 NLTE O II/III PRODUCTION on FEMERGE Phase 9b best cell (157508 baseline):
#   vi=11600, L_scale=2.45, de=10.5, p+3.7d, V3.1 SCALE stack frozen.
# Cleared by smoke 157522 (exit=0; T_inner err -2.26%, W err 45.95%).
# Compare against #271 best 157508 Snifs 0.2194 to read out O II/III line-shape
# contribution (especially O I 7773 region). 800K x 12 iter (single seed).
...
de_tag=$(echo "$de" | tr '.' 'p')
label="nlteO_smoke_vi${vi_tag}_L${l_tag}_de${de_tag}_${tag}"
...
echo "=== NLTEO-SMOKE VI=$VI_KMS L=$L_scale DE=$DE_ARG | NI=$NI_UV_FAC FE=$FE_FAC CO=$CO_FAC NI2_OPT=$NI2_OPT_FAC CA2=$CA2_FAC S9=$S9_FAC [${label}] ==="
