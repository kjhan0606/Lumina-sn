#!/bin/bash
#SBATCH --job-name=ddc15_A1b_eps_dense
...
# A1b (dense ε): confirm A1 non-monotone result.
#
# A1 (job 156553): e0p80 local max (0.225) but e0p90 J=0.229 winner. Non-monotone
# needs verification. A1b samples {0.85, 0.88, 0.91, 0.94, 0.97} on X1_b3p0 base
# (SCALE7 Ni II opt f=0.05 + SCALE8 Ca II f=3.0).
#
# Array cells (5):
#   0 — e0p85  : EPS_UV=0.85
#   1 — e0p88  : 0.88
#   2 — e0p91  : 0.91  (near A1_e0p90 → MC reproducibility)
#   3 — e0p94  : 0.94
#   4 — e0p97  : 0.97
...
LABELS=(e0p85 e0p88 e0p91 e0p94 e0p97)
EPS_GRID=(0.85 0.88 0.91 0.94 0.97)
...
label="ddc15A1b_${tag}"
work_root="$ROOT/logs/ddc15A1b_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "=== A1b [${label}]  X1_b3p0 base + EPS_UV=${EPS_UV}  N_PKT=${N_PKT}  N_ITER=${N_ITER} ==="
