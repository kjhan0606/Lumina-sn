#!/usr/bin/env python3
"""A3: ρ-11fe + narrow Si IME shell [v_in, v_in+2500] + Fe05 — vs HST."""
...
JOB  = sys.argv[1] if len(sys.argv) > 1 else "155718"

VINS = [9800, 10100, 10400, 10800]
CELLS = [(vi, ROOT/f"logs/rho11feA3_{JOB}_rho11feNarrowSi_v{vi:05d}/lumina_spectrum_formal.csv")
         for vi in VINS]
OUT = ROOT/f"figures/rho11fe_A3_{JOB}_vs_hst.png"
C_KMS = 299792.458

PATH_A_REF = ROOT/"logs/rho11feA_155702_rho11fe_fullNLTE/lumina_spectrum_formal.csv"
...
pa_ref = load_and_scale(PATH_A_REF)
...
for (vi, (lam, flu)), c in zip(data.items(), cmap):
    ax.plot(lam, flu*1e14, color=c, lw=0.9, alpha=0.85, label=f'A3 narrowSi v_in={vi}')
if pa_ref is not None:
    ax.plot(pa_ref[0], pa_ref[1]*1e14, color='tab:red', lw=0.7, alpha=0.55, ls='--',
            label='Path A fullNLTE v10400 (champ)')
ax.set_xlim(1700, 9000)
ax.set_ylim(0, max(hflu[(hlam>2900)&(hlam<9000)])*1e14*1.15)
ax.set_xlabel('λ (Å)'); ax.set_ylabel('F_λ × 10¹⁴')
ax.set_title(f'A3: ρ-11fe + narrow Si IME [v_in, v_in+2500] + Fe05 (job {JOB})')
...
for i, ((vi, (lam, flu)), c) in enumerate(zip(data.items(), cmap)):
    ...
    if tm:
        ...
        ax.text(tm, ymax*(0.25 + i*0.1), f'v_in={vi}\nλ={tm:.0f}\nv={v_obs:.0f}',
...
if tm:
    ...
    ax.text(tm, ymax*0.10, f'HST {tm:.0f}\nv={v_obs:.0f}', color='k', fontsize=9,
            ha='center', va='bottom', fontweight='bold')
ax.set_xlabel('λ (Å)'); ax.set_ylabel('F_λ × 10¹⁴')
ax.set_title('Si II 6355 zoom — narrow IME shell tracks v_in?')
ax.legend(loc='upper right', fontsize=9); ax.grid(alpha=0.2)
...
if pa_ref is not None:
    ax.plot(pa_ref[0], pa_ref[1]*1e14, color='tab:red', lw=1.0, alpha=0.5, ls='--',
            label='Path A fullNLTE v10400')
...
ax.set_xlabel('λ (Å)'); ax.set_ylabel('F_λ × 10¹⁴')
ax.set_title('UV blanketing 2300-4000Å — A3 narrow IME vs Path A champion')
ax.legend(loc='upper right', fontsize=9); ax.grid(alpha=0.2)
...
print(f"\n=== Si II 6355 trough position (A3 narrow IME) ===")
tm = trough_min(hlam, hflu)
...
for vi, (lam, flu) in data.items():
    ...
    if tm:
        v = (1-tm/6355)*C_KMS
        print(f"{'A3 narrowSi v_in='+str(vi):40s}  λ={tm:.1f}  v={v:.0f} km/s  (HST diff {v-9934:+.0f})")
...
for vi, (lam, flu) in data.items():
    ...
    print(f"A3 narrowSi v_in={vi}                    RMS_bn = {rms:.4f}")

if pa_ref is not None:
    mlam, mflu = gauss_baseline(pa_ref[0], pa_ref[1])
    if mlam is not None:
        fm = interp1d(mlam, mflu, kind='linear', bounds_error=False, fill_value=np.nan)
        mflu_h = fm(hlam_n)
        diff = mflu_h - hflu_n
        rms = np.sqrt(np.nanmean(diff**2))
        print(f"Path A fullNLTE v10400 (champ)            RMS_bn = {rms:.4f}")
