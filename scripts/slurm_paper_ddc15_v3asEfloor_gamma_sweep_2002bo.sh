#!/bin/bash
#SBATCH --job-name=paper_ddc15_v3asEfloor_gamma_2002bo
...
# #297 (γ) Outer-shell T_e damper sweep on paper-DDC15 v3asEfloor.
# After #292/#293 (NLTE NULL), #294 (abundance NULL), #295 (A_ul magnitude NULL),
# #296 (atomic data correct within 4% NIST), the LAST remaining structural lever
# is per-shell T_e in the outer line-forming region (s >= 18, v >= 22kkm/s).
# LUMINA_OUTER_TE_DAMP_FACTOR multiplies T_e in shells s >= SMIN, with a 1000 K
# hard floor. Hypothesis: cooler outer T_e reduces Mg II 9218 cascade emission,
# deepens O I 7774 absorption (more neutral O via Saha), reduces Ni II permitted
# forest at 7378. Knob is post-multiplier in compute_electron_temperature().
#   cell 0: factor=0.85 SMIN=18   (mild outer cool — 15% T_e reduction)
#   cell 1: factor=0.70 SMIN=18   (moderate — 30%)
#   cell 2: factor=0.55 SMIN=18   (aggressive — 45%)

OUTER_TE_DAMP_FACTOR_LIST=( "0.85" "0.70" "0.55" )
OUTER_TE_DAMP_SMIN_LIST=(   "18"   "18"   "18"   )

export OUTER_TE_DAMP_FACTOR="${OUTER_TE_DAMP_FACTOR_LIST[$SLURM_ARRAY_TASK_ID]}"
export OUTER_TE_DAMP_SMIN="${OUTER_TE_DAMP_SMIN_LIST[$SLURM_ARRAY_TASK_ID]}"
GAMMA_TAG="f${OUTER_TE_DAMP_FACTOR//./p}_s${OUTER_TE_DAMP_SMIN}"
...
if echo "$GPU_NAME" | grep -qi H100; then
    BIN="$ROOT/lumina_cuda_oTri_h100_floor_gamma"
elif echo "$GPU_NAME" | grep -qi H200; then
    BIN="$ROOT/lumina_cuda_oTri_h100_floor_gamma"
else
    BIN="$ROOT/lumina_cuda_oTri_a100_floor_gamma"
fi
...
label="paperDDC15v3asEfloorGAMMA_${GAMMA_TAG}_2002bo_vi${vi_tag}_L${l_tag}_${RUN_TAG}"
...
echo "=== PAPER-DDC15-v3 GAMMA outer-T_e damper (FACTOR=$OUTER_TE_DAMP_FACTOR SMIN=$OUTER_TE_DAMP_SMIN)  vs SN 2002bo  VI=$VI_KMS L=$L_scale  RUN=$RUN_TAG [${label}] ==="
...
env LUMINA_BF_OPACITY=1 \
    ...
    LUMINA_OUTER_TE_DAMP_FACTOR="$OUTER_TE_DAMP_FACTOR" \
    LUMINA_OUTER_TE_DAMP_SMIN="$OUTER_TE_DAMP_SMIN" \
...
echo ""
echo "--- GAMMA damper diagnostic ---"
grep -E "\[GAMMA\]" stdout.log | head -3
