#!/bin/bash
#SBATCH --job-name=ddc15_N1_KLM_stack
...
# N1: Triple stack K1 + L1 + M1 — uses LUMINA_AUL_SCALE_* (Ni II),
# LUMINA_AUL_SCALE2_* (Si II), LUMINA_AUL_SCALE3_* (Fe II).
# Audit (audit_baseline_metric.py) showed at FWHM=20k:
#   KL1 stack vs HST  = -0.0227  (consistent vs TARDIS = -0.0196)
#   M1 FeII f=0.5 vs HST = -0.0267 (weaker vs TARDIS = -0.0128)
# Triple test: do M1 + KL1 deltas stack additively? Or is M1 a code-vs-data
# artefact (helps HST only)? Compare ΔRMS vs TARDIS for both reference codes.
#   array 0 — ctrl (all f=1.0) — fresh seed for clean MC baseline
#   array 1 — K1 only replicate (Ni II f=0.3)  — to bound MC noise
#   array 2 — KL1 stack replicate (Ni f=0.3 + Si f=0.05)
#   array 3 — N1 TRIPLE (Ni f=0.3 + Si f=0.05 + Fe f=0.5)
...
BIN="$ROOT/lumina_cuda_h100_aulscale2"

N_PKT=800000
N_ITER=12

LABELS=(ctrl K1_repl KL1_repl N1_triple)
...
case $SLURM_ARRAY_TASK_ID in
    1) NI_FAC=0.3 ;;
    2) NI_FAC=0.3; SI_FAC=0.05 ;;
    3) NI_FAC=0.3; SI_FAC=0.05; FE_FAC=0.5 ;;
esac
...
label="ddc15N1_${tag}"
work_root="$ROOT/logs/ddc15N1_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "=== N1 KLM triple [${label}]  NI_FAC=${NI_FAC}  SI_FAC=${SI_FAC}  FE_FAC=${FE_FAC}  λ<4000Å  N_PKT=${N_PKT}  N_ITER=${N_ITER} ==="
echo "Host: $(hostname)  GPU: $(nvidia-smi --query-gpu=name --format=csv,noheader|head -1)"
echo "Time: $(date)"

# Primary rule: Ni II (Z=28, ion=1)
# Secondary rule (suffix=2): Si II (Z=14, ion=1)
# Tertiary rule (suffix=3): Fe II (Z=26, ion=1)
LUMINA_BF_OPACITY=1 \
LUMINA_CMFGEN_SIGMA_BF="$ROOT/data/atomic/cmfgen_sigma_bf.bin" \
LUMINA_DYNAMIC_TRANSPROB=1 \
LUMINA_NLTE_START_ITER=5 \
LUMINA_NLTE_SKIP_Z=14 \
LUMINA_AUL_SCALE_FACTOR="$NI_FAC" \
LUMINA_AUL_SCALE_LAMBDA_MAX=4000 \
LUMINA_AUL_SCALE_ZMASK=28 \
LUMINA_AUL_SCALE_IONMASK=1 \
LUMINA_AUL_SCALE2_FACTOR="$SI_FAC" \
