#!/usr/bin/env python3
"""A2: ρ-11fe + v_in fine bracket + X_Fe inner cap (0.05) — Si II + UV scan vs HST."""
...
JOB  = sys.argv[1] if len(sys.argv) > 1 else "155709"

VINS = [9550, 9650, 9700, 9750]
CELLS = [(vi, ROOT/f"logs/rho11feA2_{JOB}_rho11feFe05_v{vi:05d}/lumina_spectrum_formal.csv")
         for vi in VINS]
OUT = ROOT/f"figures/rho11fe_A2_{JOB}_vs_hst.png"
C_KMS = 299792.458
...
# A1 reference for comparison
a1_path = ROOT/"logs/rho11feA1_155704_rho11fe_v09500/lumina_spectrum_formal.csv"
a1_ref = load_and_scale(a1_path)
...
for (vi, (lam, flu)), c in zip(data.items(), cmap):
    ax.plot(lam, flu*1e14, color=c, lw=0.9, alpha=0.85, label=f'A2 ρ-11fe+Fe05 v_in={vi}')
if a1_ref is not None:
    ax.plot(a1_ref[0], a1_ref[1]*1e14, color='tab:red', lw=0.7, alpha=0.55, ls='--',
            label='A1 ρ-11fe Fe10 v_in=9500')
ax.set_xlim(1700, 9000)
ax.set_ylim(0, max(hflu[(hlam>2900)&(hlam<9000)])*1e14*1.15)
ax.set_xlabel('λ (Å)'); ax.set_ylabel('F_λ × 10¹⁴')
ax.set_title(f'A2: ρ-11fe + v_in fine bracket + X_Fe_inner=0.05 (job {JOB})')
...
for i, ((vi, (lam, flu)), c) in enumerate(zip(data.items(), cmap)):
    ...
    if tm:
        ...
        ax.text(tm, ymax*(0.3 + i*0.1), f'v_in={vi}\nλ={tm:.0f}\nv={v_obs:.0f}',
...
if tm:
    ...
    ax.text(tm, ymax*0.15, f'HST {tm:.0f}\nv={v_obs:.0f}', color='k', fontsize=9,
            ha='center', va='bottom', fontweight='bold')
ax.set_xlabel('λ (Å)'); ax.set_ylabel('F_λ × 10¹⁴')
ax.set_title('Si II 6355 zoom — A2 v_in {9550,9650,9700,9750} + Fe05')
ax.legend(loc='upper right', fontsize=9); ax.grid(alpha=0.2)

ax = axes[2]
ax.plot(hlam, hflu*1e14, 'k-', lw=2.0, label='HST', alpha=0.95)
for (vi, (lam, flu)), c in zip(data.items(), cmap):
    ax.plot(lam, flu*1e14, color=c, lw=1.3, alpha=0.9, label=f'v_in={vi}')
if a1_ref is not None:
    ax.plot(a1_ref[0], a1_ref[1]*1e14, color='tab:red', lw=1.0, alpha=0.5, ls='--',
            label='A1 v_in=9500 Fe10')
...
ax.set_xlabel('λ (Å)'); ax.set_ylabel('F_λ × 10¹⁴')
ax.set_title('UV blanketing 2300-4000Å — A2 Fe05 vs A1 Fe10 reference')
ax.legend(loc='upper right', fontsize=9); ax.grid(alpha=0.2)
...
print(f"\n=== Si II 6355 trough position (A2 fine bracket) ===")
tm = trough_min(hlam, hflu)
...
for vi, (lam, flu) in data.items():
    ...
    if tm:
        v = (1-tm/6355)*C_KMS
        print(f"{'A2 ρ-11fe+Fe05 v_in='+str(vi):40s}  λ={tm:.1f}  v={v:.0f} km/s  (HST diff {v-9934:+.0f})")
...
for vi, (lam, flu) in data.items():
    ...
    print(f"A2 ρ-11fe+Fe05 v_in={vi}              RMS_bn = {rms:.4f}")

# A1 reference RMS for direct comparison
if a1_ref is not None:
    mlam, mflu = gauss_baseline(a1_ref[0], a1_ref[1])
    if mlam is not None:
        fm = interp1d(mlam, mflu, kind='linear', bounds_error=False, fill_value=np.nan)
        mflu_h = fm(hlam_n)
        diff = mflu_h - hflu_n
        rms = np.sqrt(np.nanmean(diff**2))
        print(f"A1 ρ-11fe Fe10 v_in=9500 (ref)            RMS_bn = {rms:.4f}")
