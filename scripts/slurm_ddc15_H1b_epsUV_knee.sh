#!/bin/bash
#SBATCH --job-name=ddc15_H1b_epsUV_knee
...
# H1b: Dense LUMINA_EPS_UV knee sweep on C2 champion. H1 (job 156014) found
# monotone↓ red/HST 1.384→1.255 with RMS_bn 0.291→0.202 over EPS_UV {0,0.3,0.6,0.9}
# but UV+blue collapses 0.92→0.78 at the high end. This sweep maps the trade-off:
# dense at the 0.1-0.3 transition where UV+blue is still ≥0.85, anchors at 0.5/0.7.
#
#   array 0 — 0.10        | array 4 — 0.30 (H1 boundary, replicate)
#   array 1 — 0.15        | array 5 — 0.40
#   array 2 — 0.20        | array 6 — 0.50
#   array 3 — 0.25        | array 7 — 0.70
...
EPS_UVS=(0.10 0.15 0.20 0.25 0.30 0.40 0.50 0.70)
...
label=$(printf "ddc15H1b_epsUV%s" "$eps_uv")
work_root="$ROOT/logs/ddc15H1b_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "=== H1b epsUV knee [${label}]  EPS_UV=${eps_uv}  C2-base  N_PKT=${N_PKT} ==="
