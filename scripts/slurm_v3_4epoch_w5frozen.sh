...
# V3: W5 niF005 frozen stack across 4 HST epochs × {Pereira-L, L×1.45}.
# W5 niF005 = X1_b3p0(SCALE1-4,6-8) + SCALE9(Ni II f=0.05 [7500,10000])
# SCALE5 DROPPED (W2 finding: counterproductive).
#
# 8 cells = 4 epochs × 2 L_scale:
#   idx 0-3: L_scale=1.00 (honest Pereira+2013 LC)
#   idx 4-7: L_scale=1.45 (T_inner-matched at p+3.7d)
...
L_SCALE=(1.00 1.00 1.00 1.00 1.45 1.45 1.45 1.45)
...
SUFFIX=(pereira pereira pereira pereira hot1p45 hot1p45 hot1p45 hot1p45)
...
label="v3_${tag}_${suffix}"
work_root="$ROOT/logs/v3_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "=== V3 [${label}] L_base=${L_base} L_scale=${L_scale} L=${L_inner} N_PKT=${N_PKT} ==="
...
print(f"  V3 ${tag} ${suffix}  Snifs RMS_bn [3300,8000]  = {r_snifs:.4f}")
...
if parts:
    ...
    if gH > 0:
        ...
        print(f"  V3 ${tag} ${suffix}  HST RMS_bn [3000,8000]  = {r_hst_can:.4f}")
        print(f"  V3 ${tag} ${suffix}  HST RMS_bn [1700,3000]= {r_hst_uv:.4f}  (UV blanketing)")
        # SED band ratios
        def br(lo, hi):
        ...
        ref_m, ref_h = bm["Ref"]
        print(f"  V3 ${tag} ${suffix}  --- SED band ratios (LUMINA/HST, normed to Ref) ---")
        for (n,lo,hi) in bands:
            mm, hh = bm[n]
            if ref_m>0 and ref_h>0 and hh>0:
                r = (mm/ref_m)/(hh/ref_h)
                print(f"  V3 ${tag} ${suffix}    band {n:<6} [{lo:5d},{hi:5d}]  L/HST = {r:.3f}")
PYEOF
