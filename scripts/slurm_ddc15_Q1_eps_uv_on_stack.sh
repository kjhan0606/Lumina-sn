#!/bin/bash
#SBATCH --job-name=ddc15_Q1_eps_on_stack
...
# Q1: cascade-reddening attack via EPS_UV on top of P2 A_ul stack winner
# (Ni 0.3 + Si 0.05 + Fe 0.3 + Co 0.3, all λ<4000Å, ion=1).
# A_ul family closed at floor 0.2384 (P2 #213); gap to ≤0.20 = +0.038.
# Cascade-reddening was H1/H2 200K champion (RMS 0.202) but vaporized at
# 800K on plain base (#200, 0.288). Hypothesis: A_ul stack pre-removes UV
# blanketing so EPS_UV no longer triggers cooling-feedback collapse.
# Binary: lumina_cuda_h100_aulscale5 (same as P2; EPS_UV + RED_ONLY env knobs
# are in lumina_cuda.cu at lines 2192-2227).
#
#   array 0 — Q1_ref:    A_ul stack only (replicates P2 Fe0.3+Co0.3 0.2384)
#   array 1 — Q1_eps04:  + LUMINA_EPS_UV=0.4   (H1b knee, broad)
#   array 2 — Q1_eps04r: + LUMINA_EPS_UV=0.4 + RED_ONLY=1 (H2-style)
#   array 3 — Q1_eps06r: + LUMINA_EPS_UV=0.6 + RED_ONLY=1 (stronger red-only)
...
BIN="$ROOT/lumina_cuda_h100_aulscale5"

N_PKT=800000
N_ITER=12

LABELS=(Q1_ref Q1_eps04 Q1_eps04r Q1_eps06r)
...
case $SLURM_ARRAY_TASK_ID in
    1) EPS_UV=0.4 ;;
    2) EPS_UV=0.4; EPS_RED_ONLY=1 ;;
    3) EPS_UV=0.6; EPS_RED_ONLY=1 ;;
esac
...
label="ddc15Q1_${tag}"
work_root="$ROOT/logs/ddc15Q1_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "=== Q1 [${label}]  stack(Ni0.3/Si0.05/Fe${FE_FAC}/Co${CO_FAC})  EPS_UV=${EPS_UV}  RED_ONLY=${EPS_RED_ONLY}  N_PKT=${N_PKT}  N_ITER=${N_ITER} ==="
...
LUMINA_EPS_UV_RED_ONLY="$EPS_RED_ONLY" \
