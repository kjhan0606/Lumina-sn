...
# CROSS-GPU DETERMINISM PROBE (h100 vs H200 baseline parity44). NOT a physics run.
# PREREGISTERED: exact replica of 51_parity44_droff env + binary withParityW on a
# h100 GPU. A) all physical CSVs byte-identical to logs/coevolve_consume_parity44
#   -> h100 OPENS for judgment runs. B) any diff -> quantify (max rel dev), h100
#   STAYS CLOSED, deviation registered quietly (B9 instrument debt). No tuning.
set -u
R=/home/kjhan/BACKUP/Eunha.A1/Claude/Lumina-sn
export LUMINA_RUN_ROOT=/gpfs/kjhan/lumina_probe_h100
cd "$LUMINA_RUN_ROOT"
...
export PKTS=100000 NITER=12 P0TAG=parity44h100probe
...
mkdir -p "$R/logs/coevolve_consume_parity44h100probe" && rsync -a "$OUT/" "$R/logs/coevolve_consume_parity44h100probe/"
echo "DONE probe h100"
