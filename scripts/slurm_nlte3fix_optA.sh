...
# #219e2 OptionA: probe spectral impact of iron-peak III NLTE bug.
# Setup identical to job 157277 (V3.1 p+3.7d champion, vi=10600 L=1.55 cleanz,
# seed 23111963) EXCEPT LUMINA_NLTE_SKIP_Z=14,24,26,27 instead of just 14.
#
# Mechanism: SKIP_Z makes nlte_update_tau_sobolev() bypass NLTE-corrected tau
# for those Z's lines (Cr/Fe/Co II AND III bb tau revert to nebular Saha-LTE).
# This neutralizes the iron-peak III top-level pop singularity bug AT the
# opacity output, while leaving NLTE solver state intact upstream.
#
# Comparison: 157277 (V3.1 baseline w/ SKIP_Z=14 only) at same seed.
...
label="nlte3fix_optA_${tag}"
work_root="$ROOT/logs/nlte3fix_optA_${SLURM_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "Time: $(date)"
echo "** SKIP_Z=14,24,26,27 — Cr/Fe/Co II+III bb tau → nebular (bypass NLTE bug) **"

LUMINA_BF_OPACITY=1 \
LUMINA_CMFGEN_SIGMA_BF="$ROOT/data/atomic/cmfgen_sigma_bf.bin" \
LUMINA_DYNAMIC_TRANSPROB=1 \
LUMINA_NLTE_SKIP_Z=14,24,26,27 \
...
print(f"  [OptA] Snifs [3300,8000]   rms_bn = {r:.4f}   chi_bn = {chi:.3f}")
    ...
    if gH > 0:
        for lo, hi, lbl in [(3000., 8000., "[3000,8000]"), (1700., 3000., "[1700,3000]")]:
            r, chi = rms_bn_nw(mlam, mflu, hl, hf, gH,
                               ...
                               wl_lo=lo, wl_hi=hi)
            print(f"  [OptA] HST   {lbl}  rms_bn = {r:.4f}   chi_bn = {chi:.3f}")
PYEOF
