...
# DISCRIMINATING TEST (no code change) for the 6-agent root-cause verdict.
# Root cause (project_macroatom_transprob_J_pump_root.md): the macroatom
# over-redshift (blue 0.062, NIR 2.34) is driven by the UNCAPPED internal-up
# rate B_lu*J fed by the raw UV-inflated MC J(nu) histogram. The dynamic
# transprob recompute is SECRETLY ON in every "DYNAMIC_TRANSPROB=0" run (env
# existence-check bug cuda.cu:2513), and j_cap_factor defaults to 0 (uncapped),
# so p_iup ~ 0.9 -> 1250-deep cascade -> red pile-up.
#
# THIS RUN: identical to fixtrad ARM B (macroatom 162323) EXCEPT it sets
# LUMINA_J_CAP_FACTOR>0, which caps J_line to j_cap*W*B_nu(T_rad) inside
# compute_transition_probabilities (plasma.c:1337-1340). Everything else held:
# same frozen thermal structure, same binary, same packets/iters.
#
# PRE-REGISTERED: if the J-pump is the driver, capping J_line collapses p_iup,
# shortens the cascade, and blue rises toward 1 / NIR drops back toward ~1.
# If blue STILL crushes with J capped, the J-pump is NOT the driver and the
# defect is in the static 1/N branching table itself.
#
#   sbatch --export=ALL,JCAP=1.0 scripts/slurm_jcap_test.sh
#   sbatch --export=ALL,JCAP=0.5 scripts/slurm_jcap_test.sh   # tighter cap

JCAP=${JCAP:-0}          # J-cap OFF: isolate the internal cascade-depth effect alone
MACAP=${MACAP:-20}       # internal cascade cap (5000 baseline -> 20 forces early deactivation)
LINE_INT=macroatom
DIFFUSE_BC=1
BINNEDJ=1
...
echo "=== MA-INTERNAL-CAP discriminating test  MACAP=$MACAP  JCAP=$JCAP  LINE_INT=$LINE_INT  FIXED_PROFILE=$FIXED_PROFILE  DIFFUSE_BC=$DIFFUSE_BC  VI=$VI_KMS L=$L_SCALE_ARG  N_PKT=$N_PKT N_ITER=$N_ITER ==="
echo "Host: $(hostname)  GPU: $GPU_NAME"
echo "Binary: $BIN  Time: $(date)"
echo "Identical to fixtrad ARM B (162323) EXCEPT J_CAP_FACTOR=$JCAP caps J_line to j_cap*W*B_nu(T_rad) in the macroatom internal-up rate. Tests whether the uncapped B_lu*J up-pump is the over-redshift driver."

env LUMINA_BF_OPACITY=1 \
    LUMINA_CMFGEN_SIGMA_BF=$SRC_REF/cmfgen_sigma_bf.bin \
    LUMINA_DYNAMIC_TRANSPROB=1 \
    LUMINA_J_CAP_FACTOR=$JCAP \
    LUMINA_MA_INTERNAL_CAP=$MACAP \
...
echo "  exit=$rc"
echo ""
echo "--- transprob mode (MUST say DYNAMIC) + J-cap activation ---"
grep -E "Transition probs:|\[TransProb\]|j_cap" stdout.log | head -20
echo ""
...
grep -E "\[CAP\]|truncat|MA_INTERNAL_CAP" stdout.log | head -20
echo ""
