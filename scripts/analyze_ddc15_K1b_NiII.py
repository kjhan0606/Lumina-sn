#!/usr/bin/env python3
"""K1b: Ni II A_ul stronger factor sweep {0.3, 0.2, 0.1, 0.05} (job 156081).
Same metrics as K1 (UV/HST, red/HST, cost, band-log10-rms, RMS_bn).
f0p3_rep is K1 winner replication / control reference."""
...
JOB  = sys.argv[1] if len(sys.argv) > 1 else "156081"

CELLS = [
    ("NiII_f0p3_rep", "factor=0.30 (K1 winner repl)"),
    ("NiII_f0p2",     "factor=0.20 (strong)"),
    ("NiII_f0p1",     "factor=0.10 (very strong)"),
    ("NiII_f0p05",    "factor=0.05 (extreme)"),
]
C_KMS = 299792.458
...
print(f"\n=== K1b Ni II A_ul stronger sweep (job {JOB}, 800K × 12 iter) ===")
print(f"  K1 result: f0p3 was best, non-monotone (0.7/0.5 worse). Push extreme.\n")
print(f"  {'cell':<30}  {'UV/HST':<7}  {'red/HST':<8}  {'cost':<7}  {'band-log10':<11}  {'RMS_bn':<8}")
print("-"*92)

rows = []
for label, desc in CELLS:
    p = ROOT/f"logs/ddc15K1b_{JOB}_ddc15K1b_{label}/lumina_spectrum_formal.csv"
    d = scale(p, hlam, hflu)
    if d is None:
        print(f"  {desc:<30}  MISSING ({p.parent.name})")
    ...
    print(f"  {desc:<30}  {uv:6.3f}   {rt:7.3f}  {cost:6.3f}   {blrms:8.4f}     {bnrms:6.4f}")
...
if valid:
    ...
    # vs f0p3_rep (K1 winner replication, our local control)
    if "NiII_f0p3_rep" in dict(rows):
        ctrl = dict(rows)["NiII_f0p3_rep"]
        print(f"\n  --- vs f0p3_rep (K1 winner replication, factor=0.3) ---")
        for label, r in valid:
            if label == "NiII_f0p3_rep": continue
            dbn = r["bnrms"] - ctrl["bnrms"]
            dco = r["cost"]  - ctrl["cost"]
            print(f"  {label:<22}  ΔRMS_bn = {dbn:+.4f}   Δcost = {dco:+.4f}")

    # K1 f0p3 reference: bnrms=0.366, cost=0.386 (from memory)
    print(f"\n  --- vs K1 (156056) f0p3 reference: bnrms=0.366, cost=0.386 ---")
    for label, r in valid:
        dbn = r["bnrms"] - 0.366
        dco = r["cost"]  - 0.386
        print(f"  {label:<22}  ΔRMS_bn = {dbn:+.4f}   Δcost = {dco:+.4f}")
...
hdr2 = f"  {'band':<14}  " + "  ".join(f"{l[:13]:<13}" for l,_ in CELLS)
print(hdr2)
for lo, hi, name in SUB_BANDS:
    ...
    for label,_ in CELLS:
        p = ROOT/f"logs/ddc15K1b_{JOB}_ddc15K1b_{label}/lumina_spectrum_formal.csv"
        d = scale(p, hlam, hflu)
        row += (f"{band_int(*d, lo, hi)/iH:<13.3f}" if d is not None else f"{'---':<13}") + "  "
    print(row)
print()
