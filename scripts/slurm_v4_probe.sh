...
# V4 PROBE: diagnostic — is dynamic_transprob the iter 3 stall bottleneck?
#
# Tiny config: 50k packets × 5 iter, single cell (hot1p45),
# CMFGEN clean data, SAME SCALE stack as V4 smoke, BUT LUMINA_DYNAMIC_TRANSPROB=0.
#
# Expected runtime if DT was the bottleneck: <10 min (lands all 5 iter)
# If still hangs in iter 3 → architectural issue, not DT recompute.
...
label="v4_probe_skipFeIronpeak"
work_root="$ROOT/logs/v4_probe_${SLURM_JOB_ID}"
mkdir -p "$work_root"
cd "$work_root"
...
print(f"  t_exp={t_exp:.0f}s  L={L_inner:.3e}  T_inner={T_inner:.1f}K")
PYEOF

echo "=== V4 PROBE SKIP_IRON_PEAK  N_PKT=$N_PKT N_ITER=$N_ITER ==="
echo "Host: $(hostname)  GPU: $(nvidia-smi --query-gpu=name --format=csv,noheader|head -1)"
echo "Time: $(date)"
echo "Hypothesis: 12806 NLTE levels (iron-peak Co/Ni/Fe/Cr/Ti) overload cuBLAS LU"
echo "Skip: Z=14,21,22,23,24,25,26,27,28 (Si+all iron-peak) -> NLTE down to ~1800 lev"

LUMINA_BF_OPACITY=1 \
LUMINA_CMFGEN_SIGMA_BF="$ROOT/data/atomic/cmfgen_sigma_bf.bin" \
LUMINA_DYNAMIC_TRANSPROB=0 \
LUMINA_NLTE_SKIP_Z="14,21,22,23,24,25,26,27,28" \
...
stdbuf -oL -eL "$BIN" "$REF_DIR" "$N_PKT" "$N_ITER" spectrum nlte \
