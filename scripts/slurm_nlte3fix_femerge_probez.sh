#!/bin/bash
#SBATCH --job-name=nlte3fix_probez
...
# #219f Phase 6: probe new (Z, ion) UV damp on top of current best.
# Repurposes SCALE2 slot (Si II UV, dead since Si SKIP_Z'd).
#
# Args: $1 = NEW_Z       (atomic number for SCALE2 repurpose)
#       $2 = NEW_ION     (ion stage; 1=II, 2=III)
#       $3 = NEW_FAC     (A_ul factor for that (Z,ion) at λ<4000)
#       $4 = NI_UV_FAC   (SCALE1, default 0.3)
#       $5 = FE_FAC      (SCALE3, default 0.02 = best)
#       $6 = CO_FAC      (SCALE4, default 0.3)
...
ni_tag=$(echo "$NI_UV_FAC" | tr '.' 'p')
fe_tag=$(echo "$FE_FAC" | tr '.' 'p')
co_tag=$(echo "$CO_FAC" | tr '.' 'p')
new_tag=$(echo "$NEW_FAC" | tr '.' 'p')
label="nlte3fix_probez_z${NEW_Z}i${NEW_ION}f${new_tag}_${tag}"
...
echo "=== NLTE3FIX-PROBEZ Z=$NEW_Z ion=$NEW_ION f=$NEW_FAC | NI=$NI_UV_FAC FE=$FE_FAC CO=$CO_FAC [${label}] ==="
...
env LUMINA_BF_OPACITY=1 \
    ...
    LUMINA_AUL_SCALE2_FACTOR=$NEW_FAC \
    LUMINA_AUL_SCALE2_LAMBDA_MAX=4000 \
    LUMINA_AUL_SCALE2_ZMASK=$NEW_Z \
    LUMINA_AUL_SCALE2_IONMASK=$NEW_ION \
...
print(f"  [PROBEZ Z=$NEW_Z i=$NEW_ION f=$NEW_FAC] Snifs [3300,8000]   rms_bn = {r:.4f}   chi_bn = {chi:.3f}")
    ...
    if gH > 0:
        for lo, hi, lbl in [(3000., 8000., "[3000,8000]"), (1700., 3000., "[1700,3000]")]:
            r, chi = rms_bn_nw(mlam, mflu, hl, hf, gH,
                               ...
                               wl_lo=lo, wl_hi=hi)
            print(f"  [PROBEZ Z=$NEW_Z i=$NEW_ION f=$NEW_FAC] HST   {lbl}  rms_bn = {r:.4f}   chi_bn = {chi:.3f}")
PYEOF
