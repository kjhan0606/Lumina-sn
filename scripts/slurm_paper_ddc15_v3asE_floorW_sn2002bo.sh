#!/bin/bash
#SBATCH --job-name=paper_ddc15_v3asEfloorW_2002bo
...
# #284 v3 — ALL paper parameters applied (Blondin+ 2013 DDC15 + Table 2):
#   ✓ t_rise=17.76d, L_bol=1.22e43, v_τ=9019 km/s   (BC)
#   ✓ Photospheric X(Si)=0.214, X(Ca)=0.056, X(Fe)=0.0927, X(O)≈2e-6 at v=9019  (Table 2)
#   ✓ 56Ni→56Co→56Fe decay at t=17.76d, X(56Ni_init)=0.625 in IME shell (so X(Co)=0.46 at v=9019)
#   ✓ 56Ni shell upper bound v(56Ni)=11200 km/s   (Table 1)
#   ✓ DDC15 broken power-law density: DE_in=4 (v<11000 km/s), DE_out=13 (v>11000)  (Fig 2-style)
#   ✓ Total mass in grid (vi=9019 to v_max) normalized to M_WD - M(τ_es=2/3) = 1.4-0.786=0.614 M⊙
# NO LUMINA_AUL_SCALE* knobs. Plain physics.
...
if echo "$GPU_NAME" | grep -qi H100; then
    BIN="$ROOT/lumina_cuda_oTri_h100_floorW"
elif echo "$GPU_NAME" | grep -qi H200; then
    BIN="$ROOT/lumina_cuda_oTri_h100_floorW"
else
    BIN="$ROOT/lumina_cuda_oTri_a100_floorW"
fi
...
label="paperDDC15v3asEfloorW_2002bo_vi${vi_tag}_L${l_tag}_${RUN_TAG}"
...
echo "=== PAPER-DDC15-v3 (ALL paper params) vs SN 2002bo  VI=$VI_KMS L=$L_scale  RUN=$RUN_TAG [${label}] ==="
