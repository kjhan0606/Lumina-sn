#!/bin/bash
#SBATCH --job-name=ddc15_W1_ca2boost
...
# W1 (Tier-2 Probe V1, per-trough diag): Ca II A_ul BOOST stacked on U1_f0p1.
#
# U1_f0p3 vs HST trough zoom-in (figures/U1_f0p3_vs_hst_troughs.png) shows:
#   - Ca H&K (3934 Å): shallow trough, CaK band ratio 0.81
#   - Ca IR triplet (8498/8542/8662 Å): NEARLY absent in LUMINA
#   - NIR1 over-emission (ratio 2.24) likely tied to Ca IR absorption deficit
# Both failures involve SAME species (Ca II) — single A_ul boost on Z=20 ion=1
# across [3700, 9000] Å targets H&K resonance + IR triplet simultaneously.
#
# Base = U1_f0p1 (best J=0.257, RMS_bn 0.196; Ni II opt f=0.10 layered on T1_eps0p7).
# SCALE8 is the remaining free knob slot. Range [3700,9000] covers Ca II diagnostic lines.
#
# Array cells:
#   0 — ctrl  : SCALE8 OFF (calibration = U1_f0p1 reproducibility @ 0.196)
#   1 — b1p5  : SCALE8 f=1.5
#   2 — b2p0  : f=2.0
#   3 — b3p0  : f=3.0
...
# Fixed (U1_f0p1 base = T1_eps0p7 + Ni II opt f=0.10)
FE_FAC=0.3
CO_FAC=0.3
RED_FAC=0.3
SI_OPT_FAC=0.10
...
label="ddc15W1_${tag}"
work_root="$ROOT/logs/ddc15W1_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "=== W1 [${label}]  U1_f0p1 base + Ca2_boost(SCALE8) f=${ca2}  N_PKT=${N_PKT}  N_ITER=${N_ITER} ==="
