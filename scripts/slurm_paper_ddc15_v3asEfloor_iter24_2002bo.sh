#!/bin/bash
#SBATCH --job-name=paper_ddc15_v3asEfloor_iter24_2002bo
...
# #291 N_ITER=24 convergence probe on 157921 base (EPS_IR=0.0, α v1).
# Hypothesis (from #290 diagnostic of 157921 iter trail):
#   T_inner +9.71% overshoot is iter-undamping. 12 iters insufficient to
#   damp from iter-4 bootstrap shock (L_em/L_req=0.46) down to ratio=1.0.
# Test: run identical config with N_ITER=24. Watch T_inner trajectory:
#   - drops to 10500-10800 K → convergence-artifact (just bump prod default)
#   - plateaus at 11400-11500 → structural over-emission, atomic-data needed.
...
label="paperDDC15v3asEfloorITER24_2002bo_vi${vi_tag}_L${l_tag}_${RUN_TAG}"
...
echo "=== PAPER-DDC15-v3 (ALL paper params) vs SN 2002bo  VI=$VI_KMS L=$L_scale EPS_IR=$EPS_IR  RUN=$RUN_TAG [${label}] ==="
