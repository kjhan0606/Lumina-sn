#!/bin/bash
#SBATCH --job-name=v2_hst_bracket
...
# V2: X1_b3p0 champion stack across 4 HST-matching SN 2011fe epochs.
# Scores against BOTH Snifs (shape) and HST stitched (UV blanketing test).
#
# Cells anchored to HST observation phases (sn2011fe HST GO-12592 program):
#   0 — p−6.6d : t_exp=10.6d, L=0.40e43, Snifs m6d7d, HST G230LB+G430L+G750L
#   1 — p+0.4d : t_exp=17.6d, L=1.30e43, Snifs p0d2d, HST G230LB+G430L+G750L
#   2 — p+3.7d : t_exp=20.7d, L=1.10e43, Snifs p4d2d, HST G230LB+G430L+G750L+FUV
#   3 — p+9.6d : t_exp=26.6d, L=0.70e43, Snifs p9d2d, HST G230LB+G430L+G750L
...
TAGS=(p-6.6d p+0.4d p+3.7d p+9.6d)
T_EXP_S=(915840 1520640 1788480 2298240)
L_INNER=(4.0e+42 1.3e+43 1.1e+43 7.0e+42)
SNIFS=(sn2011fe_m6d7d.csv sn2011fe_p0d2d.csv sn2011fe_p4d2d.csv sn2011fe_p9d2d.csv)
HST_TAG=(phase-06.6 phase+00.4 phase+03.7 phase+09.6)

tag=${TAGS[$SLURM_ARRAY_TASK_ID]}
t_exp_s=${T_EXP_S[$SLURM_ARRAY_TASK_ID]}
L_inner=${L_INNER[$SLURM_ARRAY_TASK_ID]}
snifs=${SNIFS[$SLURM_ARRAY_TASK_ID]}
hst_tag=${HST_TAG[$SLURM_ARRAY_TASK_ID]}

# X1_b3p0 champion stack (identical to V1)
FE_FAC=0.3
CO_FAC=0.3
RED_FAC=0.3
SI_OPT_FAC=0.10
...
label="v2_${tag}"
work_root="$ROOT/logs/v2_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "=== V2 [${label}] N_PKT=${N_PKT} N_ITER=${N_ITER} t_exp=${t_exp_s}s L=${L_inner} ==="
...
echo ""
echo "=== score vs Snifs $snifs + HST $hst_tag ==="
python3 <<PYEOF
...
print(f"  V2 ${tag}  Snifs RMS_bn [3300,8000]  = {r_snifs:.4f}  (vs $snifs)")

# --- HST stitched: G230LB (UV) + G430L (blue) + G750L (red) ---
...
for grat, w_lo, w_hi in [("G230LB", 1700, 2900), ("G430L", 2900, 5266), ("G750L", 5266, 10000)]:
    ...
    if not files:
        ...
        continue
    # average overlapping exposures (often two split exposures per grating)
...
if parts:
    ...
    if gH > 0:
        r_hst_can  = rms_bn(mlam, mflu, hl, hf, gH, wl_lo=3000., wl_hi=8000.)  # canonical 0.193-style band
        r_hst_uv   = rms_bn(mlam, mflu, hl, hf, gH, wl_lo=1700., wl_hi=3000.)  # UV blanketing only
        r_hst_uvopt= rms_bn(mlam, mflu, hl, hf, gH, wl_lo=2000., wl_hi=8000.)  # full UV+opt
        print(f"  V2 ${tag}  HST  RMS_bn [3000,8000]   = {r_hst_can:.4f}")
        print(f"  V2 ${tag}  HST  RMS_bn [1700,3000] = {r_hst_uv:.4f}  (UV blanketing)")
        print(f"  V2 ${tag}  HST  RMS_bn [2000,8000]   = {r_hst_uvopt:.4f}  (UV+opt)")
    else:
        print(f"  ! HST 4500-5800 norm band empty for ${hst_tag}")
else:
    print(f"  ! no HST stitched possible for ${hst_tag}")
PYEOF
