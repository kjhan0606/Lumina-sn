#!/bin/bash
#SBATCH --job-name=ddc15_H1_epsUV
...
# H1: LUMINA_EPS_UV sweep on C2 champion (DDC15 v_in=10400, X_Fe_outer=0.05,
# X_Fe_inner=0.15). EPS_UV is the probability that a UV-entry macro-atom call
# is short-circuited and re-emitted as Planck(T_rad) instead of running the
# downward cascade. Mechanism test: if red excess is UV→red cascade-driven,
# EPS_UV>0 should redirect UV exits to Planck(T_rad ~7000K) ≈ optical peak,
# REDUCING red-tot. If C2+D1+F1+G1 levers' null result is structural (red is
# not from cascade), EPS_UV will be NULL.
#   array 0 — EPS_UV=0.0 (baseline reproducibility)
#   array 1 — EPS_UV=0.3 (mild cascade short-circuit)
#   array 2 — EPS_UV=0.6 (strong)
#   array 3 — EPS_UV=0.9 (near-complete UV-cascade bypass)
...
# C2 champion params (fixed across array)
vi=10400
ro=0.00
de=9.0
xfeo=0.05

label=$(printf "ddc15H1_epsUV%s" "$eps_uv")
work_root="$ROOT/logs/ddc15H1_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "=== H1 epsUV sweep [${label}]  EPS_UV=${eps_uv}  C2-base  N_PKT=${N_PKT} ==="
