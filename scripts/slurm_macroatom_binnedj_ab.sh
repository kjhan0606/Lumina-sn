...
# Task #27 MACROATOM (two-way fluorescence) + binned-J estimator A/B.
# Downbranch+binnedJ (162213) HALVED but did NOT cure the one-way redshift
# cascade collapse (T_rad floor 2297->3927K, NIR 3.27->2.63, blue 0.03->0.07,
# L1=4.45). Hypothesis: macroatom's TWO-WAY redistribution (up-transitions
# return red->blue, unlike downbranch's one-way red drain) produces LESS net
# redshift, so the binned-J estimator may stay sane and fluorescence becomes
# usable. Cap LUMINA_MAX_INTERACTIONS=200 to dodge the iter3 macroatom hang.
#   A (ARM=A): scatter   + binned-J  (= 162212 known-good control, L1=2.44)
#   B (ARM=B): macroatom + binned-J  (two-way fluorescence, capped)
# ---- original downbranch header retained below ----
...
if [ "$ARM" = "B" ]; then
    LINE_INT=macroatom           # two-way fluorescence (paper macro-atom)
else
    LINE_INT=scatter             # baseline control (= 162212)
fi
...
echo "=== Task #27 macroatom+binned-J A/B  ARM=$ARM  BINNED_J=$BINNEDJ  LINE_INT=$LINE_INT  MAX_INT=200  DIFFUSE_BC=$DIFFUSE_BC  VI=$VI_KMS L=$L_SCALE_ARG  N_PKT=$N_PKT N_ITER=$N_ITER ==="
...
echo "  exit=$rc"
echo ""
echo "--- interaction cap + binned-J estimator activation ---"
grep -E "\[CAP\]|\[binned-J\]|truncat" stdout.log | head -30
echo ""
