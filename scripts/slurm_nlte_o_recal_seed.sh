...
# Args: $1=FE_FAC  $2=CO_FAC  $3=NI_UV_FAC  $4=NI2_OPT_FAC  $5=SEED (default 23111963)
# Other SCALE knobs (CA2/S9/NI3/SI_OPT) held at V3.1 historical values
# while we explore the 4 dominant axes against the new NLTE-O baseline.
...
# Frozen knobs (V3.1 historical, hold while we sweep the 4 above)
CA2_FAC=3.0
S9_FAC=0.05
NI3_FAC=0.0
SI_OPT_FAC=0.10
...
seed_tag=$(printf '%s' "$SEED" | tail -c 4)
label="nlteO_recalS_fe${fe_tag}_co${co_tag}_niuv${niuv_tag}_n2o${n2o_tag}_s${seed_tag}_${tag}"
...
echo "=== NLTEO-RECALS VI=$VI_KMS L=$L_scale DE=$DE_ARG SEED=$SEED | FE=$FE_FAC CO=$CO_FAC NI_UV=$NI_UV_FAC NI2_OPT=$NI2_OPT_FAC | (frozen) CA2=$CA2_FAC S9=$S9_FAC NI3=$NI3_FAC SI_OPT=$SI_OPT_FAC [${label}] ==="
...
print(f"  [RECALS fe=$FE_FAC co=$CO_FAC niuv=$NI_UV_FAC n2o=$NI2_OPT_FAC seed=$SEED] Snifs [3300,8000]   rms_bn = {r:.4f}   chi_bn = {chi:.3f}")
    ...
    if gH > 0:
        for lo, hi, lbl in [(3000., 8000., "[3000,8000]"), (1700., 3000., "[1700,3000]")]:
            r, chi = rms_bn_nw(mlam, mflu, hl, hf, gH,
                               ...
                               wl_lo=lo, wl_hi=hi)
            print(f"  [RECALS fe=$FE_FAC co=$CO_FAC niuv=$NI_UV_FAC n2o=$NI2_OPT_FAC seed=$SEED] HST   {lbl}  rms_bn = {r:.4f}   chi_bn = {chi:.3f}")
PYEOF
