#!/bin/bash
#SBATCH --job-name=ddc15_T1_s1_h2
...
# T1 (#228 follow-up): Combine S1 Si II opt damp f=0.1 (RMS_bn 0.2077 champ)
# with H2 EPS_UV redonly cascade (real continuum lever, redII 2.61→1.81 monotone).
#
# Hypothesis: S1 fixes line shape (Si II 6355 depth match HST), H2 fixes
# continuum (red excess). Stacking might decouple the metric blind spot.
#
# Same Q1b r03 stack base as S1/S2 (Ni0.3+Si0.05+Fe0.3+Co0.3 UV stack +
# Cr/Fe/Co III red), SCALE6 f=0.1 fixed, EPS_UV varied:
#
#   array 0 — eps0p0 : EPS_UV=0.0   (control = S1_f01 reproducibility @ 0.2077)
#   array 1 — eps0p3 : EPS_UV=0.3
#   array 2 — eps0p5 : EPS_UV=0.5
#   array 3 — eps0p7 : EPS_UV=0.7   (H2p best continuum, redII 1.90)
#   array 4 — eps0p9 : EPS_UV=0.9   (H2p best red-flux, redII 1.81)
#
# All EPS_UV_RED_ONLY=1 (post-cascade gate, only when bypass exits in red).
...
# aulscale8 has all SCALE1-6 knobs + EPS_UV_RED_ONLY; h3-era atomic data (1086212 levels)
...
LABELS=(eps0p0 eps0p3 eps0p5 eps0p7 eps0p9)
EPS_UVS=(0.0 0.3 0.5 0.7 0.9)
...
label="ddc15T1_${tag}"
work_root="$ROOT/logs/ddc15T1_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "=== T1 [${label}]  S1_f01 base + EPS_UV=${eps_uv} RED_ONLY=1   N_PKT=${N_PKT}  N_ITER=${N_ITER} ==="
...
grep -E 'Mean \|W error|Mean \|T_rad error|T_inner final|L_emitted|AUL_SCALE|EPS_UV' stdout.log | tail -20
echo "Done: $(date)"
