...
# V3.1 EPS_IR sweep on vi=10600 champion (Snifs 0.139, HST opt 0.169, HST UV 0.346).
# LUMINA_EPS_IR = NIR-entry (lam>7000 A) macro-atom thermalization prob (counterpart to EPS_UV).
# Untested env knob — last remaining novel single-knob lever after V4 path closed.
# Cells: {0.0 baseline, 0.3, 0.5, 0.7} on V3.1 W5 niF005 stack at p+3.7d hot1p45.
...
EPS_IR_LIST=(0.0 0.3 0.5 0.7)
EPS_IR=${EPS_IR_LIST[$SLURM_ARRAY_TASK_ID]}
...
label="v3_epsir${EPS_IR}"
work_root="$ROOT/logs/v3epsir_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
print(f"  t_exp={t_exp:.0f}s  L={L_inner:.3e}  T_inner={T_inner:.1f}K  vi={vi_kms} EPS_IR=$EPS_IR")
PYEOF

echo "=== V3.1 EPS_IR SWEEP [${label}] EPS_IR=${EPS_IR} ==="
...
echo ""
echo "--- convergence ---"
grep -E 'Mean \|W error|Mean \|T_rad error|T_inner final|L_emitted|EPS-IR' stdout.log | tail -8
...
verdict = '<=0.10 TARGET MET' if r_snifs<=0.10 else ('beats V3.1 0.139' if r_snifs<0.139 else 'no improvement')
print(f"  V3.1 EPS_IR=$EPS_IR  Snifs RMS_bn [3300,8000]  = {r_snifs:.4f}  ({verdict})")
...
if parts:
    ...
    if gH > 0:
        ...
        print(f"  V3.1 EPS_IR=$EPS_IR  HST RMS_bn [3000,8000]  = {r_hst_can:.4f}  (V3.1 0.169)")
        print(f"  V3.1 EPS_IR=$EPS_IR  HST RMS_bn [1700,3000]  = {r_hst_uv:.4f}  (V3.1 0.346)")
        def br(lo, hi):
        ...
        ref_m, ref_h = bm["Ref"]
        print(f"  V3.1 EPS_IR=$EPS_IR  --- SED band ratios ---")
        for (n,lo,hi) in bands:
            mm, hh = bm[n]
            if ref_m>0 and ref_h>0 and hh>0:
                r = (mm/ref_m)/(hh/ref_h)
                print(f"  V3.1 EPS_IR=$EPS_IR    band {n:<6} [{lo:5d},{hi:5d}]  L/HST = {r:.3f}")
PYEOF
