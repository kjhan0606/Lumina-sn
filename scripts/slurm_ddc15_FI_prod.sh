...
# Production validation for FI ablation (Task #227 follow-up):
#   600K×10 screening (job 156433) gave:
#     FI_base   0.2180  (S2_f015 replicate)
#     FI_cont   0.1457  (Δ -0.072, e-scatter + W·B(T_rad) continuum)
#     FI_both   0.1402  (cont + INNER_DILUTE=0.5, matches TARDIS 0.140)
#   Validate at 800K×12 to rule out MC noise.
#   Drop FI_idil05 (modest) and FI_tau0 (null) — keep base/cont/both.
...
# Pick binary by partition (sm_80 vs sm_90)
if [[ "$SLURM_JOB_PARTITION" == h100* || "$SLURM_JOB_PARTITION" == h200* ]]; then
    BIN="$ROOT/lumina_cuda_h100_fi"
else
...
LABELS=(FIp_base FIp_cont FIp_both)
...
label="ddc15FIp_${tag}"
work_root="$ROOT/logs/ddc15FIp_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "=== FIp [${label}]  S2_f015 base + CONT=${USE_CONT} W_INNER=${W_INNER}  N_PKT=${N_PKT}  N_ITER=${N_ITER} ==="
