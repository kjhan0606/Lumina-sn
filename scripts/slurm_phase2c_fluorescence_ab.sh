...
# Task #27 Phase 2c: fluorescence (downbranch) line-interaction A/B, built ON TOP
# of the Phase-2a diffusive inner BC fix. Phase-2b proved the [3500-4500] over-
# absorption is NOT ion-stage (write-back doubled Co II tau x2.8 but band NULL,
# saturated) and NOT W-weight (~3% at dominant shell) and NOT Co II gf (saturated
# lines tau_top 473-1020). It is a SATURATED line-blanketing + source-function
# REDISTRIBUTION problem. The paper (Blondin+2013 CMFGEN) uses full NLTE bb
# cascade with no ad-hoc thermalization = fluorescence redistribution, which in
# LUMINA = LINE_INTERACTION=downbranch, NOT coherent scatter. All prior
# fluorescence verdicts (CMF ~ scatter) were on the BROKEN T_inner=16000 K state;
# this re-tests on the FIXED Phase-2a baseline (confound removed).
#   A (ARM=A): coherent scatter  LUMINA_LINE_INTERACTION=scatter     (Phase-2a B repro)
#   B (ARM=B): fluorescence      LUMINA_LINE_INTERACTION=downbranch  (paper-faithful)
# BOTH: diffusive BC ON, self-pin ON, DDC15-strat, 200k, N_ITER=10. Single
# variable = LINE_INTERACTION. DYNAMIC_TRANSPROB=0 to use the validate_reference-
# clean static transprob table (0% zero rows) and dodge the #257 V4 zero-bug.
# Pre-registered prediction: if fluorescence is the lever, B's blue[3500-4500]
# deficit 0.825 rises toward 1.0 AND green/red excess (1.04-1.07) falls
# (blue->red redistribution reverses). If B ~ A, the residual is upstream
# (estimator T_rad / NLTE pops) -> pivot to (b).
#   sbatch --export=ALL,ARM=A scripts/slurm_phase2c_fluorescence_ab.sh
#   sbatch --export=ALL,ARM=B scripts/slurm_phase2c_fluorescence_ab.sh
...
echo "=== Task #27 Phase 2c fluorescence A/B  ARM=$ARM  LINE_INT=$LINE_INT  DIFFUSE_BC=$DIFFUSE_BC  VI=$VI_KMS L=$L_SCALE_ARG  N_PKT=$N_PKT N_ITER=$N_ITER ==="
echo "Host: $(hostname)  GPU: $GPU_NAME"
echo "Binary: $BIN  Time: $(date)"
echo "Both arms: diffusive BC ON, self-pin ON. A=scatter (Phase-2a B repro), B=downbranch fluorescence (paper-faithful). Static clean transprobs (DYNAMIC_TRANSPROB=0)."
...
echo "  exit=$rc"
echo ""
echo "--- inner-BC config + per-iter re-emission count ---"
grep -E "\[INNER-BC\]" stdout.log
echo ""
