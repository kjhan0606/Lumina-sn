#!/bin/bash
#SBATCH --job-name=ddc15_KL1_stack
...
# KL1: Stack K1 (Ni II A_ul f=0.3) × L1 (Si II A_ul f=0.05) using new
# LUMINA_AUL_SCALE2_* secondary rule. Tests independence/synergy.
#   K1 alone: RMS_bn ~0.366 (Ni II f=0.3, λ<4000)
#   L1 alone: RMS_bn 0.3154 (Si II f=0.05, λ<4000)
#   Independent → expected stack RMS_bn ~0.28-0.30
# 800K × 12 iter, C2 base. Binary: lumina_cuda_h100_aulscale2.
#
#   array 0 — control (no scale)
#   array 1 — Ni II f=0.3 only (replicate K1 best)
#   array 2 — Si II f=0.05 only (replicate L1 best)
#   array 3 — STACK: Ni II f=0.3 + Si II f=0.05
...
LABELS=(ctrl niIIonly_f0p3 siIIonly_f0p05 stack_ni0p3_si0p05)
tag=${LABELS[$SLURM_ARRAY_TASK_ID]}

# Default: no scaling (all unset). Per-cell overrides below.
NI_FAC=1.0
SI_FAC=1.0
case $SLURM_ARRAY_TASK_ID in
    1) NI_FAC=0.3 ;;
    2) SI_FAC=0.05 ;;
    3) NI_FAC=0.3; SI_FAC=0.05 ;;
esac
...
label="ddc15KL1_${tag}"
work_root="$ROOT/logs/ddc15KL1_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "=== KL1 stack [${label}]  NI_FAC=${NI_FAC} (λ<4000 Z=28 ion=1)  SI_FAC=${SI_FAC} (λ<4000 Z=14 ion=1)  N_PKT=${N_PKT}  N_ITER=${N_ITER} ==="
echo "Host: $(hostname)  GPU: $(nvidia-smi --query-gpu=name --format=csv,noheader|head -1)"
echo "Time: $(date)"

# Primary rule: Ni II
# Secondary rule (suffix=2): Si II
LUMINA_BF_OPACITY=1 \
