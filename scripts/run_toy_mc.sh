...
T=$ROOT/${TOYDIR}; OUT=logs/toy/${TAG}; mkdir -p $OUT/mc_${NPKT}; cd $OUT/mc_${NPKT}
...
env $COMMON LUMINA_CMFGEN_THEN_MC=1 LUMINA_LINE_INTERACTION=macroatom $ROOT/lumina_cuda $T ${NPKT} 6 virtual nlte > stdout.log 2>stderr.log
cp lumina_spectrum.csv ../mc_${NPKT}_spectrum.csv 2>/dev/null
echo "done MC NPKT=${NPKT}"
