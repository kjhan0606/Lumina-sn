#!/usr/bin/env python3
"""J1: Si II-targeted UV internal-down suppression analysis (job 156044).
...
CELLS = [
    ("SiII_f1p0_ctrl", "factor=1.0 (control)"),
    ("SiII_f0p7",      "factor=0.7 (mild)"),
    ("SiII_f0p5",      "factor=0.5 (moderate)"),
    ("SiII_f0p3",      "factor=0.3 (aggressive)"),
]
C_KMS = 299792.458
...
print(f"\n=== J1 Si II UV-idown sweep (job {JOB}, 800K × 12 iter) ===")
print(f"  H3 motivation: Si II = 21% of UV→red + 83% optical→red self-coupling\n")
print(f"  {'cell':<26}  {'UV/HST':<7}  {'red/HST':<8}  {'cost':<7}  {'band-log10':<11}  {'RMS_bn':<8}")
print("-"*80)

rows = []
for label, desc in CELLS:
    p = ROOT/f"logs/ddc15J1_{JOB}_ddc15J1_{label}/lumina_spectrum_formal.csv"
    d = scale(p, hlam, hflu)
    if d is None:
...
if valid:
    ...
    if "SiII_f1p0_ctrl" in dict(rows):
        ctrl = dict(rows)["SiII_f1p0_ctrl"]
        print(f"\n  --- vs control (factor=1.0, same run) ---")
        for label, r in valid:
            if label == "SiII_f1p0_ctrl": continue
