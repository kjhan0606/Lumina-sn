#!/usr/bin/env python3
"""P2: Stacked Fe II + Co II A_ul push around P1 winner Co0.3."""
...
CELLS = [
    ("Co0.3_repl",   f"logs/ddc15P2_{JOB}_ddc15P2_Co0p3_repl/lumina_spectrum_formal.csv"),
    ("Fe0.3+Co0.3",  f"logs/ddc15P2_{JOB}_ddc15P2_Fe0p3Co0p3/lumina_spectrum_formal.csv"),
    ("Fe0.2+Co0.3",  f"logs/ddc15P2_{JOB}_ddc15P2_Fe0p2Co0p3/lumina_spectrum_formal.csv"),
    ("Co0.2",        f"logs/ddc15P2_{JOB}_ddc15P2_Co0p2/lumina_spectrum_formal.csv"),
]
...
print("="*88)
print(f"=== P2 stacked Fe/Co A_ul push around P1 Co0.3 winner, job {JOB} ===")
print("    Baked: Ni II f=0.3, Si II f=0.05, λ<4000Å; baseline = Co0.3_repl (Fe0.5/Co0.3)")
...
print()
print("="*88)
print("=== Δ RMS_bn vs Co0.3_repl (local intra-job baseline) @ FWHM=20k & 40k ===")
print(f"{'comparison':<28s}  {'Δ HST_20k':>11s} {'Δ HST_40k':>11s} {'Δ TAR_20k':>11s} {'Δ TAR_40k':>11s}")
print("-"*88)
ch = rms["Co0.3_repl"]
for name in ["Fe0.3+Co0.3", "Fe0.2+Co0.3", "Co0.2"]:
    d = tuple(rms[name][i] - ch[i] for i in range(4))
    print(f"{name+' − Co0.3':<28s}  {d[0]:>+11.4f} {d[1]:>+11.4f} {d[2]:>+11.4f} {d[3]:>+11.4f}")

# Cross-job MC noise
print()
print("="*88)
print("=== MC noise envelope (Co0.3 reproducibility, cross-seed) ===")
print(f"    job 156163 P1 Co0.3:        HST_20k=0.2407")
print(f"    job 156183 P2 Co0.3_repl:   HST_20k={ch[0]:.4f}")
print(f"    Cross-seed Δ: {ch[0]-0.2407:+.4f}  (MC noise σ~0.02)")
...
print()
print("  Trends:")
fe_axis = [(0.5, ch[0]), (0.3, rms['Fe0.3+Co0.3'][0]), (0.2, rms['Fe0.2+Co0.3'][0])]
print(f"    Fe II axis (Co=0.3): " + " | ".join([f"f={f:.1f}→{r:.4f}" for f,r in fe_axis]))
print(f"    Co II axis (Fe=0.5): f=0.3→{ch[0]:.4f} | f=0.2→{rms['Co0.2'][0]:.4f}")
# Additive prediction
p1_fe_delta = -0.0099   # P1 Fe0.3 vs champ_repl @ HST_20k
p1_co_delta = -0.0164   # P1 Co0.3 vs champ_repl @ HST_20k
stacked = rms['Fe0.3+Co0.3'][0] - rms['Co0.3_repl'][0]
print()
print(f"  Additive test (Fe×Co):")
print(f"    P1 Fe0.3 alone Δ vs ref:   {p1_fe_delta:+.4f}")
print(f"    P2 Fe0.3+Co0.3 Δ vs Co0.3: {stacked:+.4f}")
ratio = stacked/p1_fe_delta if p1_fe_delta else float('nan')
print(f"    Stacking ratio: {ratio:+.2f}× (1.0 = perfect addition, 0 = saturated, <0 = antagonism)")
