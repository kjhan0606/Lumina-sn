#!/bin/bash
#SBATCH --job-name=ddc15_FI_ablation
#SBATCH --partition=a100,a100_pcie,a40
#SBATCH --gres=gpu:1
...
# FI (Task #227): formal-integral algorithmic ablation on S2_f015 base.
# Hypothesis: the >6000Å baseline excess (×1.5-2.0 over HST) is from
# inner-Planck leakage through line-only formal integral. The line walker
# uses Planck(T_inner)*exp(-tau_line) at z<r_phot but skips e-scatter,
# bf, ff continuum opacity entirely. Shell-0 chi_e=4.7e-15, dz~5e12 cm,
# tau_e per shell ~0.02, total ejecta tau_e ~ 0.7 → exp(-0.7)=0.5
# attenuation MISSING from the formal-integral inner Planck path.
#
# Cells:
#   0 — baseline: no env (replicates S2_f015 0.2061)
#   1 — CONT_OPACITY=1: e-scatter attenuation + W·B(T_rad) source in segments
#   2 — INNER_DILUTE=0.5: dilute T_inner Planck by W=0.5 (proxy)
#   3 — both: CONT + INNER_DILUTE=0.5 (stacked)
#   4 — TAU_CUTOFF=0: no weak-line skip (test if accumulation matters)
...
BIN="$ROOT/lumina_cuda_a100_fi"

N_PKT=600000
N_ITER=10

LABELS=(FI_base FI_cont FI_idil05 FI_both FI_tau0)
tag=${LABELS[$SLURM_ARRAY_TASK_ID]}

# Default env (no FI ablation)
USE_CONT=0
W_INNER=1.0
TAU_CUT=1e-5
...
label="ddc15FI_${tag}"
work_root="$ROOT/logs/ddc15FI_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "=== FI [${label}]  S2_f015 base + CONT=${USE_CONT} W_INNER=${W_INNER} TAU_CUT=${TAU_CUT}  N_PKT=${N_PKT}  N_ITER=${N_ITER} ==="
echo "Host: $(hostname)  GPU: $(nvidia-smi --query-gpu=name --format=csv,noheader|head -1)"
echo "Time: $(date)"

# S2_f015 base env (unchanged from #222)
LUMINA_BF_OPACITY=1 \
...
LUMINA_AUL_SCALE6_FACTOR=0.15 \
LUMINA_AUL_SCALE6_LAMBDA_MIN=4000 \
LUMINA_AUL_SCALE6_LAMBDA_MAX=7000 \
LUMINA_AUL_SCALE6_ZMASK=14 \
LUMINA_AUL_SCALE6_IONMASK=1 \
LUMINA_FI_CONT_OPACITY=$USE_CONT \
LUMINA_FI_INNER_DILUTE=$W_INNER \
LUMINA_FI_TAU_CUTOFF=$TAU_CUT \
"$BIN" "$REF_DIR" "$N_PKT" "$N_ITER" spectrum nlte \
    > stdout.log 2> stderr.log

rc=$?
echo "  exit=$rc"
echo "--- FI banner from log ---"
grep -E '^\s*\[FI\]|Mean \|W error|Mean \|T_rad error|T_inner final|L_emitted' stdout.log | tail -16
echo "Done: $(date)"
