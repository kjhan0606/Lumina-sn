#!/bin/bash
#SBATCH --job-name=einsteinFix_2002bo_tpin12630
...
# Test (a): pin T_inner to paper photospheric T_tau=12630 K instead of the
# self-consistent L-pin, which overshoots to ~15337 K in the O-fix baseline
# 160767. Single-variable A/B vs 160767 to test the agent verdict that the
# red/NIR continuum over-brightness + iron-peak III over-ionization trace to
# the T_inner overshoot. Everything else identical to 160767.
...
echo "=== (a) T_inner pinned 12630K  VI=$VI_KMS L=$L_scale  N_PKT=$N_PKT N_ITER=$N_ITER ==="
