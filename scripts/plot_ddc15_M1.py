#!/usr/bin/env python3
"""M1: Fe II A_ul λ<4000Å sweep — smoking-gun test (carsus Fe II UV 2-46× over NIST)."""
...
JOB  = sys.argv[1] if len(sys.argv) > 1 else "156136"

FACTORS = ["FeII_f1p0_ctrl", "FeII_f0p5", "FeII_f0p3", "FeII_f0p1"]
CELLS = [(f, ROOT/f"logs/ddc15M1_{JOB}_ddc15M1_{f}/lumina_spectrum_formal.csv")
         for f in FACTORS]
OUT = ROOT/f"figures/ddc15_M1_{JOB}_FeII_Aul_sweep.png"
C_KMS = 299792.458
SI_LAB = 6355.0
...
ax.set_xlabel('λ (Å)'); ax.set_ylabel('F_λ × 10¹⁴')
ax.set_title('Fe II UV multiplets [2200-3500]Å — does cut open UV?')
ax.legend(loc='upper right', fontsize=8); ax.grid(alpha=0.2)
...
for lam_m, lab in [(5972,'Si II'),(6355,'Si II'),(7773,'O I'),
    ...
    ax.text(lam_m, ymax*0.92, lab, rotation=90, fontsize=7, va='top', ha='right', alpha=0.5)
ax.set_xlabel('λ (Å)'); ax.set_ylabel('F_λ × 10¹⁴')
ax.set_title('Red continuum [5500-9500]Å — Fe II cut → cascade redistribution?')
ax.legend(loc='upper right', fontsize=8); ax.grid(alpha=0.2)
...
for f, (lam, flu) in data.items():
    ...
    print(f"  M1 {f:<16s}  v={v:+8.0f} km/s  depth={d:.3f}  Δv={v-hv:+6.0f}")
...
for f, (lam, flu) in data.items():
    print(f"  M1 {f:<16s}  band-log10-RMS = {band_log_rms(lam,flu):.4f}")
...
for f, (lam, flu) in data.items():
    ...
    print(f"  M1 {f:<16s}  RMS_bn = {rms:.4f}")
