#!/bin/bash
#SBATCH --job-name=ddc15_A1c_eps_top
...
# A1c (ε top-end): confirm A1b_e0p97 champion + locate ε=1.0 plateau.
#
# A1b (job 156558): e0p97 RMS_bn=0.1778, J=0.2244 — new joint-metric champion
# beating X1_b3p0 (J=0.237) and A1_e0p90 (J=0.229). A1b sweep flat ±0.004 over
# [0.85,0.97] within MC noise (±0.014). Need (a) MC reproducer of e0p97 and
# (b) ε=1.0 to find true upper boundary (full UV cascade bypass).
#
# Array cells (2):
#   0 — e0p97r : EPS_UV=0.97  (MC reproducer of A1b_e0p97)
#   1 — e1p00  : EPS_UV=1.00  (full UV cascade bypass)
...
label="ddc15A1c_${tag}"
work_root="$ROOT/logs/ddc15A1c_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "=== A1c [${label}]  X1_b3p0 base + EPS_UV=${EPS_UV}  N_PKT=${N_PKT}  N_ITER=${N_ITER} ==="
