#!/bin/bash
#SBATCH --job-name=ddc15_Q1b_iron3_red
...
# Q1b: red-window iron-peak III A_ul damp via new LAMBDA_MIN env knob.
# Goal: attack red excess (Si-red 1.51, OI/cont 1.79, Ca-IR 1.65) by
# directly reducing iron-peak III emission rates in [4500,7500]Å, while
# preserving Si II 6355 P-Cygni shape (which EPS_UV destroyed in Q1).
#
# Base = P2 winner stack (Ni 0.3 + Si 0.05 + Fe 0.3 + Co 0.3, all λ<4000Å,
# ion=1, RMS_bn=0.2384). New suffix-5 rule scales Cr/Fe/Co III in red band.
# Binary: lumina_cuda_h100_aulscale6 (new LAMBDA_MIN support).
#
#   array 0 — Q1b_ref:  stack only, replicate P2 Fe0.3+Co0.3 0.2384
#   array 1 — Q1b_r05:  +SCALE5 Cr/Fe/Co III f=0.5 λ∈[4500,7500]
#   array 2 — Q1b_r03:  +SCALE5 Cr/Fe/Co III f=0.3 λ∈[4500,7500]
#   array 3 — Q1b_r01:  +SCALE5 Cr/Fe/Co III f=0.1 λ∈[4500,7500]
...
BIN="$ROOT/lumina_cuda_h100_aulscale6"

N_PKT=800000
N_ITER=12

LABELS=(Q1b_ref Q1b_r05 Q1b_r03 Q1b_r01)
...
label="ddc15Q1b_${tag}"
work_root="$ROOT/logs/ddc15Q1b_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "=== Q1b [${label}]  stack(Ni0.3/Si0.05/Fe${FE_FAC}/Co${CO_FAC}) + RED_FAC=${RED_FAC} Cr/Fe/Co III λ∈[4500,7500]  N_PKT=${N_PKT}  N_ITER=${N_ITER} ==="
echo "Host: $(hostname)  GPU: $(nvidia-smi --query-gpu=name --format=csv,noheader|head -1)"
echo "Time: $(date)"

# Suffixes "", 2, 3, 4 = stack base (Ni II/Si II/Fe II/Co II, λ<4000Å, ion=1).
# Suffix 5 = NEW red-window iron-peak III rule (RED_FAC=1.0 → effectively off).
LUMINA_BF_OPACITY=1 \
