...
# #219c smoke: clean-ζ (Co III + Ni III from full partition functions)
# 4 cells × 800K × 12 iter at V3.1 base, p+3.7d (champion epoch).
# Pairwise comparison to control for MC noise:
#   idx 0: V3.1 L=1.45 reference-ζ  (control, expect Snifs ≈ 0.139)
#   idx 1: V3.1 L=1.45 clean-ζ      (treatment)
#   idx 2: V3.2 L=1.55 reference-ζ  (control, expect Snifs ≈ 0.141)
#   idx 3: V3.2 L=1.55 clean-ζ      (treatment)
# All at vi=10600, de=9.5, W5 niF005 stack.
...
# Select binary by GPU arch (set at runtime per allocated node)
GPU_NAME=$(nvidia-smi --query-gpu=name --format=csv,noheader | head -1)
case "$GPU_NAME" in
    *H100*|*H200*)   BIN="$ROOT/lumina_cuda_h100_aulscale9" ;;
    *A100*)          BIN="$ROOT/lumina_cuda_a100_aulscale9" ;;
    *)               BIN="$ROOT/lumina_cuda_h100_aulscale9" ;;  # fallback
esac

N_PKT=800000
N_ITER=12

# Source reference dirs (per cell)
SRC_REF_LIST=(
    "$ROOT/data/tardis_reference_strat6_2011fe_physical"
    "$ROOT/data/tardis_reference_zeta_clean"
    "$ROOT/data/tardis_reference_strat6_2011fe_physical"
    "$ROOT/data/tardis_reference_zeta_clean"
)
L_SCALE=(1.45 1.45 1.55 1.55)
SUFFIX=(v31_refz v31_cleanz v32_refz v32_cleanz)

# Common p+3.7d epoch params
t_exp_s=1788480
L_base=1.1e+43
...
SRC_REF=${SRC_REF_LIST[$SLURM_ARRAY_TASK_ID]}
...
# V3.x structural axes
vi=10600
ro=0.00
de=9.5
xfeo=0.05

label="zclean_${tag}_${suffix}"
work_root="$ROOT/logs/zclean_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
# config.json may be a symlink — read from the original location
...
echo "=== ZETA-CLEAN [${label}] L=${L_inner} L_scale=${L_scale} N_PKT=${N_PKT} SRC_REF=${SRC_REF} ==="
...
print(f"  [${suffix}] Snifs [3300,8000]   rms_bn = {r:.4f}   chi_bn = {chi:.3f}")
    ...
    if gH > 0:
        for lo, hi, lbl in [(3000., 8000., "[3000,8000]"), (1700., 3000., "[1700,3000]")]:
            r, chi = rms_bn_nw(mlam, mflu, hl, hf, gH,
                               ...
                               wl_lo=lo, wl_hi=hi)
            print(f"  [${suffix}] HST   {lbl}  rms_bn = {r:.4f}   chi_bn = {chi:.3f}")
PYEOF
