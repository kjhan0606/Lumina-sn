#!/usr/bin/env python3
"""Q1: cascade-reddening EPS_UV probe on top of P2 A_ul stack base."""
...
CELLS = [
    ("Q1_ref",     f"logs/ddc15Q1_{JOB}_ddc15Q1_Q1_ref/lumina_spectrum_formal.csv"),
    ("Q1_eps04",   f"logs/ddc15Q1_{JOB}_ddc15Q1_Q1_eps04/lumina_spectrum_formal.csv"),
    ("Q1_eps04r",  f"logs/ddc15Q1_{JOB}_ddc15Q1_Q1_eps04r/lumina_spectrum_formal.csv"),
    ("Q1_eps06r",  f"logs/ddc15Q1_{JOB}_ddc15Q1_Q1_eps06r/lumina_spectrum_formal.csv"),
]
...
print("="*88)
print(f"=== Q1 EPS_UV cascade-reddening probe on P2 stack base, job {JOB} ===")
print("    A_ul stack baked: Ni 0.3 + Si 0.05 + Fe 0.3 + Co 0.3, λ<4000Å, ion=1")
print("    Q1_ref=no EPS, eps04=broad, eps04r=red-only, eps06r=stronger red-only")
...
print()
print("="*88)
print("=== Δ RMS_bn vs Q1_ref (intra-job A_ul-only anchor) @ FWHM=20k & 40k ===")
print(f"{'comparison':<28s}  {'Δ HST_20k':>11s} {'Δ HST_40k':>11s} {'Δ TAR_20k':>11s} {'Δ TAR_40k':>11s}")
print("-"*88)
ch = rms["Q1_ref"]
for name in ["Q1_eps04", "Q1_eps04r", "Q1_eps06r"]:
    d = tuple(rms[name][i] - ch[i] for i in range(4))
    print(f"{name+' − Q1_ref':<28s}  {d[0]:>+11.4f} {d[1]:>+11.4f} {d[2]:>+11.4f} {d[3]:>+11.4f}")

# Cross-job MC noise vs P2 Fe0.3+Co0.3
print()
print("="*88)
print("=== Cross-job reproducibility (Q1_ref same config as P2 Fe0.3+Co0.3) ===")
print(f"    job 156183 P2 Fe0.3+Co0.3:  HST_20k=0.2384")
print(f"    job 156190 Q1_ref:          HST_20k={ch[0]:.4f}")
print(f"    Cross-seed Δ: {ch[0]-0.2384:+.4f}  (MC noise σ~0.02)")
...
print()
print("  Q1 cascade-reddening verdict:")
deltas = {name: rms[name][0] - ch[0] for name in ["Q1_eps04", "Q1_eps04r", "Q1_eps06r"]}
best_q = min(deltas.items(), key=lambda x: x[1])
if best_q[1] <= -0.005:
    print(f"    → Q1 LEVER ACTIVE: best {best_q[0]} Δ={best_q[1]:+.4f}")
elif abs(best_q[1]) < 0.005:
    print(f"    → Q1 lever within MC noise (best Δ={best_q[1]:+.4f}) — INCONCLUSIVE")
else:
    print(f"    → Q1 lever WORSE (best deviation {best_q[1]:+.4f}) — EPS_UV mechanism closed on A_ul stack base too")
print()
print("  Red-band ratio (HST relative) by cell:")
