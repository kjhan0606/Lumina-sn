...
# #273 NLTE-O SCALE re-calibration sweep on FEMERGE Phase 9b best cell:
#   vi=11600, L_scale=2.45, de=10.5, p+3.7d, x_fe_outer=0.05.
# Args: $1=FE_FAC  $2=CO_FAC  $3=NI_UV_FAC  $4=NI2_OPT_FAC  $5=NI3_FAC
# Other SCALE knobs (CA2/S9/SI_OPT) held at V3.1 historical values
# while we explore the 5 dominant axes against the new NLTE-O baseline.
# PRODUCTION: 800K x 12 iter (single seed). Internal score_nw against
# Snifs p+3.7d so all cross-cell comparisons are same-metric.
...
# Frozen knobs (V3.1 historical, hold while we sweep the 5 above)
CA2_FAC=3.0
S9_FAC=0.05
SI_OPT_FAC=0.10
...
BIN="$ROOT/lumina_cuda_a100_nlte_o"

N_PKT=${N_PKT:-800000}
N_ITER=${N_ITER:-12}

SRC_REF="${LUMINA_SRC_REF:-$ROOT/data/tardis_reference_v3_femerge}"
L_scale=$L_SCALE_ARG

# #280: prefer ref-local σ_bf bin (baked to match ref's n_levels) over the
# global CMFGEN-only bin (which the C loader rejects on n_levels mismatch
# → falls back to Kramers for all levels).
if [[ -f "$SRC_REF/cmfgen_sigma_bf.bin" ]]; then
  SIGMA_BF_BIN="$SRC_REF/cmfgen_sigma_bf.bin"
else
  SIGMA_BF_BIN="$ROOT/data/atomic/cmfgen_sigma_bf.bin"
fi
...
fe_tag=$(echo "$FE_FAC" | tr '.' 'p')
co_tag=$(echo "$CO_FAC" | tr '.' 'p')
niuv_tag=$(echo "$NI_UV_FAC" | tr '.' 'p')
n2o_tag=$(echo "$NI2_OPT_FAC" | tr '.' 'p')
ni3_tag=$(echo "$NI3_FAC" | tr '.' 'p')
label="nlteO_recalP_fe${fe_tag}_co${co_tag}_niuv${niuv_tag}_n2o${n2o_tag}_ni3${ni3_tag}_${tag}"
...
echo "=== NLTEO-RECAL VI=$VI_KMS L=$L_scale DE=$DE_ARG | FE=$FE_FAC CO=$CO_FAC NI_UV=$NI_UV_FAC NI2_OPT=$NI2_OPT_FAC NI3=$NI3_FAC | (frozen) CA2=$CA2_FAC S9=$S9_FAC SI_OPT=$SI_OPT_FAC [${label}] ==="
echo "Host: $(hostname)  GPU: $(nvidia-smi --query-gpu=name --format=csv,noheader|head -1)"
echo "Binary: $BIN  Time: $(date)"
echo "sigma_bf: $SIGMA_BF_BIN"

env LUMINA_BF_OPACITY=1 \
    LUMINA_CMFGEN_SIGMA_BF=$SIGMA_BF_BIN \
    ...
    LUMINA_AUL_SCALE2_FACTOR=$NI3_FAC \
    LUMINA_AUL_SCALE2_LAMBDA_MAX=4000 \
    LUMINA_AUL_SCALE2_ZMASK=28 \
    LUMINA_AUL_SCALE2_IONMASK=2 \
...
echo ""
echo "--- score vs Snifs / HST (same-metric score_nw) ---"
python3 <<PYEOF
...
print(f"  [RECAL fe=$FE_FAC co=$CO_FAC niuv=$NI_UV_FAC n2o=$NI2_OPT_FAC ni3=$NI3_FAC] Snifs [3300,8000]   rms_bn = {r:.4f}   chi_bn = {chi:.3f}")
    ...
    if gH > 0:
        for lo, hi, lbl in [(3000., 8000., "[3000,8000]"), (1700., 3000., "[1700,3000]")]:
            r, chi = rms_bn_nw(mlam, mflu, hl, hf, gH,
                               ...
                               wl_lo=lo, wl_hi=hi)
            print(f"  [RECAL fe=$FE_FAC co=$CO_FAC niuv=$NI_UV_FAC n2o=$NI2_OPT_FAC ni3=$NI3_FAC] HST   {lbl}  rms_bn = {r:.4f}   chi_bn = {chi:.3f}")
PYEOF
