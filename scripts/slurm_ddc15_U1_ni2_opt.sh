#!/bin/bash
#SBATCH --job-name=ddc15_U1_ni2opt
...
# U1 (Tier-2 Probe B): Ni II OPTICAL-band A_ul damp layered on T1_eps0p7.
#
# T1_eps0p7 (J=0.262 #1) already damps Ni II UV (SCALE1 λ<4000 f=0.3).
# H3 fate hist: Ni II responsible for 68% of UV→red cascade in ε=0.0 baseline.
# H1b/H2p showed redII excess persists after EPS_UV cascade (2.0× HST in T1).
# Hypothesis: extending Ni II damp into optical [4000,7500] further trims redII
# without disturbing line-shape (since Ni II opt has fewer Si/Ca diagnostic
# overlaps). SCALE7 is the free slot.
#
# Array cells (all stack: Q1b r03 NLTE + S1 Si II opt + EPS_UV=0.7 redonly):
#   0 — ctrl    : SCALE7 OFF (calibration = T1_eps0p7 reproducibility @ 0.205)
#   1 — f0p5    : SCALE7 Z=28 ion=1 λ∈[4000,7500] f=0.50
#   2 — f0p3    : f=0.30
#   3 — f0p1    : f=0.10 (matches S1 SCALE6 Si II opt knee)
#   4 — f0p05   : f=0.05
...
LABELS=(ctrl  f0p5 f0p3 f0p1 f0p05)
NI2_OPTS=(1.0 0.5  0.3  0.1  0.05)
tag=${LABELS[$SLURM_ARRAY_TASK_ID]}
ni2_opt=${NI2_OPTS[$SLURM_ARRAY_TASK_ID]}

# Fixed (T1_eps0p7 base)
FE_FAC=0.3
CO_FAC=0.3
RED_FAC=0.3
SI_OPT_FAC=0.10
...
echo "=== U1 [${label}]  T1_eps0p7 base + Ni2_opt(SCALE7) f=${ni2_opt}  N_PKT=${N_PKT}  N_ITER=${N_ITER} ==="
...
LUMINA_AUL_SCALE7_FACTOR="$ni2_opt" \
