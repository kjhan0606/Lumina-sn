#!/bin/bash
#SBATCH --job-name=ddc15_L1_SiII_Aul
...
# L1: Same A_ul scaling as K1 but applied to Si II instead of Ni II.
# K1/K1b proved Ni II A_ul is real lever (saturates at f0p3, floor RMS_bn ~0.38)
# but red excess (1.48-1.57) persists. H3 attribution: Si II = 83% of OPTICAL→red
# self-coupling. Test if cutting Si II UV-line strength addresses red excess.
# Note: LUMINA_NLTE_SKIP_Z=14 still set (Si non-NLTE), but A_ul scaling at load
# is upstream of plasma calc — affects opacity regardless of LTE/NLTE.
# 800K × 12 iter, C2 base (DDC15 v_in=10400, X_Fe_outer=0.05). Binary: lumina_cuda_h100_aulscale.
#
#   array 0 — factor=1.0   control (no scale)
#   array 1 — factor=0.5   moderate
#   array 2 — factor=0.2   strong
#   array 3 — factor=0.05  extreme (full sweep span)
...
BIN="$ROOT/lumina_cuda_h100_aulscale"

N_PKT=800000
N_ITER=12

FACTORS=(1.0  0.5  0.2  0.05)
LABELS=(SiII_f1p0_ctrl SiII_f0p5 SiII_f0p2 SiII_f0p05)
...
label="ddc15L1_${tag}"
work_root="$ROOT/logs/ddc15L1_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "=== L1 Si II A_ul scale [${label}]  FACTOR=${factor}  Z=14(Si) ion=1(II)  λ_max=4000Å  N_PKT=${N_PKT}  N_ITER=${N_ITER} ==="
...
LUMINA_AUL_SCALE_FACTOR="$factor" \
LUMINA_AUL_SCALE_LAMBDA_MAX=4000 \
LUMINA_AUL_SCALE_ZMASK=14 \
...
grep -E 'Mean \|W error|Mean \|T_rad error|T_inner final|L_emitted|AUL_SCALE' stdout.log | tail -12
echo "Done: $(date)"
