...
# Task #27 Phase 2a: luminosity-conserving diffusive inner boundary A/B.
# Phase 0 verdict = Branch A (reabsorption-dominated, trunc=0): the absorbing
# hard sphere eats back-scatter-trapped energy, so the self-pin over-heats
# T_inner to ~16000 K to close the deficit -> over-ionizes iron to III ->
# UV curtain lost. Phase 2a returns that energy: a packet falling through the
# photosphere is re-emitted from the core at T_inner instead of killed.
#   A (ARM=A): absorbing hard sphere       LUMINA_DIFFUSE_INNER_BC=0  (keeper)
#   B (ARM=B): diffusive lower boundary    LUMINA_DIFFUSE_INNER_BC=1
# CRUCIAL: self-pin is ON (no LUMINA_T_INNER_FIX). Success = B's T_inner
# settles near 12630 ON ITS OWN, fIII<0.5 line-forming, UV leak shrinks --
# all three together. Same keeper config as 161922 (DDC15-strat, scatter,
# radeq-OFF, 200k, N_ITER=10).
#   sbatch --export=ALL,ARM=A scripts/slurm_phase2a_innerbc_ab.sh
#   sbatch --export=ALL,ARM=B scripts/slurm_phase2a_innerbc_ab.sh
...
echo "=== Task #27 Phase 2a inner-BC A/B  ARM=$ARM  DIFFUSE_BC=$DIFFUSE_BC  VI=$VI_KMS L=$L_SCALE_ARG  N_PKT=$N_PKT N_ITER=$N_ITER ==="
echo "Host: $(hostname)  GPU: $GPU_NAME"
echo "Binary: $BIN  Time: $(date)"
echo "Self-pin ON (no T_INNER_FIX) -- testing whether diffusive BC lets T_inner settle near paper 12630 on its own."
