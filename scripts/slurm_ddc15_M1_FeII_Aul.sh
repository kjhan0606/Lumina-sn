#!/bin/bash
#SBATCH --job-name=ddc15_M1_FeII_Aul
...
# M1: Fe II A_ul λ<4000Å direct scaling sweep. Carsus audit
# (audit_carsus_uv_aul.py) revealed Fe II UV multiplets 2-46× over NIST
# (2260 46×, 2344 2.9×, 2383 2.06×, 2587 2.21×). Fe II carries 139k lines
# total, 34k in UV (10× more than Ni II/Si II combined) → potentially the
# largest A_ul lever. K1/L1/KL1 family floor ≈ 0.30 RMS_bn.
# Binary: lumina_cuda_h100_aulscale2 (same as KL1, primary rule only).
#
#   array 0 — f=1.0 control
#   array 1 — f=0.5
#   array 2 — f=0.3 (K1-equivalent magnitude)
#   array 3 — f=0.1 (10× cut, matches median over-strength of UV multiplets)
...
LABELS=(FeII_f1p0_ctrl FeII_f0p5 FeII_f0p3 FeII_f0p1)
...
label="ddc15M1_${tag}"
work_root="$ROOT/logs/ddc15M1_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "=== M1 Fe II A_ul scale [${label}]  FACTOR=${factor}  Z=26(Fe) ion=1(II)  λ_max=4000Å  N_PKT=${N_PKT}  N_ITER=${N_ITER} ==="
...
grep -E 'Mean \|W error|Mean \|T_rad error|T_inner final|L_emitted|AUL_SCALE' stdout.log | tail -10
echo "Done: $(date)"
