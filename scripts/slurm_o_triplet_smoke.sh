...
# #281 O triplet NLTE smoke test on v3_femerge_capraise ref.
# Refactor: NLTE_MAX_IONS=31, NLTE_PAIR_COUNT=16.
#   slot 28 = O I, slot 29 = O II, slot 30 = O III.
#   pair 14 (28,29) and pair 15 (29,30) share slot 29 (O II).
# Sanity checks:
#  (a) banner shows Z=8 ion=0/1/2 (3 separate slots)
#  (b) pair "O(I-II)" and "O(II-III)" both run without cuBLAS errors
#  (c) no NaN/Inf, no n_total→1e-28 collapse on O ions
#  (d) spectrum lands → score vs #278 capraise champ 0.1914
# 200K x 3 iter (single seed, NLTE turns on iter 2).
...
# capraise ref tree (#278 champion: Fe III 800→1500, Co III 1500→3917)
...
de_tag=$(echo "$de" | tr '.' 'p')
label="oTri_smoke_vi${vi_tag}_L${l_tag}_de${de_tag}_${tag}"
...
echo "=== OTRI-SMOKE VI=$VI_KMS L=$L_scale DE=$DE_ARG | FE=$FE_FAC CO=$CO_FAC [${label}] ==="
...
echo ""
echo "--- NLTE init: confirm Z=8 ion=0/1/2 all present (3 slots) ---"
grep -E "Total NLTE levels|Z=8 ion=|^  \[NLTE\]" stdout.log | head -10

echo ""
echo "--- NLTE pair iterations (look for O(I-II), O(II-III)) ---"
grep -E "\\[NLTE-GPU\\]|O\\(.*\\)|Pair.*O" stdout.log | head -20
