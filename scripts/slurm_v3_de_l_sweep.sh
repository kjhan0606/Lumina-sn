...
# V3.1 structural completeness probe at NEW vi=10600 champion.
# Prior 3D structural sweep #173 found (vi=10400, de=9.0, L=1.45) but used OLD vi.
# Now that vi=10600 is the new V3.1 champion, re-test de and L_scale axes.
# 4 cells: density_exp × L_scale grid corners at vi=10600.
...
# 4-corner grid around current optimum (de=9.0, L=1.45)
DE_LIST=(9.5 9.5 9.5 9.5)
L_LIST=(1.55 1.60 1.65 1.70)
de=${DE_LIST[$SLURM_ARRAY_TASK_ID]}
...
label="v3_de${de}_L${L_scale}"
work_root="$ROOT/logs/v3del_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
print(f"  t_exp={t_exp:.0f}s  L={L_inner:.3e}  T_inner={T_inner:.1f}K  vi={vi_kms} de=$de L_scale=$L_scale")
PYEOF

echo "=== V3.1 de x L SWEEP [${label}] de=${de} L_scale=${L_scale} ==="
...
print(f"  V3.1 de=$de L=$L_scale  Snifs RMS_bn [3300,8000]  = {r_snifs:.4f}  ({verdict})")
...
if parts:
    ...
    if gH > 0:
        ...
        print(f"  V3.1 de=$de L=$L_scale  HST RMS_bn [3000,8000]  = {r_hst_can:.4f}  (V3.1 0.169)")
        print(f"  V3.1 de=$de L=$L_scale  HST RMS_bn [1700,3000]  = {r_hst_uv:.4f}  (V3.1 0.346)")
PYEOF
