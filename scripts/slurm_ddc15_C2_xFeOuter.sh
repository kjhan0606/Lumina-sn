#!/bin/bash
#SBATCH --job-name=ddc15_C2_xFeOuter
...
# C2: outer X_Fe cap on DDC15 v_in=10400 — Fe II UV multiplet test as
# real cascade-reddening lever (C1 X_Ti found null).
#   Inner Fe plateau (v<11000): X_Fe=0.15 fixed (needed for green-band Fe II forest)
#   Outer Fe shoulder (v>=11000): exp decay from {0.15, 0.05, 0.01, 1e-3}
# X_Mn=0.03·X_Fe, X_Cr=0.05·X_Fe follow automatically.
# 200k packets to beat C1 ±15% MC noise floor to ±7%.
...
X_FEOS=(0.15 0.05 0.01 1e-3)
xfe=${X_FEOS[$SLURM_ARRAY_TASK_ID]}
vi=10400
ro=0.00
de=9.0
label=$(printf "ddc15C2_xFeO%s" "$xfe")

work_root="$ROOT/logs/ddc15C2_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
src = "$SRC_REF"
dst = "$REF_DIR"
ro, de, vi_kms, X_FE_OUTER = $ro, $de, $vi, $xfe
...
# DDC15 profile with outer X_Fe cap (Mn, Cr scale with it)
def ddc15_profile(v, x_fe_outer):
    ...
    # Inner plateau v<11000: X_Fe=0.15 (FIXED, needed for green-band Fe II forest)
    # Outer shoulder v>=11000: exp decay from x_fe_outer (swept)
...
print(f"  built: v_in={vi_kms} X_Fe_outer={X_FE_OUTER}  rho0={rho0:.3e}  T_in_init={T_inner:.0f}")
print(f"  sh   v_mid    X_Si    X_Fe    X_Cr    X_Mn")
for s in [0, 2, 4, 6, 10, 15, 20, 25, 29]:
    print(f"  {s:>2}  {v_mid_new[s]:>6.0f}  {X[Z_LIST.index(14),s]:.3f}  "
          f"{X[Z_LIST.index(26),s]:.2e}  {X[Z_LIST.index(24),s]:.2e}  "
          f"{X[Z_LIST.index(25),s]:.2e}")
PYEOF

echo "=== DDC15 C2 [${label}]  X_Fe_outer=${xfe}  v_in=${vi}  N_PKT=${N_PKT} ==="
