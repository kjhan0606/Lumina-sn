#!/bin/bash
#SBATCH --job-name=einsteinFix_2002bo_macroatom_smoke
#SBATCH --partition=a100,h100,h200,a40
#SBATCH --gres=gpu:1
...
# PAPER-METHOD test: single-variable A/B vs self-pin baseline 160767.
# Everything identical to ofix_prod (einsteinFix ref, O fix, INV_CEIL gate,
# self-pin T_inner, N_ITER=15) EXCEPT LUMINA_LINE_INTERACTION=macroatom instead
# of scatter. Blondin+2013 uses fluorescence/bb-cascade redistribution, NOT
# coherent scatter; this is the first run that follows the paper's line-
# interaction method. L2M schema verified 100% valid on this ref (range
# [5,37491] in [0,37513)), so macroatom is actually active (not silently
# disabled per #305-3). Tests whether scatter-mode "ROOT A" symptoms
# (T_inner overshoot, L_em deficit, 4300 peak, over-deep troughs) self-resolve
# once thermalization/fluorescence is engaged.

VI_KMS=9019
L_SCALE_ARG=1.0
N_PKT=200000
N_ITER=6
RUN_TAG=macroatom_smoke
...
if echo "$GPU_NAME" | grep -qi H100; then
    ...
    BIN="$ROOT/lumina_cuda_paperli_a40_einsteinFix"
else
...
env LUMINA_BF_OPACITY=1 \
    ...
    LUMINA_MAX_INTERACTIONS=${MAX_INT:-5000} \
    ${FE_SCATTER:+LUMINA_FE_SCATTER=$FE_SCATTER} \
