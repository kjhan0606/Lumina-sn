#!/bin/bash
#SBATCH --job-name=nlte3fix_struct
...
# #219f Phase 9b: structural axis (vi, L_scale, de) re-scan on FEMERGE base.
# SCALE stack frozen at the 157472 baseline (NI_UV=0.3, FE=0.02, CO=0.3,
# SCALE6/7/8/9 = 0.10/0.05/3.0/0.05, no Ni III). V3.1 tune was set under buggy
# NLTE — this sweep tests whether the FEMERGE-corrected rate matrix shifts the
# (vi, L, de) optimum.
#
# Args: $1 = VI_KMS  (inner v boundary, default 10600)
#       $2 = L_SCALE (L_inner = L_base * L_SCALE, default 1.55)
#       $3 = DE_VAL  (density exponent, default 9.5)
...
de_tag=$(echo "$de" | tr '.' 'p')
label="nlte3fix_struct_vi${vi_tag}_L${l_tag}_de${de_tag}_${tag}"
...
echo "=== NLTE3FIX-STRUCT VI=$VI_KMS L=$L_scale DE=$DE_ARG | NI=$NI_UV_FAC FE=$FE_FAC CO=$CO_FAC NI2_OPT=$NI2_OPT_FAC CA2=$CA2_FAC S9=$S9_FAC [${label}] ==="
...
print(f"  [STRUCT vi=$VI_KMS L=$L_scale de=$DE_ARG] Snifs [3300,8000]   rms_bn = {r:.4f}   chi_bn = {chi:.3f}")
    ...
    if gH > 0:
        for lo, hi, lbl in [(3000., 8000., "[3000,8000]"), (1700., 3000., "[1700,3000]")]:
            r, chi = rms_bn_nw(mlam, mflu, hl, hf, gH,
                               ...
                               wl_lo=lo, wl_hi=hi)
            print(f"  [STRUCT vi=$VI_KMS L=$L_scale de=$DE_ARG] HST   {lbl}  rms_bn = {r:.4f}   chi_bn = {chi:.3f}")
PYEOF
