#!/usr/bin/env python3
"""A5: DDC15 + ultra-narrow low-X_Si IME (0.20, [v_in, v_in+1500]) + v_in scan vs HST."""
...
JOB  = sys.argv[1] if len(sys.argv) > 1 else "155734"

VINS = [9500, 9700, 9900, 10100]
CELLS = [(vi, ROOT/f"logs/ddc15A5_{JOB}_ddc15UN_v{vi:05d}/lumina_spectrum_formal.csv")
         for vi in VINS]
OUT = ROOT/f"figures/ddc15_A5_{JOB}_vs_hst.png"
C_KMS = 299792.458
...
for (vi, (lam, flu)), c in zip(data.items(), cmap):
    ax.plot(lam, flu*1e14, color=c, lw=0.9, alpha=0.85, label=f'A5 DDC15-UN v_in={vi}')
...
ax.set_title(f'A5: DDC15 ultra-narrow low-Si IME (X_Si=0.20, 1500km/s window) + v_in scan (job {JOB})')
...
ax.set_xlabel('λ (Å)'); ax.set_ylabel('F_λ × 10¹⁴')
ax.set_title('Si II 6355 zoom — ultra-narrow IME → trough should follow v_in')
ax.legend(loc='upper right', fontsize=8); ax.grid(alpha=0.2)
...
ax.set_xlabel('λ (Å)'); ax.set_ylabel('F_λ × 10¹⁴')
ax.set_title('UV blanketing 2300-4000Å — ultra-narrow IME vs Path A')
ax.legend(loc='upper right', fontsize=8); ax.grid(alpha=0.2)
...
print(f"\n=== Si II 6355 trough position (A5 DDC15-UN) ===")
tm = trough_min(hlam, hflu)
...
for vi, (lam, flu) in data.items():
    ...
    if tm:
        v = (1-tm/6355)*C_KMS
        print(f"{'A5 DDC15-UN v_in='+str(vi):40s}  λ={tm:.1f}  v={v:.0f} km/s  (HST diff {v-9934:+.0f})")
...
for vi, (lam, flu) in data.items():
    ...
    print(f"A5 DDC15-UN v_in={vi}                     RMS_bn = {rms:.4f}")
if pa_champ is not None:
