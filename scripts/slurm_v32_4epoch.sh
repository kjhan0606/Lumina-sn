...
# V3.2 4-epoch validation: V3.1 baseline (L=1.45) vs V3.2 (L=1.55) at
# vi=10600, de=9.5, full W5 niF005 stack across 4 HST epochs.
# 8 cells = 4 epochs × {V3.1 L=1.45, V3.2 L=1.55}:
#   idx 0-3: L_scale=1.45 V3.1 baseline (de=9.5, vi=10600)
#   idx 4-7: L_scale=1.55 V3.2 candidate (de=9.5, vi=10600)
#   epoch order in each block: p-6.6d, p+0.4d, p+3.7d, p+9.6d
...
TAGS=(p-6.6d p+0.4d p+3.7d p+9.6d p-6.6d p+0.4d p+3.7d p+9.6d)
T_EXP_S=(915840 1520640 1788480 2298240 915840 1520640 1788480 2298240)
L_BASE=(4.0e+42 1.3e+43 1.1e+43 7.0e+42 4.0e+42 1.3e+43 1.1e+43 7.0e+42)
L_SCALE=(1.45 1.45 1.45 1.45 1.55 1.55 1.55 1.55)
SNIFS=(sn2011fe_m6d7d.csv sn2011fe_p0d2d.csv sn2011fe_p4d2d.csv sn2011fe_p9d2d.csv \
       sn2011fe_m6d7d.csv sn2011fe_p0d2d.csv sn2011fe_p4d2d.csv sn2011fe_p9d2d.csv)
HST_TAG=(phase-06.6 phase+00.4 phase+03.7 phase+09.6 phase-06.6 phase+00.4 phase+03.7 phase+09.6)
SUFFIX=(v31_L1p45 v31_L1p45 v31_L1p45 v31_L1p45 v32_L1p55 v32_L1p55 v32_L1p55 v32_L1p55)

tag=${TAGS[$SLURM_ARRAY_TASK_ID]}
t_exp_s=${T_EXP_S[$SLURM_ARRAY_TASK_ID]}
L_base=${L_BASE[$SLURM_ARRAY_TASK_ID]}
L_scale=${L_SCALE[$SLURM_ARRAY_TASK_ID]}
...
# V3.2 structural axes
vi=10600
ro=0.00
de=9.5
xfeo=0.05

label="v32_${tag}_${suffix}"
work_root="$ROOT/logs/v32_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "=== V3.2 [${label}] L_base=${L_base} L_scale=${L_scale} L=${L_inner} de=${de} vi=${vi} N_PKT=${N_PKT} ==="
...
print(f"  V3.2 ${tag} ${suffix}  Snifs RMS_bn [3300,8000]  = {r_snifs:.4f}")
...
if parts:
    ...
    if gH > 0:
        ...
        print(f"  V3.2 ${tag} ${suffix}  HST RMS_bn [3000,8000]  = {r_hst_can:.4f}")
        print(f"  V3.2 ${tag} ${suffix}  HST RMS_bn [1700,3000]= {r_hst_uv:.4f}  (UV blanketing)")
        def br(lo, hi):
        ...
        ref_m, ref_h = bm["Ref"]
        print(f"  V3.2 ${tag} ${suffix}  --- SED band ratios (LUMINA/HST, normed to Ref) ---")
        for (n,lo,hi) in bands:
            mm, hh = bm[n]
            if ref_m>0 and ref_h>0 and hh>0:
                r = (mm/ref_m)/(hh/ref_h)
                print(f"  V3.2 ${tag} ${suffix}    band {n:<6} [{lo:5d},{hi:5d}]  L/HST = {r:.3f}")
PYEOF
