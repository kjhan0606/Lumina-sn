#!/usr/bin/env python3
"""C2: DDC15 v10400 outer X_Fe cap sweep — Fe II UV multiplet test (real cascade lever)."""
...
JOB  = sys.argv[1] if len(sys.argv) > 1 else "155756"

X_FES = ["0.15", "0.05", "0.01", "1e-3"]
CELLS = [(xfe, ROOT/f"logs/ddc15C2_{JOB}_ddc15C2_xFeO{xfe}/lumina_spectrum_formal.csv")
         for xfe in X_FES]
OUT = ROOT/f"figures/ddc15_C2_{JOB}_xFeOuter_sweep.png"
C_KMS = 299792.458
...
for (xfe, (lam, flu)), c in zip(data.items(), cmap):
    ax.plot(lam, flu*1e14, color=c, lw=0.9, alpha=0.85, label=f'C2 X_Fe_out={xfe}')
if a4 is not None:
    ax.plot(a4[0], a4[1]*1e14, color='tab:red', lw=0.7, alpha=0.55, ls='--',
            label='A4 baseline (uncapped)')
ax.set_xlim(1700, 9500)
ax.set_ylim(0, max(hflu[(hlam>2900)&(hlam<9000)])*1e14*1.15)
ax.set_xlabel('λ (Å)'); ax.set_ylabel('F_λ × 10¹⁴')
ax.set_title(f'C2 outer X_Fe sweep on DDC15 v_in=10400 (job {JOB})  — Fe II UV mult lever')
...
for (xfe, (lam, flu)), c in zip(data.items(), cmap):
    ax.plot(lam, flu*1e14, color=c, lw=1.3, alpha=0.9, label=f'X_Fe_out={xfe}')
...
ax.set_xlabel('λ (Å)'); ax.set_ylabel('F_λ × 10¹⁴')
ax.set_title('Ti II curtain [3000-4500]Å — outer Fe cap fills the curtain')
ax.legend(loc='upper right', fontsize=8); ax.grid(alpha=0.2)
...
for lam_m, lab in [(5972,'Si II'),(6355,'Si II'),(7773,'O I'),
    ...
    ax.text(lam_m, ymax*0.92, lab, rotation=90, fontsize=7, va='top', ha='right', alpha=0.5)
ax.set_xlabel('λ (Å)'); ax.set_ylabel('F_λ × 10¹⁴')
ax.set_title('Red continuum [5500-9500]Å — should drop with outer Fe cap')
ax.legend(loc='upper right', fontsize=8); ax.grid(alpha=0.2)
...
if a4 is not None:
    row = f"{'A4 base':<10s}"
    for lo,hi,_ in BANDS:
        rH = band_int(hlam, hflu, lo, hi); rM = band_int(a4[0], a4[1], lo, hi)
        row += f"{rM/rH:>10.3f}"
    print(row)
...
print(f"  {'X_Fe_out':<10s} {'ΔF_UV':>11s} {'ΔF_red':>11s} {'red/uv':>8s}")
for xfe, (lam, flu) in data.items():
    ...
    print(f"  {xfe:<10s} {dF_uv:>11.3e} {dF_red:>11.3e} {r:>8.2f}")
...
for xfe, (lam, flu) in data.items():
    print(f"  C2 X_Fe_out={xfe:8s}  band-log10-RMS = {band_log_rms(lam,flu):.4f}")
if a4 is not None:
    print(f"  A4 baseline (uncapped)     band-log10-RMS = {band_log_rms(a4[0],a4[1]):.4f}")
...
for xfe, (lam, flu) in data.items():
    ...
    print(f"  C2 X_Fe_out={xfe:8s}  RMS_bn = {rms:.4f}")
if a4 is not None:
    mlam, mflu = gauss_baseline(a4[0], a4[1])
    if mlam is not None:
        fm = interp1d(mlam, mflu, kind='linear', bounds_error=False, fill_value=np.nan)
        rms = np.sqrt(np.nanmean((fm(hlam_n) - hflu_n)**2))
        print(f"  A4 baseline (uncapped)     RMS_bn = {rms:.4f}")
