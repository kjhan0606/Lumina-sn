...
# C1: X_Ti sweep on A4 DDC15 v_in=10400 — Ti II curtain [3200,3500]Å 영향 검증.
# Hypothesis: A4 baseline X_Ti=1e-4 → [3200,3500] ratio 0.5-0.6 (40-50% over-absorb).
# UV deficit ≈ red excess (ratio 1.11) → UV opacity reduction drops both UV depth AND red excess.
...
xti=${X_TIS[$SLURM_ARRAY_TASK_ID]}
vi=10400
ro=0.00
de=9.0
label=$(printf "ddc15C1_xTi%s" "$xti")

work_root="$ROOT/logs/ddc15C1_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
src = "$SRC_REF"
dst = "$REF_DIR"
ro, de, vi_kms, X_TI = $ro, $de, $vi, $xti
...
# DDC15 profile (A4 baseline) — X_Ti only knob varied via env
def ddc15_profile(v, x_ti):
    if   v < 9000:           X_Si = 0.05
...
for s in range(n_shells):
    prof = ddc15_profile(v_mid_new[s], X_TI)
...
print(f"  built: v_in={vi_kms} X_Ti={X_TI:.2e}  rho0={rho0:.3e}  T_in_init={T_inner:.0f}")
print(f"  sh   v_mid    X_Si    X_Ti    X_Fe")
for s in [0, 2, 4, 6, 10, 15, 20, 25]:
    print(f"  {s:>2}  {v_mid_new[s]:>6.0f}  {X[Z_LIST.index(14),s]:.3f}  "
          f"{X[Z_LIST.index(22),s]:.2e}  {X[Z_LIST.index(26),s]:.2e}")
PYEOF

echo "=== DDC15 C1 [${label}]  X_Ti=${xti}  v_in=${vi}  +SkipSi ==="
