...
# Task #27 Phase 2b: feed the binned-J NLTE-rate ion stage into opacity, ON TOP
# of the (now correct) diffusive inner boundary.
#
# Phase 2a fixed the T_inner self-pin runaway (diffusive BC, T_inner 15420->10178K).
# Windowed tau attribution on the converged B (161987) then showed the residual
# 3500-3800 / 4000-4500 OVER-ABSORPTION vs SN 2002bo is carried by Co II (89%/62%
# at the line-forming shell ~5) + Fe II + Ni II. Cobalt's abundance is physically
# correct (DDC15 radioactive Co ~44% mass at 21d) -> NOT a distribution error.
# The carrier ion stage (Co II vs III) is set by phi_neb(W,T_rad), and W rails to
# 2.838 (>1, unphysical) under blanketing -> the diluted-Planck estimator biases
# T_rad low -> over-recombines Co/Fe to II -> over-blankets.
#
# The PHYSICAL cure already exists in-code: the NLTE solve computes bound-free
# photoionization R_bf = integral(4pi J_nu sigma_bf / h nu dnu) from the binned
# J_nu histogram (plasma.c:3370) + Milne recomb -> a correct ion balance from the
# real frequency-resolved field. nlte_writeback_ion_stage() (LUMINA_NLTE_OPACITY_
# IONSTAGE=1) writes that NLTE ion split back into ion_number_density so opacity
# uses it INSTEAD of phi_neb. This was NULL in task #29 (161967/68) ONLY because
# T_inner was 16000K then (both estimators gave III). With the diffusive BC the
# T_inner is correct, so this is now a clean test.
#
#   A (ARM=A): diffusive BC, opacity = phi_neb (W,T_rad)   [= reproduce B/161987]
#   B (ARM=B): diffusive BC + opacity = binned-J NLTE ion stage
# Success = B thins the Co II/Fe II curtain at shell ~5 (probe_b fIII up toward
# III) and shrinks the 3500-4500 over-absorption vs obs, WITHOUT re-breaking UV.
# Self-pin ON. Same keeper config (DDC15-strat, scatter, radeq-OFF, 200k, N_ITER=10).
#   sbatch --export=ALL,ARM=A scripts/slurm_phase2b_nlte_opacity_ionstage_ab.sh
#   sbatch --export=ALL,ARM=B scripts/slurm_phase2b_nlte_opacity_ionstage_ab.sh
...
DIFFUSE_BC=1   # both arms keep the corrected diffusive inner boundary

VI_KMS=9019
L_SCALE_ARG=1.0
N_PKT=200000
N_ITER=10
RUN_TAG="p2b_ionstage_${ARM}"
...
if echo "$GPU_NAME" | grep -qiE "H100|H200"; then
    BIN="$ROOT/lumina_cuda_p2a_h100"
else
    BIN="$ROOT/lumina_cuda_p2a_a40"   # sm_80: a40(PTX) / a100(native) / a10(PTX)
fi
...
echo "=== Task #27 Phase 2b NLTE-opacity-ionstage A/B  ARM=$ARM  DIFFUSE_BC=$DIFFUSE_BC  NLTE_OPACITY_IONSTAGE=$IONSTAGE  N_PKT=$N_PKT N_ITER=$N_ITER ==="
echo "Host: $(hostname)  GPU: $GPU_NAME"
echo "Binary: $BIN  Time: $(date)"
echo "Both arms: diffusive BC ON, self-pin ON. B feeds binned-J NLTE ion stage into opacity instead of phi_neb(W,T_rad)."
...
env LUMINA_BF_OPACITY=1 \
    ...
    LUMINA_NLTE_OPACITY_IONSTAGE=$IONSTAGE \
...
echo "  exit=$rc"
echo ""
echo "--- inner-BC config + write-back firing ---"
grep -E "\[INNER-BC\]|LUMINA_NLTE_OPACITY_IONSTAGE" stdout.log | tail -20
echo ""
