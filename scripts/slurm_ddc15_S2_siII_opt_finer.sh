#!/bin/bash
#SBATCH --job-name=ddc15_S2_siII_finer
...
# S2 (Task #222): Si II opt A_ul fine sweep around f=0.1 knee.
#
# S1 (156411) showed:
#   f=1.0 (ref): 0.2390   f=0.5: 0.2308   f=0.3: 0.2387   f=0.1: 0.2077  ← best
# Non-monotone (f03 regresses), knee near f≈0.1. S2 brackets:
#
#   array 0 — S2_f015:  SCALE6 f=0.15  (above the knee — confirm trend)
#   array 1 — S2_f010:  SCALE6 f=0.10  (replicate S1 best at 0.2077)
#   array 2 — S2_f008:  SCALE6 f=0.08  (push below knee)
#   array 3 — S2_f005:  SCALE6 f=0.05
#   array 4 — S2_f003:  SCALE6 f=0.03  (extreme damp — test ceiling)
#
# Tier 1 target ≤ 0.20.  Best at S1_f01 = 0.2077 (+0.008 from target).
...
LABELS=(S2_f015 S2_f010 S2_f008 S2_f005 S2_f003)
...
label="ddc15S2_${tag}"
work_root="$ROOT/logs/ddc15S2_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
echo "=== S2 [${label}]  Q1b r03 base + SCALE6 Si II opt λ∈[4000,7000] f=${SI_OPT_FAC}  N_PKT=${N_PKT}  N_ITER=${N_ITER} ==="
