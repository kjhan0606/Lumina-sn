#!/bin/bash
#SBATCH --job-name=v1_epoch_bracket
...
# V1: X1_b3p0 champion stack across 3 SN 2011fe epochs (homologous scaling).
# Tests whether the champion generalizes vs single-epoch fit artifact.
#
# Reference structure: data/tardis_reference_strat6_2011fe_physical (t_exp=17.2d, L=1.3e43)
# Homologous scaling: r ∝ t_exp, ρ ∝ (t_exp_base/t_exp)^3
# L_inner from 2011fe bolometric LC (Pereira+2013, Tab. 2 approximation).
#
# Cells:
#   0 — p−4.8d : t_exp=12.4d, L=0.55e43, vs Snifs sn2011fe_m4d8d.csv (rising)
#   1 — p+0.2d : t_exp=17.5d, L=1.30e43, vs Snifs sn2011fe_p0d2d.csv (B-max baseline)
#   2 — p+4.2d : t_exp=21.5d, L=1.10e43, vs Snifs sn2011fe_p4d2d.csv (declining)
...
if [[ "$SLURM_JOB_PARTITION" == h100* || "$SLURM_JOB_PARTITION" == h200* ]]; then
    ...
    BIN="$ROOT/lumina_cuda_a100_aulscale9"
fi

N_PKT=800000
N_ITER=12

TAGS=(p-4.8d p+0.2d p+4.2d)
T_EXP_S=(1071360 1512000 1857600)            # 12.4 / 17.5 / 21.5 days in seconds
L_INNER=(5.5e+42 1.3e+43 1.1e+43)             # erg/s, Pereira+2013 approx
SNIFS=(sn2011fe_m4d8d.csv sn2011fe_p0d2d.csv sn2011fe_p4d2d.csv)
...
# X1_b3p0 champion stack (unchanged from rerank #234 winner)
FE_FAC=0.3
CO_FAC=0.3
RED_FAC=0.3
SI_OPT_FAC=0.10
...
label="v1_${tag}"
work_root="$ROOT/logs/v1_${SLURM_ARRAY_JOB_ID}_${label}"
mkdir -p "$work_root"
cd "$work_root"
...
# Velocity grid identical (homologous) — only t_exp scales r
...
# Density: homologous scaling rho(t) = rho(t0) * (t0/t)^3 applied
# on top of the same v-profile (which is the v_inner-scaled X1_b3p0 family).
v_ref_kms = 11000
...
# Reference t_exp from src config — needed for the homologous (t0/t)^3 factor.
...
echo "=== V1 [${label}] N_PKT=${N_PKT} N_ITER=${N_ITER} t_exp=${t_exp_s}s L=${L_inner} ==="
echo "Host: $(hostname)  GPU: $(nvidia-smi --query-gpu=name --format=csv,noheader|head -1)"
echo "Binary: $BIN  Snifs: $snifs"
echo "Time: $(date)"
...
r = rms_bn(mlam, mflu, olam, oflu, gO)
print(f"  V1 ${tag}  MC RMS_bn vs Snifs $snifs = {r:.4f}")
PYEOF
