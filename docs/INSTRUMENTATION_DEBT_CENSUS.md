...
| Q3-5 | **census 수치에 재현 경로가 없다** — 157·12·5·5 어느 것도 커밋된 집계 스크립트가 없다. C7 자기위반의 일반형. 98→157 교정 자체가 "패턴이 수치를 60% 흔든다"는 실증인데 제3 패턴이 190 을 안 낸다는 보장이 없다. **집계한 트리 상태(커밋 해시)도 미기재** — 미추적 ~1,500파일의 더러운 트리 | HIGH |
