...
## 8. ★DET 판정런이 32코어를 잡고 **1코어로 돈다** (2026-08-20 실측, user 지시로 기재)

발견 경위: user 가 "판정런이 실제로 실행 중인지 검증해봐" 라고 지시 → 운전석이 5갈래로
검증하다 드러남. 결함 조사가 아니라 **생존 확인의 부산물**이다.

### 실측 (slurm job 321104, `syn101`, SH-A209-IDSEAL 판정런)

| 잣대 | 값 |
|---|---|
| 할당 | **32 CPU** + `gres/gpu:2` (`OMP_NUM_THREADS=32`) |
| 프로세스 | `lumina_cuda` PID 1332681, 경과 1:17:51 |
| `%CPU` | **108** (= 1.08 코어) |
| 스레드 | 35개 — **`R` 1개 / `S` 34개** |
| CPU 편중 | 주 스레드가 **531,435 tick**, 2위 스레드 1,836 → **99.3% 를 한 스레드가 태운다** |
| **GPU** | 0번 54,215 MiB · 1번 38,919 MiB 점유, **둘 다 utilization 0%** |
| 진행 확인 | CPU 시간이 벽시계와 1:1 증가(60초 사이 `01:24:24`→`01:25:24`) — **교착 아님** |

⟹ **32코어를 점유한 채 31코어가 논다.** GPU 2장도 93 GB 를 잡은 채 0% 다.

### 위치 (표적 특정은 미완)

[실측] stdout 의 마지막 표지는 `[CMFGEN-LINEHEAT]`(`src/lumina_cmfgen.c:2856`·`:2871`)이나
**그것은 6줄짜리 진단 출력이라 병목이 아니다** — 느린 것은 그 **다음** 구간이다.
[실측] 직전 런(`p1_…physcmp_named_reason`)의 stdout 도 `CMFGEN-LINEHEAT` 6줄로 같다.

★**정확한 hot spot 은 특정하지 않았다** — 스택 샘플링을 하지 않았다. 다음 조사 항목:
`lumina_cmfgen.c:2871` 이후의 실행 경로를 `perf`/`gdb` 표본 또는 계측 print 로 좁힐 것.

### 왜 대장 항목인가

- 이 구간이 **판정런 벽시계의 대부분**으로 보인다(직전 런 1h51m, 이번 런 1h18m 시점에도 미도달).
  판정 사다리의 **회전 속도를 직접 규정**한다 — 오늘 A100 2장을 6일 기다린 것과 곱해지면
  캠페인 전체의 임계 경로다.
- 자원 낭비가 이중이다: 31코어 유휴 + A100 2장(93 GB) 0% 점유. 클러스터 전체로 보면
  **다른 사용자의 대기를 우리가 만든다**(오늘 아침 우리가 GPU 부족으로 6일 예약을 받은 그 자원).

### ★★정정 (2026-08-20, 같은 날 — 스택·시계열 실측으로 위 함의를 뒤집는다)

**위 절의 수치는 전부 맞으나 함의가 틀렸다.** 운전석이 "32코어를 잡고 1코어로 돈다" 로
보고했고 user 가 그에 근거해 **"OMP optimize"** 를 지시했다. **OMP 는 틀린 처방이다.**

**무엇이 틀렸나 — `nvidia-smi` 단일 스냅샷을 잣대로 썼다** [실측]:
```
GPU util 1초 간격 20회:
100 100 / 100 100 / 100 100 / 100 100 / 100 100 / 100 100 / 100 100 / 100 100
  0 100 /   0 100 /   0 100 /   0 100 /   0 100 /   0 100 /   0   9 /   0   0 ...
```
**GPU 는 100% 로 일하고 있다.** 앞 절의 `utilization 0%` 는 **유휴 구간에 떨어진 순간
스냅샷**이었다. [[feedback_audit_the_yardstick_first]] 의 전형 — *클립·순간값을 잣대로
쓰지 않는다* 를 오늘 또 어겼다.

**주 스레드는 계산하지 않는다** [실측 `eu-stack` 표본 5회, 살아 있는 job 321104]:
```
#0  __sched_yield                       ← GPU 대기 스핀
#10 cuLaunchKernel
#12 cudaLaunchKernel
#13 positive_epoch_large_segment_kernel   (cmf_exact_multigpu.cu:1018, 런치 :1399)
...다른 표본: cuMemcpyDtoH_v2
```
⟹ `%CPU=108` 은 **CUDA 드라이버 스핀 대기**이지 직렬 계산이 아니다.
⟹ **일이 CPU 에 없다. OMP 병렬화로는 아무것도 빨라지지 않는다.**
31코어 유휴는 사실이나 **낭비이지 병목이 아니다.**

### ★진짜 표적 — 두 장치의 부하 불균형

[실측] 위 시계열에서 **GPU0 이 0% 로 떨어진 뒤 GPU1 이 6초 더 혼자 돈다.**
[실측] 이 커널은 epoch 배치 루프에서 `dim3 grid(count, shard.local_rays, 1)` 로
장치별 shard 를 돌린다(`cmf_exact_multigpu.cu:1393-1401`).
⟹ shard 분할이 불균등하거나 장치 간 동기 지점이 있어 **한 장치가 먼저 끝나고 논다.**

★**이것도 아직 가설이다.** shard 분할 규칙과 동기 지점을 읽지 않았다.

### 처분 (user 지시: **"다음에 이를 parallel optimize 한다"**)

- 이 단·이 캠페인에서는 **고치지 않는다**(SH-GATEREPAIR 는 잣대 수리 계약, `src` 0줄).
- 별도 단으로 발의. 착수 조건 = **hot spot 실측 특정 먼저**(추측 최적화 금지 —
  [[feedback_audit_the_yardstick_first]]: "X 가 병목"은 가설이고 분모 실측이 선행).
- 최적화 전 **바이트-parity 기준선**을 잡을 것: 이 구간은 판정런의 산출에 직결되므로
  병렬화가 결과를 바꾸면 안 된다([[feedback_tiered_acceptance_cost]] 의 마이크로픽스처).
