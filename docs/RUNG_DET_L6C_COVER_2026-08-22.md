...
## 14. ★§13-5 정정 (판정자 적발, 운전석 재측정 확증 — 2026-08-26)

판정자가 §13-5 의 **해석 2건**을 정정했다. 수치는 전건 일치했으나 **해석이 틀렸다.**
운전석이 소스·덱으로 재측정해 판정자 정정이 옳음을 확인했다.

### 14-1. "총 1행" → **중도절단 하한 ≥1** (hazard 총수는 미지)

```c
if(tau==0.0&&n_upper>0.0) diag->zero_opacity_emitting_rows++;   /* :14076 */
...
...
```
증가 직후 같은 호출에서 차단이 발화하고 트랜잭션이 죽는다 ⟹ **이 카운터는 구조상 1 을
넘길 수 없다.** "총 1행" 은 틀렸고 **hazard 총수는 미지**다.
★Z-O 사전등록 §Z-1 의 *"부분합 합산 금지·접두 부분합"* 경고가 정확히 이 자리다 —
운전석이 그 경고를 알면서 카운터를 총수로 읽었다.

### 14-2. "inversion 경계가 아니다" → **역. 경계 그 자체다** (운전석 실측 확증)

덱 실물 [운전석 실행]:
```
line_list.csv:669994   f_lu=1.409e-10  f_ul=-1.409e-10        (f_lu ≠ 0)
levels.csv  lower_global=2771 → 26,1,100 ,  E=6.5194425068 eV, ★g=8
            upper_global=5213 → 26,1,2542,  E=15.7617685743 eV, ★g=8
```
`a208_signed_sobolev` 의 τ ∝ `f_lu·λ·t·(n_l − (g_l/g_u)·n_u)` 이고 **g_l=g_u=8** 이므로
`τ=0` ⟺ **`n_l ≡ n_u` bit-동일**. 그리고 `χ ∝ (n_l/g_l − n_u/g_u)` 의 정확한 영점 =
S4-III 판정문이 정의한 *"population inversion 경계(순 불투명도가 정확히 0, 순수 방출)"*
**그 자체**다.
⟹ 운전석이 *"기전이 다르다(축퇴 상쇄가 아니다)"* 라고 쓴 것은 **틀렸다** — 기전은 **정확히
축퇴 상쇄**이고, PC-2 가 모델링한 **대수 계급과 같다.**

### 14-3. 그러면 무엇이 반증됐나 — **인구 공급자**이지 검출 대수가 아니다

판정자가 도구 import 없이 자작 census 를 돌려 PC-2 의 candidate {1:1,048,375 · 2:708,767 ·
3:215,038}·hazard {0,0,0} 을 **후보 수까지 정확 재현**했다 ⟹ **PC-2 의 검출 대수는 무죄다.**

반증된 것은 사전등록 §3-5 의 **[추정] 전제** — *"iter0 의 물질은 LTE-at-10020 이므로 iter0
위험은 오프라인 재현 가능"*. 실물은 다르다 [운전석 실측, stderr:419]:
```
[A2-INIT][SEED-MATERIAL] event=INIT_SEED_MATERIAL_PREDICTOR lane=DET
  r1_generation=1 te_generation=1->1 population_generation=1->2 te_manifest_preserved=1
```
**시드는 LTE 가 아니라 공표 seed Te 에서의 1회 NLTE solve 커밋분**이다. LTE(10020)이었다면
`n_l/n_u = e^{ΔE/kT} ≈ 4.4e4` 라 상쇄가 **불가능**하다 — 판정자 논거.
⟹ 증인은 **시드 NLTE ladder 의 bit-동일 준위쌍**이 만든 χ=0 이다.

★**"오프라인 census 를 더 잘 설계했으면 잡았다" 는 성립하지 않는다** — 시드 solve 산출은
GPU 런 없이 재현 불능이다. 맹점의 형태는 *표적을 NLTE 권한 안으로 옮기면서 인구 모델은
LTE-공급 시대의 것을 계승한 것*이다.

### 14-4. 처분 변경 — WITNESS 는 S4-III 추론의 **반박이 아니라 확증**

운전석이 *"inversion 경계가 아니므로 Z-O 의 소스 추론을 반박한다"* 는 취지로 읽은 것을
철회한다. 정정된 사실:

- `tau_validity=2 = A208_EXACT_ZERO` ⟹ 출판이 **정당한 정확 0 으로 등록**한 행이다
  (coverage 결손이 아니다 — Z-O §2 가 가른 (a)/(b) 중 **(a)**).
- inversion 경계 **맞다** ⟹ S4-III 의 [소스 추론]이 **첫 in-vivo 전 필드 실측으로 확증**됐다.
- ⟹ Z-O 표의 *"Z-1 이 '물리적으로 정당한 행' 을 보이면 **A(진단 격리)** 유력"* 의 **전건이
  표본 1행에서 성립**했다.
- ⚠**단 1행은 census 가 아니다.** Z-1 전수 계측 없이 Z-2 를 고르지 말라는 Z-O 자신의 규율은
  그대로 유효하다.

### 14-5. ★Z-1 설계에의 신규 입력 (이 단의 기여)

hazard 는 **LTE 인구가 아니라 시드 NLTE 물질의 성질**이다(§14-3) ⟹ **Z-1 census 는 오프라인
덱 산법으로 불가능하고 라이브 커밋 물질 위에서 돌아야 한다.** Z-O 가 이미 처방한 census 런
형상(`DIAG=2` + `INDEPENDENT_CAPTURE=0`)은 차단 가드가 `independent_capture &&` 조건부이므로
완주할 수 있을 것이다 [소스 추론 — 실행 미검증].
...
§13-5 의 두 해석 오류는 **증인 행의 필드를 읽고도 `g_l=g_u` 를 덱에서 확인하지 않은 것**이
직접 원인이다. `tau_raw=0` 과 `exact_zero_provenance=0` 만 보고 "축퇴 상쇄가 아니다" 로
건너뛰었다 — **분모를 실측하지 않고 비를 말한 것**(`feedback_audit_the_yardstick_first` 계급).
카운터 오독은 Z-O 사전등록이 **명문으로 경고한 함정**을 그대로 밟은 것이다.
