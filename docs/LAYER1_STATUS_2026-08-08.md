...
| **L1-2** 세대 장부 위상 정합 (=**R7**) | commit/view 직후 a208·a209 발행 → 그 다음 A2-10.  실패 시 `(T_e,t)` 보존 | **폐합** `9012995` | `[A2-10][PRE] lane=MC te=1 r=1 line=1 o=1 e=1 m=1`, 검사기 `violations=0` |
| **L1-3** 결정론 부트스트랩 + 두 팔 합류 | 최소조건 ①세대 단일장부 ②pure a209 ③결정론 line-J̄ | **부분 폐합** | ②=R7 ✓ · ③=R6 ✓ · ①=R5 **미착수**.  두 팔 합류는 **E단 대기** |
| **L1-4** 계측 배선도 재작성 | 3열 어긋남 지도(J^MC · J^det · J^CMFGEN) | **미착수** | L1-3 완주 전제 |
| **L1-5** 물질 입력 물리화 | 시작 상태를 물리로 | **1/5 폐합** | 아래 |
| **L1-5b** 비열적 퇴적 물리화 | 비열 excitation 부재 · 상수효율 처방 | **미착수** | Γ단이 **소유권**만 폐합 |
...
| 단 | 무엇이었나 | 상태 |
|---|---|---|
| **Γ** 감마 침착 소유권 | 결정론 팔의 방사성 가열이 **항등 0** 이었다.  소비자가 0 을 "할 일 없음"으로 읽음 | **폐합** `5450518` |
| **R6** 결정론 정본 line-J̄ | 생산자는 있으나 commit 에 안 실려 a209 가 막힘 | **폐합** `3ca077d` |
| **격자 포함 계약(B)** | 생산자·소비자가 **같은 격자**인데 중개 격자가 양 끝을 파괴 | **폐합** `2e26c2f` |
| **E** 사건 측도 | 같은 부재에 세 지점이 서로 다르게 행동 — CPU MC 전송 전멸 | **발주 중** |
...
| # | 항목 | 상태 |
|---|---|---|
| 1 | R1 물리적 seed T_e 단일 선언식 | 평면 Eddington **실측 기각**(37.4% vs 22.9%).  CMFGEN 은 회색 moment 를 푼다.  발주서 있음 |
...
| 4 | E단 OFF 경로(legacy argmax) 존폐 | **ME2**(두 생산자 측도 차이) 측정 후 결정 |
| 5 | R6 적용범위 30% 가 물리적으로 충분한가 | 창 확장은 물리 결정 |
