...
## ★감리 반영 (2026-08-20, 독립 fresh 컨텍스트 Fable)

감리 판정 **수정** — 판정의 실질(**PASS·전폐합·재런 불요**)은 감리자의 **전면 독립 재실측**에서
전부 재현되어 유지되고, 문구 2건을 정정한다. 게이트 판정값이 바뀌는 지적은 없다.

### R1 — P1 행의 `[실측]` 문장이 거짓이었다 (정정 완료)
[실측·운전석 재확인] `src/lumina_plasma.c` 에 선재 경고 **CPU 41 · GPU 1** 건이 있다
(행 1131·1726·4708–5812·11028–13562·18302·22707·22837).
**이 단의 diff 구간(9100–9147)에는 0건** — 의도한 사실은 참이나 **적힌 문장은 거짓**이었다.
`[실측]` 라벨 규율상 원장 기재 전 정정이 필수라는 감리 판단을 수용한다.

### R2 — 증명 불가능한 **과정 주장**을 사실로 적었다 (계급 강등)
"판정자 제3 구현을 verifier 코드를 **보지 않고** 부록 A-1 만으로 작성했다" 는
**어떤 산출물로도 검증 불가한 자기 증언**이다 ⟹ **[판정자 증언(검증 불가)]** 으로 강등한다.

★**그러나 B2 의 확정은 이 주장에 의존하지 않는다.** 감리자가 **verifier·C 코드를 읽기 전에**
부록 A 문면만으로 작성한 **제4 구현**이 `49a7fa33…`(비행 CSV) · `d14856a5…`(source 0x7) ·
`440daeca…`(P4 픽스처)를 **전부 재현**했다 [감리 실측]. SHA-256 역상 불가에 의해
"생산 해시가 실격자의 부록 A 함수" 라는 사실은 과정 주장 없이 확정된다.

또 `NC_driver_demo` 의 `v1→vX` 실험이 입증한 것은 **`run_a2_09_selftest.py`(P4 교차검증)**
의 독립성이지 `verify_a209_grid_manifest.py` 가 **아니다** — 인용 사거리를 P4 로 한정한다.

### R3 (부기) — B4 는 사전등록 시점부터 **사각**이었다
[감리 실측] `job.slurm:52` 가 targeted 모드에 outer iteration **1** 을 강제하고 사전등록이
그 구성을 지정했으므로, B4 의 조건부("꼬리 반복이 존재할 만큼 진행 시")는 **처음부터 발화 불가**였다.
판정 결함이 아니라 **사전등록 위생** 항목이다 — ★**선택한 런 구성이 낼 수 없는 기대치는
등재하지 않는다.** 차기 사전등록에 적용.

### R4 (부기) — 오프라인 집행 동등성의 잔여 미입증 1건
계산노드의 python3 버전을 **모른다**. `§L` 스테이징 봉인 수리 단에
**"검사기 실행 인터프리터 버전을 `RUN_FOOTER` 에 기재"** 한 줄을 추가해 이 채널을 닫는다.

### 감리가 판정보다 **더 강하게** 확인해 준 것
부분폐합 분기 행의 전제 불성립 — 그 행이 열거한 후속 게이트(`INVALID_GRID`/`INVALID_VALUE`/
`STALE`)는 **모델 내부** physics_comparison 게이트로 스냅샷 tmp 삭제 → 산출물 0 을 낳는 부류다.
실제 죽음은 모델 **밖** slurm 사후 게이트였고 산출물 3종이 실재하며 B2 가 회수됐다
⟹ **강등 사유였던 "B2 미회수" 가 존재하지 않는다.**

그리고 [감리 실측] 스테이징된 검사기 사본은 **어떤 커밋된 트리에도 존재한 적 없는 화석**이다
(정본의 최초 커밋 `a00b991` 이 이미 예외 조항을 포함한 상태로 `A`).
