...
|---|------|--------------|------------------------------|-----------------------------------|
| **1** | **Isolate P1** | Run obs with `OBS_CONTONLY=1` (lines off, pure continuum) on the fine field; compare obs-continuum peak vs static-freqres 6782 | obs-cont peak ≈ 6800 → bug is in the *line* path (P-Cygni source), NOT continuum → go to step 3 | obs-cont also too-red (≈11000) → bug is in the **Doppler continuum march** → step 2 |
| **2** | **Fix P1 continuum** | Audit `cmf_obs_march_sob` 956-968: (a) `nu_cmf` interp clamping (P2) — skip sub-steps where `nu_cmf` outside `[nu_lo,nu_hi]`; (b) verify D³ vs D (Lorentz invariant I_ν/ν³ → emergent needs D³ on S, check sign of μ on in/out bound); (c) compare S_c to static's `S_fixed+r·J` per shell at a fixed ν | obs-cont peak → 6800 ≈ static; red-edge upturn gone | If D³ is correct and interp fixed but still red → the static and obs use different continuum definitions; unify by feeding obs the static `S_fixed` directly (interp S not χ,B,J separately) |
