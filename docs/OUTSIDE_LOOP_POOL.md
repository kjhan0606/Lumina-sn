...
| I3a | └ **Co IV** | 1 | **★해소 (2026-08-06 재판정)** | 정본 `validation/layer1/L1_I3A_VERDICT.md`. 구 91%는 **교차-vintage 아티팩트**다 — Co IV phot 은 `18oct00`=407준위 **전부 type 1**(Seaton 3파라미터·1,221쌍) 대 `19apr23`=264 type1+**140 type 20**(표·40,025쌍)로 **표현 방식 자체가 다르다**. 동일 vintage 실측: 구운 σ 대 CMFGEN 식(`newsubs/sub_phot_gen.f:412-419`, EDGE=`GS_EDGE+EXC_FREQ`) 중앙 **1.03e-07**(`_ftos`/18oct00 1,000준위 398,612점 · `_ophys`/19apr23 type1 557준위 243,132점 동일). 1e-6 초과는 **준위당 정확히 1점**이고 그것은 임계빈에 대한 **내 비교자**의 통짜 평균 탓(베이커는 A2-05 부분빈 정확 적분). ⚠미결: type 20 **140준위 미검증**(보간 규약 대조 필요) |
...
### ★★★I20 공기파장 규약 — **신규·확정 결함** (2026-08-06, R-5 산출)

정본 = `validation/layer1/L1_R5_VERDICT.md`. 검수 미실시(운전석 L1 실측).

R-5(구 I2 상위 기여 이온 `_ftos` 재측정)가 찾은 것. CMFGEN osc 파일은 헤더가 명시하듯
**λ>2000Å를 공기파장으로** 적는다(*"Wavelengths in air for lambda > 2000 Ang, else vacuum"*).
CMFGEN 자신은 면역이다 — `genosc_v6.f:278-286`이 **f와 준위 인덱스만** 읽고 A·λ 토큰은
건너뛰며, ν는 `:205`에서 준위 에너지(진공)로 재계산한다. **Lumina는 λ열을 그대로 쓴다.**

| 축 | 실측 |
|---|---|
| 덱 λ vs 원본 λ열 | **비트 동일** (median 0, max 2.2e-16) |
| 덱 f 레시피 | `f = A·(g_u/g_l)·λ_file²·m_e c/(8π²e²)` — 재구성 오차 **6.7e-16** |
| 오염 이온 | **45 / 58** |
| 오염 선 | **635,169 = 덱 2,220,953선의 28.6%** |
| 선 위치 편이 | **82–85 km/s** (전 이온 일관) |
| f 계통 오차 | **−5.6e-4** (= 2×(n_air−1)), λ>2000Å 구간 |
| Lumina 내 공기↔진공 변환 | **0행** (`air.?to.?vac\|n_air\|refract\|edlen` 전수 grep) |

**통제**: *같은 이온 안에서* λ≤2000Å 대역은 3–13e-9로 깨끗하고 λ>2000Å 대역만 2.76e-4다.
준위 에너지 결함이면 두 대역이 함께 어긋나야 하므로 **원인이 λ열 규약 하나로 특정된다.**
비오염 13이온(Fe I·II·III, Cr III, Ti II·III, S I·II, Ca I·II, Co I, Si II·III — 주로
`19apr23` 계열)은 전 구간 진공이라 **vintage별로 규약이 다르다. 이온 단위 판별 필수.**

| 축 | 값 |
|---|---|
| posedness | WELL |
| outcome | **DIFFER (확정)** |
| kind | CONVERSION (임포트 결함 — 계보·커버리지 아님) |
| disposition | **기재**(수리는 별도 결정) |
| evidence_status | VALID (자기완결 census, 통제 동반) |

**크기**: τ·rate 영향은 5.6e-4로 무시 가능. **선 위치 82–85 km/s가 실질** —
SN Ia 선폭 ~10⁴ km/s의 0.8%지만 **한쪽 부호이고 덱의 28.6%에 걸린다.**
narrow-band 스펙트럼 잣대·P-Cygni 최소 위치에 직접 들어간다.

부수: **e 상수 3원 대조** — 참값(SI-2019) `4.803204713e-10` / CMFGEN `4.80320427e-10`
(CODATA-2006) / Lumina `4.80320425e-10`(계보 불명). 상호차 A로 8.33e-9, 참값 대비
둘 다 ~1.9e-7. **검증엔 CMFGEN 값, production은 참값**(user 08-06). → I9.
...
### ★★★A2-02 좌표·격자 — **BLOCKED** (2026-08-05 17:20). 사다리가 계약대로 작동해 설계 결함을 적발

구현 Codex(합집합 원장 + 사다리 러너 + fine dump builder[A2-02B, 재발주 1회 — 1차는
샌드박스 exec 장애로 빈손]), 검수·실행·수리 운전석(lageunha).

```
builder      PASS  fine 192,922빈 · 50셸 · bf 96행 · line 120행 (npz 3e2b07b4…)
             수리 1: CHIETA dnu 왕복 허용치 3e-12 → 3e-6 — 실측 캘리브레이션
             (dnu 정의는 기하 edge 폭이 맞음 1.17e-6, 대안 3개는 2.7e-3 탈락;
              중심 격자는 8.9e-16 완벽 log-grid. 데이터 저장 정밀도가 잣대였다)
postcheck    PASS  스키마 12키 · j_state 4상태 · s44+ 전부 state 4 · 해시 결박
사다리       rc=3 BLOCKED  (수리 2 후에도)
             수리 2: 구조적 state-4 레코드를 invalid-eligible 이 아니라 제외로
             (s44+ 30레코드가 자동 FAIL 을 만들어 어떤 빈 수도 통과 불가였다;
              state-3 MC 공백은 fail-closed 유지 — 4상태 구분의 취지대로)
```

수리 후 8000→16000: **Γ·band_J·χ·η 전부 PASS** (max 1.1e-3 / 7.3e-4 / 1.3e-3 / 7.6e-4).
**Jbar 만 FAIL — 그리고 이것은 도구가 아니라 설계의 진실이다:**

| 발견 | 실측 |
|---|---|
| ★합집합 오염 | 하한 2.998e6 Hz(λ~100 m)의 출처 = `bound_bound_line_frequency_profile`. **덱 line list 의 퇴화 전이**: ν<1e10 Hz 382선·<1e13 34,945선. 최악 = Ni I 고준위 인접전이(l461→u462 등), **A_ul ~ 1e-16 s⁻¹** — 물리 무의미 장부선이 격자를 10.1 자릿수로 확장, 광학·UV 해상도를 ~4× 굶김 |
| ★Jbar 잔여 invalid 18 | 전부 그 퇴화 선들(예: `Z28:i0:l460:u462` 실측 표와 일치) — J 커버리지 밖 support |
| ★Jbar 실질 실패 | 유효 42레코드 중 **11건 >1%**, max 7.6% = `s8 Fe II l61→u1308` 등 **실제 UV 선숲 선들**. 전역 성긴 빈 위의 profile-가중 J̄ 는 16000빈에서도 수렴하지 않는다 |

⟹ **처분: BLOCKED 는 계약이 옳게 낸 답이다.** §4 말미("8000 대 16000 도 실패하면
BLOCKED. 1000 을 관성 유지하지 않는다")가 정확히 이 상황을 예정했다. 해소는
**발주서 §4·§2 의 개정**을 요구하며 §7 말미("변경은 원장 근거와 별도 발주")의 경로로만
가능하다. 개정 후보 3 (Codex 저작 발주로 넘김):
- (A) 합집합의 선 소비자 범위를 물리 창으로 계약화 (퇴화선 제외 목록·음성대조 동반)
- (B) line J̄ 를 전역 재빈이 아니라 **선별 estimator**(§2.2 허용 파생 캐시, generation
  결박)로 계산 — ARTIS jbar 방식과 동형, 물리적으로 옳은 아키텍처 후보
- (C) J 저장을 fine/native 격자로 (50×192,922×8B = 77 MB, CPU·GPU 32GB 모두 수용)

**A2-03 은 사전등록대로 발주 보류** (프롬프트의 "A2-02 BLOCKED 면 무효" 조항).

#### ★★개정 채택·집행 — **전역 격자 4000빈 확정** (2026-08-05 17:56)

개정안 `docs/ORDER_A2_GRID_AMENDMENT_BY_CODEX.md`(Codex 저작·운전석 검수 반박 0)
**user 채택** → 집행 A2-02C(Part 1 = §4.2 단계 1~5, lageunha).

```
합집합 재구축   2,220,953선 → BB_IN_DOMAIN 1,891,662 · 제외 329,291 (판정 산출물)
음성대조 6/6    퇴화선 강제 재진입 FAIL · 제외선 jbar 조회 거부 · hash 결박 ·
                경계 오분류 · ★세기 floor 로 약한 선 제외 = FAIL (clamp 규율 대조) ·
                continuum 재활성화 FAIL
builder v2      fine 196,510빈 (2.53 자릿수 도메인) · cohort 74행
                (종전 유효 42 이관 + s8 Fe II l61→u1308 필수 고정 + 층화 보충)
사다리 (Jbar 제거, 전역 4지표):
   1000→2000   Γ P(8.5e-3)  band_J F(1.1e-2)  χ F(2.1e-2)  η F(5.2e-2)
   2000→4000   Γ P          band_J P          χ F(2.2e-2)  η F(1.3e-2)
   4000→8000   Γ P(1.4e-3)  band_J P(5.0e-4)  χ P(8.3e-4)  η P(7.9e-4)  ← 최소 통과
   8000→16000  전부 P
⟹ selected_bins = 4000   (규칙: 최소 N whose N→2N 전부 PASS)
```

★수치 교훈: 오염 10.1 자릿수 격자에서 16000빈으로도 χ가 1.3e-3 겨우 통과했는데,
정화된 2.53 자릿수에서는 **4000빈이 그보다 여유 있게 통과** — 합집합 정화가 곧
해상도였다. 현행 1000빈은 amended 도메인에서도 FAIL (band_J 1.1e-2·χ 2.1e-2·η 5.2e-2)
— "1000 관성 유지 금지" 경고가 옳았음이 수치로 확정.

잔여 (§4.5): estimator effort 사다리 (수렴장 단일-iter capture, user 결정) +
§3.3 정합 게이트 → 완료 시 A2-02 폐합.

## ★인수인계 (2026-08-05 03:20, user 로그아웃 — 헤딩 복구: 17:20 편집이 헤딩을 삼켰음)
...
## ★★★층 구조 재분류 + `L0_VALIDATION_CLOSED` 선언 (2026-08-05 저녁, user 감사 지적)

user: *"0층 검사에 이렇게 많은 계산이 필요함? 원래 0층은 cmfgen 의 데이터 동일성과,
이를 제대로 코드에서 읽고 사용하는지 확인하는 것인데... 4-5층에서 실제로 스펙트럼
감사하는 듯한 느낌."*

**진단 인정. 장부 분류 오류였다.** 실측 분해:

### 층 0 본연 (입력 동일성 + 판독 정합) — 전부 폐합, 전부 저비용

| 계약 | 검사의 실체 | 비용 |
|---|---|---|
| H-TRANSFORM · GEN-GUARD · K-SHAPE · K-FRESH | 해시·형상·fail-closed 판독 | 각 <5분 |
| Z-INERT · D-BUILD · TE-DEAD | 비트 대조·빌드·grep | 각 5-15분 |
| CONFIG-PREC · NE-NAMING · DECK-FOSSIL | 선언 정합·provenance·quarantine | 각 5-15분 |
| A2-00 원장 자격 · 분류기 확장 | 파일 해시 manifest (143MB ~10초) | 분 단위 |

**층 0 본연 = 13계약, 총 계산 ~1.5시간.** 원 정의에 정확히 부합한다.

### 무거운 계산의 정체 — 층 0 이 아니라 **수리 캠페인**

TRAD-FIX(층 0 입력계약 위반 발견)의 수리로 user 가 A안(J_ν 직접)을 채택하며 A-2 가
태어났고, **A-2 는 감사가 아니라 재설계다.** 사다리·capture·estimator 는 전부 A-2
몫인데 장부가 층 0 아래 계속 기재했다. A-2 게이트 L-1bf~L-8 은 rate·population·
χ·η·수송·창발 스펙트럼 대조 — **층 2~5 성격**이다. (다층 검증 자체는 user 지시
*"A-2 는 다층 구조라 모든 층에서 검증하도록"* 대로다. 틀린 것은 분류다.)

### 처분

1. **A-2 를 층 0 에서 분리** — 독립 항목 「수리 캠페인 A-2 (J_ν 소유권)」.
   게이트 층 귀속 정직 표기: L-0=입력 표현(층 0 인접) / L-1bf·bb·L-2=층 2 /
   L-3~L-5=층 2-3 / L-6=층 3 / L-7·L-8=층 4-5.
2. **`L0_VALIDATION_CLOSED = yes`.** 층 0 본연 13계약 전부 폐합·커밋·푸시 완료.
   입력측 유일 잔여 = **O-PHYS 부재**(CMFGEN 재실행: WRITE_RATES·CHI/ETA·온도 풀기·
   freeze 공개) — 층 0 의 검사가 아니라 **원장 조달**, 별도 발주(user 결정 사항)로 이관.
3. **층 0 급 검사의 비용 상한 감각 = 분 단위.** 시간 단위 계산이 필요해지면 그것은
   층 0 이 아니라는 신호다 — 재분류부터 하라.

---

### ★★★★A2-02 **폐합 — PASS** (2026-08-05 22:24, 개정 2회·사다리 2회 경유)

최종 판정 `a2_02c_estimator_effort_gate2_v3_result.json`: **decision=PASS, rc=0**.

```
전역 격자        4000빈 확정 (912c0c0)
prefix (1.2M/2.4M)   z 0/54 이상치 · 분산교정 std=1.14∈χ²구간 · 집계 δ 0.349% z_agg=0.95
independent          z 0/55 이상치 · 집계 δ 0.141% z_agg=0.38
magnitude 한계   §6.3 CI-자격 집행: CI 0.72% > limit/3 → UNDERPOWERED 기록 (FAIL 도 PASS 도 아님)
                 — ladder2 실행 전 manifest v2 에 사전등록된 그 처분
```

#### 판정까지의 사다리 계보 (전부 보존, amends_after 사슬)

| 단계 | 판정 | 무엇이 밝혀졌나 |
|---|---|---|
| (200k,400k) v1 | BLOCKED | per-line 1%/0.2% 는 비편향 estimator 에 잘못된 검정 — 잡음 **순수 포아송 확정**(4중 판별: z mean −0.21·std 0.89 / 부호 p=0.32 / 관측÷예측 median 1.00 / 예측 10.2%↔실측 9.9%) |
| 개정 2호 (user 승인 B안) | — | z-게이트 + flow-가중 집계 + UNSAMPLED 사유제외·분모유지 |
| (200k,400k) gate2 | BLOCKED | 집계 median 이 §6.3 자격 미달 — ③ 필요성 확정 |
| ③ (1.2M,2.4M) gate2 | BLOCKED | ★prefix z=−27 1건 → **등분산 가정 결함 적발**: 옳은 분모는 항등식 `Var(P−2P)=Var(2P)` 의 σ_2P. 희귀 segment 지배 선(s8:line444190)에서 σ_P 만으로는 폭발 |
| 도구 v2 (운전석 수리 3) | — | 정확 분모 + **z-분산 교정 검정 신설**(σ-부풀림 적발 — self-test 백스톱이 1차 수정의 구멍을 잡아줌) + §6.3 CI-자격 집행 |
| ③ 재판정 v3 | **PASS** | 전 게이트 정합 |

#### 이 단계가 남긴 실측 자산

- **벡터화 61×** (243.8s→4.0s 파일럿) · production replay 188s/6.2M seg/s (국면B 60워커)
- **production 선별 J̄ 잡음 = median 12% @ 표본 66** — A2-06 damping 설계 입력으로 등재
- capture 인프라: 게이트 계측(src 17줄+2파일) · 55GB×2 capture(GPFS) · offline replay oracle
- 게이트 아키텍처 v2: z(옳음)·z-분산(σ교정)·집계(§6.3 크기) — 상호 회피 차단 3중

#### 검수 발견·수리 총결산 (A2-02 전 과정)

운전석 수리 7: CHIETA dnu 허용치 캘리브레이션 · 사다리 구조적 state-4 제외 ·
gate_off_compare 공허 PASS 차단 · replay ID 계약 캘리브레이션 · 벡터 경계 off-by-one ·
정확 z 분모 · §6.3 집행+분산검정. 파일럿 설계 결함 1(end-to-end 접합 미검증 — 이후
파일럿은 접합 1회 의무). Codex 재발주 1(샌드박스 장애 빈손).
...
### ★배터리 병렬화 **폐합** (2026-08-06 00:32) — 25~30분 → 40초

`scripts/run_gate_battery.py` 통합 러너 (D 19 + K 7 + Z + CP 4 = 36케이스 병렬,
픽스처 입력-해시 캐시, 내장 tee, --serial 폴백).
동일성 게이트: 직렬 대 병렬 PASS/FAIL 표 **IDENTICAL** (203초에 양방식 완주).
