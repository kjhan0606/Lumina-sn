...
### 4.1 잡 226529 (추적 빌드) 착지 — 진단 완료, 결함 넷

추적 빌드가 `LOAD-STAGE` 표지를 **하나도 못 찍고** 죽었다 ⟹ 로더 이전, env 표면 검사에서.
`BLOCKED_UNKNOWN_ENV: 15 LUMINA_* not in src/env_universe.h`.
15종을 전수 실측하니 **한 덩어리가 아니라 두 부류**였고, 그 과정에서 결함 넷이 나왔다.

| # | 결함 | 실측 | 처분 |
|---|---|---|---|
| D1 | **전집 정의가 소비자 계층 하나를 빠뜨림** | `LUMINA_MODEL_DIR` 은 src 가 안 읽지만 **셸 12개 스크립트가 소비**. 같은 부류 **13종** | derive 에 5번째 갈래 추가, 483→**496** |
| D2 | T3 가 죽은 노브 14종을 상속 설정 | 14종 전부 `getenv` 참조 **0** | 상속분만 여과(자기 설정은 STRICT 유지) |
| D3 | **Makefile 이 헤더 9개를 의존성에서 누락** | 선언 12 · 실제 include 21. `make` 가 "up to date" 라 하고 옛 전집 유지 | HEADERS 21종으로, 대조기 신설 |
| D4 | **런처 주석이 A/B arm 을 틀리게 지목** | `_jnu4` links sha=3596b927… ≠ `_ophys` 계열 f395a07c… ⟹ vintage 교락 | `_ophys` vs `_ophys_exacthyd` 로 정정 |

**D3 이 가장 넓다.** 빠진 9개에 **계약 헤더 3종**이 있다
(`gpu_radiation_field_contract` · `gpu_physics_kernels` · `lumina_cmfgen`) —
A2 계약 헤더를 고쳐도 리빌드가 안 일어났다는 뜻이다. 증상이 "오류 없이 옛 결과"라
과거 A/B 중 무엇이 이 함정을 밟았는지 알 수 없다. `python3 scripts/check_makefile_headers.py`.

**D4 는 D1~D3 보다 조용하고 더 위험하다.** 런은 정상 종료했을 것이고 결과도 나왔을 것이다.
다만 그것이 "수소꼴 A/B" 가 아니라 "vintage + 수소꼴" A/B 였을 뿐이다.
잣대 감사를 제출 **전에** 하지 않았으면 그대로 대장에 들어갔다.

수리 게이트(전부 오프라인 선등록·PASS): G1 전집 483→496 정확히 +13·삭제 0 · G2 무모순 0 ·
G3 T3 미등록 15→0(죽은 노브 14 여과) · **G4 음성대조 3종**(오타 주입 검출 · 여과 후 재설정 검출
= 자가치유가 STRICT 를 무력화하지 않음) · G5 무해성(14종 getenv 참조 합계 0, 구성상 불변).

판정런: **228594**(`_ophys`) · **228595**(`_ophys_exacthyd`) — 단일 변수 `CMFGEN_EXACT_HYD`.

### 4.2 판정런 착지 — env 수리는 성공, 그러나 ★HEAD 가 런을 시작 못 한다

228594/228595 둘 다 `unknown=0 universe=496`, **LOAD-STAGE 표지 7개 전부 통과**.
env 수리는 사전등록대로 작동했다. 그런데 둘 다 ~50초에 `exit 1`(신호 아님, MaxRSS 314MB).

진단을 붙이니(`[K-FRESH][FATAL]`) 즉시 말했다: **`POP_INVALID_TE`, `T_e_gen=0`**.

**A2-07 과 K-FRESH 가 순서에서 충돌한다.** A2-07 은 `compute_plasma_state` 가
발행된 T_e(generation ≥ 1)에서만 돌게 하는데, K-FRESH 는 그것을 **반복 루프 앞**에서
부른다 — 그 시점 세대는 초기값 0 이고, 세대를 올리는 곳은 루프 안뿐이며 그것도
그 반복의 복사장을 먼저 요구한다. ⟹ **원리적으로 성공할 수 없는 호출**이다.

계보: `a97d0e1^` 은 루프 안에만 있었고 `a97d0e1` 이 루프 앞 호출을 **추가**했다.
「고리 밖 감사 층 0: 계약 10건 폐합」 커밋이 **런을 닫았다**.

정본 `docs/BLOCKER_KFRESH_TE_GENERATION.md` (기전·재현·반증·수리 선택지 A/B/C).
**수리는 물리 계약 순서 문제라 user 판정 대기.** 검사 완화는 클램프 판별식에 걸려 제외.

★부채: **`main()` 기동 경로를 실행하는 게이트가 하나도 없다.** 배터리·selftest·픽스처는
전부 하위 함수를 직접 부르므로 이 충돌을 통과한다. 계약을 10건 닫고도 "런이 시작되는가"
를 아무도 묻지 않았다. 후보 수리: `scripts/t3_cpu_repro.sh` 를 기동 연막 게이트로 승격.
...
0. **★★★BLOCKER — HEAD 가 런을 시작 못 한다** (`docs/BLOCKER_KFRESH_TE_GENERATION.md`)
   K-FRESH ↔ A2-07 순서 충돌. 수리 선택지 A(호출 제거)/B(T_e 발행 단계 신설)/C(검사 분리).
   **이것이 풀리기 전에는 어떤 판정런도 불가능하다** — 아래 항목 전부가 여기에 걸려 있다.
