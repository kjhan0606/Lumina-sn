...
- 층 1 R-5 국면부터 Codex L2를 **5회** 발주했고 **다섯 번 다 최종 답변 없이 종료**했다
  (exit 0, 도구 호출 0). 범위 축소·예산 리셋·`-o /tmp`·순수 판정 과제 전부 무효였다.
  **원인은 작업 디렉터리였다** — `/tmp` 에서 사소 과제를 주니 즉시 답했다.
  이 저장소는 미추적 ~1,500파일 + 250MB 바이너리 수십 개라 Codex 가 시작 시 훑다가
  소진되는 것으로 보인다. 나는 5회 동안 **과제 형태만 바꾸고 cwd 는 한 번도 안 바꿨다.**
  ⟹ 발주는 **저장소 밖 cwd + 증거 전량 인라인 + 판정만**. 문의는 반복 문제에 한정
  (user 2026-08-07, `feedback_codex_only_for_recurring_problems`).
