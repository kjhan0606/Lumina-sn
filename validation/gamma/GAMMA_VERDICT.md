...
| 철회 | 사유 |
|---|---|
| **Γ2-b PASS** | 사전등록은 **byte-동일**을 요구했는데(`RUNG_GAMMA...md:131,136`) 판정은 **순서·셸 위치를 버린 다중집합**만 비교했다.  값이 셸 사이에서 교환돼도 통과한다 ⟹ **무효** |
| **단 전체 PASS** | Γ1 이 요구한 NC2·NC4 와 Γ4 가 미실행이다.  미실행을 기재했다고 통과가 되는 것은 아니다 |

**남는 것**(부정되지 않음): 소유권 설계 자체는 코드에 실재한다 —
epoch 검사·매니페스트 재검산·이중발행 거부(`lumina_plasma.c:16418,16461`).
NC1/NC3 의 양방향 시연도 유효하다.

**다시 해야 할 것**: Γ2-b 를 **위치 결박 byte 비교**로 재실행 · NC2 · NC4 · Γ4.
