...
FORMAL (clean complete run): all 12 iters 0-11; last block = iter 11.
"""
import os
import numpy as np
import pandas as pd
...
print(f"\n=== stability of the ratio over the last 3 iters ({ni-3},{ni-2},{ni-1}) ===")
last = rb[rb["iter"] >= ni - 3]
agg = last.groupby(["ion", "shell"]).ratio.agg(["mean", "std", "min", "max"])
agg["spread"] = agg["max"] / agg["min"]
print(agg.round(3).to_string())

print(f"\n=== empty-bin FILL + MC-unsampled share of the GROUND edge kernel (iter {ni-1}) ===")
