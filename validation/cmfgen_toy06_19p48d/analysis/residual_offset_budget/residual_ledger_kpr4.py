#!/usr/bin/env python3
"""residual_ledger_kpr4.py -- kpr4 T_e warm-offset budget (OFFLINE).
Mirrors simul_r1 terms for the INTEGRATED-REPAIR run (PUMPF alpha-blend a=1 -> pump
reads mc_J; DBFB Wien on; stage4-r2). Shells s0/s2/s4/s6/s8. Committed field + pops +
CMFGEN sigma_bf. Reconstructs the ledger, the root, dr/dT, and field-swap dTe for each
offset candidate, incl. the 25.8% PUMPF cs-fallback residue.
Formulas identical to te_bias_budget/recon_terms.py (validated there)."""
import csv, math, struct, numpy as np, collections
...
lam_c=2.99792458e18/nu_mid
CS_FALLBACK_FRAC=7473805.0/(21503332.0+7473805.0)   # 0.2579 from PUMPF steady-state log
SHELLS=[0,2,4,6,8]
...
field={}; tmp=collections.defaultdict(lambda:[np.full(NFB,1e-30),np.full(NFB,1e-30)])
with open(f"{RUN}/lumina_coevolve_field.csv") as f:
    rd=csv.reader(f);next(rd)
    for row in rd:
        s=int(row[0]);b=int(row[1]); tmp[s][0][b]=float(row[3]);tmp[s][1][b]=float(row[4])
for s in SHELLS: field[s]=tmp[s]
state={}
...
def build_bf(s):
    W,Trad,ne,Te=state[s]; kT=KB*Te; pairs=[]
    ...
    for Z in Zs:
        ...
        for j in stages:
            ...
            gidx=np.array(gidx); E=lev_E[gidx]; g=lev_g[gidx]; hs=has_sig[gidx]; ln=lev_ln[gidx]
            x=E/kT; U=np.sum(g[x<50]*np.exp(-np.minimum(x[x<50],300)))
            if U<1.0: U=1.0
            sel=hs & (x<50) & ((chi_j-E)>0)
            if not sel.any(): continue
            gi=gidx[sel]; Es=E[sel]; gs=g[sel]; lns=ln[sel]; chil=chi_j-Es; nul=chil/H
            nlev=np.zeros(len(gi))
            for k in range(len(gi)):
                v=nk_map[s].get((Z,j,int(lns[k]))); nlev[k]=v if v is not None else nlo*gs[k]*math.exp(-Es[k]/kT)/U
...
def bf_term(pairs,Jarr,per_pair=None):
    tot=0.0
    for (Z,j,gi,chil,nlev,thr) in pairs:
        acc=0.0
        for k in range(len(gi)):
            l=gi[k]; b0=thr[k]; sr=sig[l,b0:]; m=sr>0
            if not m.any(): continue
            bb=np.arange(b0,NFB)[m]; w=4*math.pi*sr[m]*Jarr[bb]/hnu_mid[bb]*dnu[bb]
...
def Lambda(s,T,pop_map,Jarr,line_mask=None):
    ...
    contrib=dE*(nlo*qlu*ne-nup*qul*ne)
    if line_mask is not None: contrib=contrib*line_mask
...
# EUV-line mask (fallback lines are the sparse-bin ones -> EUV/short-wave)
euv_line=(lam_line<=912.0).astype(float)

results=[]
for s in SHELLS:
    W,Trad,ne,Te=state[s]
    print(f"\n{'='*74}\n### s{s}: T_e={Te:.0f}  CMFGEN={CMFTe[s]:.0f}  offset={Te-CMFTe[s]:+.0f} K | n_e={ne:.3e} W={W:.4f} Hdep={Hdep[s]:.3e}")
    pairs,_,_=build_bf(s); csJ,mcJ=field[s]; cmJ=cmfJ(s)
    pp={}
    Hph_mc=bf_term(pairs,mcJ,pp); Hph_cs=bf_term(pairs,csJ); Hph_cmf=bf_term(pairs,cmJ)
    Cfb_W=bf_term(pairs,Bwien(Te)); Cfb_P=bf_term(pairs,Bplanck(Te))
    pop_map={(Z,st):pops[s].get((Z,st),0.0) for (Z,st) in pops[s]}
    Lam_mc=Lambda(s,Te,pop_map,mcJ); Lam_cs=Lambda(s,Te,pop_map,csJ); Lam_cmf=Lambda(s,Te,pop_map,cmJ)
    # cs-fallback residue: 26% of lines pumped by cs instead of mc. EUV-line-restricted split as the likely-fallback subset.
    Lam_mc_euv=Lambda(s,Te,pop_map,mcJ,euv_line); Lam_cs_euv=Lambda(s,Te,pop_map,csJ,euv_line)
    cff=Cff(s,Te);cad=Cad(s,Te)
    print(f"  H_photo: mc={Hph_mc:+.3e}({100*Hph_mc/Hdep[s]:+5.1f}%) cs={Hph_cs:+.3e} CMFGEN={Hph_cmf:+.3e}")
    print(f"  C_fb(Wien)={Cfb_W:.3e}({100*Cfb_W/Hdep[s]:5.1f}%) C_fb(Planck)={Cfb_P:.3e} Wien-defect={Cfb_P-Cfb_W:.3e}({100*(Cfb_P-Cfb_W)/max(Cfb_W,1e-30):.2f}%)")
    print(f"  Lambda_line: mc={Lam_mc:+.3e}({100*Lam_mc/Hdep[s]:+5.1f}%) cs={Lam_cs:+.3e} CMFGEN={Lam_cmf:+.3e} | split(cs-mc)={Lam_cs-Lam_mc:+.3e}")
    print(f"  C_ff={cff:.3e}({100*cff/Hdep[s]:.2f}%) C_ad={cad:.3e}")
    top=sorted(pp.items(),key=lambda kv:-abs(kv[1]))[:5]
    print("  top bf pairs:",", ".join(f"Z{z}+{st}:{v:.2e}" for (z,st),v in top))
    # committed residual with kpr4 config: pump=mc (PUMPF a=1), DBFB Wien
    r_comm=Hdep[s]+Hph_mc-cff-cad-Cfb_W-Lam_mc
    def r_of(T):
        cW=bf_term(pairs,Bwien(T)); lam=Lambda(s,T,pop_map,mcJ)
        return Hdep[s]+Hph_mc-Cff(s,T)-Cad(s,T)-cW-lam
    dT=500.0; slope=(r_of(Te+dT)-r_of(Te-dT))/(2*dT)
    Tlo,Thi=6000.0,Te+3000; root=None
    if r_of(Tlo)>0:
        for _ in range(50):
            Tm=0.5*(Tlo+Thi)
            if r_of(Tm)>0: Tlo=Tm
        ...
        root=0.5*(Tlo+Thi)
    print(f"  residual r(Te)={r_comm:+.3e}({100*r_comm/Hdep[s]:+.1f}%) dr/dT={slope:+.3e}/K root={'%.0f'%root if root else None} K")
    # candidate dTe (dr converted via -dr/slope)
    # r = Hdep + Hph - Cff - Cad - Cfb - Lam.  Hph enters with +sign; Cfb,Lam with -sign.
    dr_C1=-(Cfb_P-Cfb_W); dT_C1=-dr_C1/slope                 # Cfb has -sign in r
    dr_pumpCMF=-(Lam_cmf-Lam_mc); dT_pumpCMF=-dr_pumpCMF/slope  # Lam has -sign in r
    dr_gphCMF=+(Hph_cmf-Hph_mc); dT_gphCMF=-dr_gphCMF/slope   # Hph has +sign in r (FIX: was -)
    # cs-fallback residue: 26% of lines get cs instead of mc. Bound with the EUV-line split (those are the sparse/fallback bins)
    dr_fbEUV=-CS_FALLBACK_FRAC*(Lam_cs_euv-Lam_mc_euv); dT_fbEUV=-dr_fbEUV/slope
    dr_fbALL=-CS_FALLBACK_FRAC*(Lam_cs-Lam_mc);         dT_fbALL=-dr_fbALL/slope
    print(f"  >> dTe candidates (K):  C1 Wien->Planck={dT_C1:+.0f} | pump mc->CMFGEN={dT_pumpCMF:+.0f} | Gph/Hex mc->CMFGEN={dT_gphCMF:+.0f}")
    print(f"     cs-fallback residue: EUV-lines(26%)={dT_fbEUV:+.0f} | all-lines(26%)={dT_fbALL:+.0f}")
    results.append(dict(s=s,Te=Te,cmf=CMFTe[s],offset=Te-CMFTe[s],ne=ne,W=W,Hdep=Hdep[s],
        Hph_mc=Hph_mc,Hph_cs=Hph_cs,Hph_cmf=Hph_cmf,Cfb_W=Cfb_W,Cfb_P=Cfb_P,
        Lam_mc=Lam_mc,Lam_cs=Lam_cs,Lam_cmf=Lam_cmf,cff=cff,cad=cad,r_comm=r_comm,slope=slope,root=root,
        dT_C1=dT_C1,dT_pumpCMF=dT_pumpCMF,dT_gphCMF=dT_gphCMF,dT_fbEUV=dT_fbEUV,dT_fbALL=dT_fbALL))
# csv
import json
with open(f"{REPO}/validation/cmfgen_toy06_19p48d/analysis/residual_offset_budget/ledger_shells.csv","w") as f:
    keys=list(results[0].keys()); f.write(",".join(keys)+"\n")
    for r in results: f.write(",".join(f"{r[k]:.4g}" if isinstance(r[k],float) else str(r[k]) for k in keys)+"\n")
print("\n[out] ledger_shells.csv written")
