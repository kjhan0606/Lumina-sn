...
int main(int argc, char *argv[]) {
    ...
    for (int iter = 0; iter < n_iterations; iter++) { /* Phase 5 - Step 5 */
        ...
        /* Phase 5 - Step 5b: Spectrum binning (serial, after parallel) */
        for (int p = 0; p < n_packets; p++) { /* Phase 5 - Step 5b */
            if (escaped_flag[p]) { /* Phase 5 - Step 5b */
                bin_escaped_packet(spec, escaped_nu[p], escaped_energy[p] * L_inner, /* Phase 5 - Step 5b */
                                    geo.time_explosion); /* Phase 5 - Step 5b */
            }
        }
        ...
        if (iter >= config.hold_iterations) { /* Phase 5 - Step 7 */
            ...
            update_t_inner(&config, escape_fraction); /* Phase 5 - Step 7 */
            printf("  T_inner: %.2f K -> %.2f K (escape=%.4f)\n", /* Phase 5 - Step 7 */
                   old_T, config.T_inner, escape_fraction); /* Phase 5 - Step 7 */
        } else { /* Phase 5 - Step 7 */
    ...
    if (argc > 4) output_file = argv[4]; /* Phase 5 - Step 9 */
